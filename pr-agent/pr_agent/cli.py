...
def run(inargs=None, args=None):
    ...
    async def inner():
        ...
        if not get_settings().config.publish_output and not get_settings().config.publish_combined_output:
            print('=== BEGIN RESULTS ARTIFACT ===', file=sys.stderr)
            print(dict(get_settings().data)['artifact'], file=sys.stderr)
            print('=== END RESULTS ARTIFACT ===', file=sys.stderr)
