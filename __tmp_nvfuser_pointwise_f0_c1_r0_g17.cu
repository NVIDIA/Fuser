...
__global__ void nvfuser_pointwise_f0_c1_r0_g17(Tensor<float, 2, 2> T19, Tensor<__bfloat, 3, 3> T9, Tensor<float, 3, 3> T20, Tensor<float, 3, 3> T165, Tensor<float, 3, 3> T206, Tensor<float, 3, 3> T212, Tensor<__bfloat, 3, 3> T245) {
  nvfuser_index_t i0;
  i0 = 4 * ((nvfuser_index_t)threadIdx.x);
  nvfuser_index_t i1;
  i1 = (4 * ((nvfuser_index_t)blockDim.x)) * ((nvfuser_index_t)blockIdx.x);
  nvfuser_index_t i2;
  i2 = (i0 + (12288 * ((nvfuser_index_t)blockIdx.y))) + i1;
  bool b3;
  b3 = ((3 + i0) + i1) < 12288;
  double d4;
  d4 = reciprocal(1.22880000000000000e+04);
  if ((((i0 + 3) + i1) < 12288)) {
    Array<float, 1, 1> T284;
    T284[0] = 0;
    T284[0]
       = T20[((nvfuser_index_t)blockIdx.y)];
    Array<float, 1, 1> T164;
    T164[0]
      = T284[0]
      * T284[0]
      * T284[0];
    ...
    T285[0] = 0;
    T285[0]
       = T165[((nvfuser_index_t)blockIdx.y)];
    Array<float, 1, 1> T167;
    T167[0]
      = T285[0]
      * T164[0];
    Array<float, 1, 1> T184;
    T184[0]
       = T167[0];
    Array<float, 1, 1> T188;
    T188[0]
    ...
    T282[0] = 0;
    T282[0]
       = T19[((nvfuser_index_t)blockIdx.y)];
    Array<float, 1, 1> T176;
    T176[0]
       = T282[0];
    Array<float, 1, 1> T182;
    T182[0]
       = T176[0];
    Array<__bfloat, 4, 4> T283;
    T283.set(__bfloat(0));
    loadGlobalToLocal<__bfloat, /*vec_size=*/4, /*is_volatile=*/false, CacheOp::Streaming>(&T283[0],  &T9[i2]);
    Array<float, 1, 1> T286;
    T286[0] = 0;
    T286[0]
       = T206[((nvfuser_index_t)blockIdx.y)];
    Array<float, 4, 4> T287;
    T287.set(float(0));
    loadGlobalToLocal<float, /*vec_size=*/4, /*is_volatile=*/false, CacheOp::Streaming>(&T287[0],  &T212[i2]);
    // Alias Allocation - register
    auto& T288 = T283;
    #pragma unroll
    for(nvfuser_index_t i5 = 0; i5 < 4; ++i5) {
      Array<float, 1, 1> T281;
      T281[0]
         = __bfloat2float(T283[i5]);
      Array<float, 1, 1> T187;
      T187[0]
        = T281[0]
        - T182[0];
      Array<float, 1, 1> T197;
      T197[0]
        = T188[0]
        * T187[0];
      Array<float, 1, 1> T205;
      T205[0]
        = T197[0]
        * (float) d4;
      Array<float, 1, 1> T211;
      T211[0]
        = T286[0]
        + T205[0];
      Array<float, 1, 1> T220;
      T220[0]
        = T287[i5]
        + T211[0];
      T288[i5]
         = __float2bfloat(T220[0]);
    }
    loadLocalToGlobal<__bfloat, /*vec_size=*/4, /*is_volatile=*/false>( &T245[i2], &T288[0]);
  } else {
    Array<float, 1, 1> T284;
    T284[0] = 0;
    T284[0]
