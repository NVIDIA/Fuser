...
def nvfuser_fusion_id0(fd : FusionDefinition) -> None :
    ...
    T2 = fd.define_tensor(shape=[-1], contiguity=[True], dtype=DataType.BFloat16, is_cpu=False)
    V3 = fd.ops.shape(T0)
    ...
    T40 = fd.ops.reshape(T15, new_shape=V38)
    T41 = fd.ops.broadcast_in_dim(T39, shape=V29, broadcast_dims=[0, 1, 2, 3, 4])
    T42 = fd.ops.mul(T33, T41)
    T43 = fd.ops.broadcast_in_dim(T40, shape=V29, broadcast_dims=[0, 1, 2, 3, 4])
    T44 = fd.ops.add(T42, T43)
    T45 = fd.ops.cast(T44, dtype=DataType.BFloat16)
    T46 = fd.ops.reshape(T45, new_shape=V3)
    fd.add_output(T46)
