...
def test_sdpa_backward():
    ...
    dqkv_tensor = torch.empty(math.prod(qkv_shape) * 3,
                              dtype=torch.bfloat16).cuda()
    dq_tensor = torch.as_strided(dqkv_tensor,
                                 qkv_shape,
                                 out_strides,
                                 storage_offset=0)
    dk_tensor = torch.as_strided(dqkv_tensor,
                                 qkv_shape,
                                 out_strides,
                                 storage_offset=h * stride_h)
    dv_tensor = torch.as_strided(dqkv_tensor,
                                 qkv_shape,
                                 out_strides,
                                 storage_offset=h * stride_h * 2)
    ...
    dq.set_output(True).set_dim(dq_tensor.size()).set_stride(
        dq_tensor.stride())
    dk.set_output(True).set_dim(dk_tensor.size()).set_stride(
        dk_tensor.stride())
    dv.set_output(True).set_dim(dv_tensor.size()).set_stride(
        dv_tensor.stride())
