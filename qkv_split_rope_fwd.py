...
def nvfuser_fusion_id0(fd : FusionDefinition) -> None :
    ...
    T11 = fd.ops.slice(T10, start_indices=[0, 0, 0, 0, 0], end_indices=[32, 32, 1, 4096, 128], strides=[1, 1, 1, 1, 1])
    T12 = fd.ops.slice(T10, start_indices=[0, 0, 1, 0, 0], end_indices=[32, 32, 2, 4096, 128], strides=[1, 1, 1, 1, 1])
    T13 = fd.ops.slice(T10, start_indices=[0, 0, 2, 0, 0], end_indices=[32, 32, 3, 4096, 128], strides=[1, 1, 1, 1, 1])
    ...
    T32 = fd.ops.slice(T19, start_indices=[0, 0, 0, 0], end_indices=[32, 32, 4096, 128], strides=[1, 1, 1, 1])
    T33 = fd.ops.slice(T32, start_indices=[0, 0, 0, 0], end_indices=[32, 32, 4096, 64], strides=[1, 1, 1, 1])
    T34 = fd.ops.slice(T32, start_indices=[0, 0, 0, 64], end_indices=[32, 32, 4096, 128], strides=[1, 1, 1, 1])
    ...
    T59 = fd.ops.slice(T25, start_indices=[0, 0, 0, 0], end_indices=[32, 32, 4096, 128], strides=[1, 1, 1, 1])
    T60 = fd.ops.slice(T59, start_indices=[0, 0, 0, 0], end_indices=[32, 32, 4096, 64], strides=[1, 1, 1, 1])
    T61 = fd.ops.slice(T59, start_indices=[0, 0, 0, 64], end_indices=[32, 32, 4096, 128], strides=[1, 1, 1, 1])
    ...
    T86 = fd.ops.slice(T19, start_indices=[0, 0, 0, 0], end_indices=[32, 32, 4096, 0], strides=[1, 1, 1, 1])
    T87 = fd.ops.cat([T58, T86], dim=-1)
    T88 = fd.ops.slice(T25, start_indices=[0, 0, 0, 0], end_indices=[32, 32, 4096, 0], strides=[1, 1, 1, 1])
...
inputs = [
    torch.randn((524288,), dtype=torch.bfloat16, device='cuda:0').as_strided((4096, 128), (128, 1)),
    torch.randn((1610612736,), dtype=torch.bfloat16, device='cuda:0').as_strided((32, 4096, 12288), (50331648, 12288, 1)),
    torch.randn((524288,), dtype=torch.bfloat16, device='cuda:0').as_strided((4096, 128), (128, 1)),
]
fd.execute(inputs)
