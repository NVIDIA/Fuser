...
__global__ void nvfuser_pointwise_f0_c1_r0_g9(Tensor<__bfloat, 4, 4> T135, Tensor<__bfloat, 4, 4> T136) {
  nvfuser_index_t i0;
  i0 = (((6144 * (((nvfuser_index_t)threadIdx.x) / 16)) + (8 * (((nvfuser_index_t)threadIdx.x) % 16))) + (49152 * (((nvfuser_index_t)blockIdx.x) % 256))) + (128 * (((nvfuser_index_t)blockIdx.x) / 256));
  nvfuser_index_t i1;
  ...
  b4 = ((7 + i1) + i2) < 12582912;
  if ((((i1 + 7) + i2) < 12582912)) {
    Array<__bfloat, 8, 8> T282;
    T282.set(__bfloat(0));
    loadGlobalToLocal<__bfloat, /*vec_size=*/8, /*is_volatile=*/false, CacheOp::Streaming>(&T282[0],  &T135[i0]);
    Array<__bfloat, 8, 8> T283;
    #pragma unroll
    for(nvfuser_index_t i5 = 0; i5 < 8; ++i5) {
      T283[i5]
         = T282[i5];
    }
    loadLocalToGlobal<__bfloat, /*vec_size=*/8, /*is_volatile=*/false>( &T136[i3], &T283[0]);
  } else {
    Array<__bfloat, 8, 8> T282;
    T282.set(__bfloat(0));
    if (b4) {
