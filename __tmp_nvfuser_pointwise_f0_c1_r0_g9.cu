...
__global__ void nvfuser_pointwise_f0_c1_r0_g9(Tensor<__bfloat, 4, 5> T134, Tensor<__bfloat, 4, 5> T137) {
  nvfuser_index_t i0;
  i0 = ((nvfuser_index_t)blockIdx.x) / 256;
  nvfuser_index_t i1;
  i1 = (((6144 * (((nvfuser_index_t)threadIdx.x) / 16)) + (8 * (((nvfuser_index_t)threadIdx.x) % 16))) + (49152 * (((nvfuser_index_t)blockIdx.x) % 256))) + (128 * i0);
  nvfuser_index_t i2;
  i2 = 8 * ((nvfuser_index_t)threadIdx.x);
  nvfuser_index_t i3;
  i3 = 1024 * ((nvfuser_index_t)blockIdx.x);
  nvfuser_index_t i4;
  i4 = i2 + i3;
  bool b5;
  b5 = ((7 + i2) + i3) < 25165824;
  if (((48 + i0) < 96)) {
    ...
    T284.set(__bfloat(0));
    loadGlobalToLocal<__bfloat, /*vec_size=*/8, /*is_volatile=*/false, CacheOp::Streaming>(&T284[0],  &T134[i1]);
    Array<__bfloat, 8, 8> T285;
    #pragma unroll
    for(nvfuser_index_t i6 = 0; i6 < 8; ++i6) {
      T285[i6]
         = T284[i6];
    }
    loadLocalToGlobal<__bfloat, /*vec_size=*/8, /*is_volatile=*/false>( &T137[i4], &T285[0]);
  } else {
    Array<__bfloat, 8, 8> T284;
    T284.set(__bfloat(0));
    if (b5) {
