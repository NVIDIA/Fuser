...
def dropout_layernorm_fwd_fusion(
    ...
    S9 = fd.define_scalar(dropout_p, dtype=DataType.Double)
    T10 = fd.ops.lt(T8, S9)
...
def test_dropout_layernorm_fwd_benchmark(
    ...
    if not disable_validation:
        ...
        eager_output = torch.nn.functional.layer_norm(
            inputs[0] + torch.nn.functional.dropout(inputs[0], dropout_p),
            inputs[0].shape[1:],
            weight=inputs[1],
            bias=inputs[2],
        )
        ...
        fd.validate(
            inputs, [eager_output.to(dtype), mean, invstd, dropout_mask.to(torch.float)]
        )
