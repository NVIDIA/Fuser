...
def norm_bwd_fusion(
    ...
    mean= fd.define_tensor(shape=[-1] * num_spatial_dims, contiguity=[True] * num_spatial_dims, dtype=DataType.Float, is_cpu=False)
    invstd = fd.define_tensor(shape=[-1] * num_spatial_dims, contiguity=[True] * num_spatial_dims, dtype=DataType.Float, is_cpu=False)
    ...
    if not disable_validation:
        ...
        if norm == "batch_norm":
            ...
            eager_output = torch.nn.functional.instance_norm(
                at_inputs, running_mean.to(dtype), running_var.to(dtype), weight=weight, bias=bias,
            )
