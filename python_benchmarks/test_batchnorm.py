...
def batchnorm_fwd_fusion(
    ...
    var = fd.ops.broadcast_in_dim(var, shape=V1, broadcast_dims=[1])
...
@pytest.mark.parametrize("size", [(16, 64, 32, 32)])
...
def test_batchnorm_fwd_benchmark(
    ...
    if not disable_validation:
        ...
        eager_output = torch.nn.functional.batch_norm(
            inputs[0], inputs[1], inputs[2], weight=inputs[3], bias=inputs[4], training=True,
        )
