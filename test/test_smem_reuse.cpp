...
// In this example we initially have two shared memory tensors A and B. We call
// promoteReuse on A so that B reuses its memory. Then we show that when an
// additional tensor C shaped the same as A is added after B, it aliases A,
// preventing the re-use of A for B's allocation.
TEST_F(SmemReuseTest, PromoteReuseAliasShadowed) {
  auto fusion = std::make_unique<Fusion>();
  FusionGuard fg(fusion.get());
  ...
  { // This should not re-use memory
    ...
    EXPECT_EQ(smem_usage, alignInt((H + 1) * 4) + H * 4);
  }

  { // Request that we re-use A for B
    tv0->promoteReuse();
    ...
    // High water mark has C stacked on top of B
    EXPECT_EQ(smem_usage, (H + 1) * 4);
  }

  { // Request that we re-use A for B, but add another smem tensor C that will
    // alias A, preventing re-use of A for B (but reusing A for C without any
    // inserted syncs). This changes the last aliased read time of A to come
    // after that of B, so in this case, A will stack on top of B, unlike in the
    // case without re-use and without C.

    // pad by negative one to trim back to size H
    auto tv4 = pad(tv3, {fusion->zeroVal(), neg(fusion->oneVal())});
    tv4->setMemoryType(MemoryType::Shared);
    ...
    tv0->promoteReuse(); // will not have any effect
    ...
    // B is stacked on A (which is aliased by C)
    EXPECT_EQ(smem_usage, alignInt(H * 4) + (H + 1) * 4);
  }
}
