...
TEST_F(AllocationDomainTest, NHWC4d_To_NHWC4d_PointwiseScheduler_CUDA) {
  ...
  auto has_vectorize = [](Val* val) {
    auto leaf = val->as<TensorView>()->getLeafDomain();
    return std::any_of(leaf.begin(), leaf.end(), [](IterDomain* id) {
      return id->getParallelType() == ParallelType::Vectorize;
    });
  };

  for (auto tv : fusion.outputs()) {
    ASSERT_TRUE(has_vectorize(tv));
  }
  for (auto tv : fusion.inputs()) {
    for (auto consumer_tv : ir_utils::consumerTvsOf(tv->as<TensorView>())) {
      ASSERT_TRUE(has_vectorize(consumer_tv));
    }
  }
