...
TEST_F(NVFuserTest, FusionSelectOpPointwise_CUDA) {
  ...
  auto index = IrBuilder::create<Scalar>(DataType::Int);
  fusion.addInput(tv0);
...
TEST_F(NVFuserTest, FusionIndexSelect_CUDA) {
  ...
  TensorView* tv3 = add(IrBuilder::create<Scalar>(17.0), tv2);
  fusion.addOutput(tv3);
...
TEST_F(NVFuserTest, FusionIndexSelect3DTv_CUDA) {
  ...
  TensorView* tv_sel = index_select(tv0, 0, tv_idx);
  TensorView* tv2 = mul(tv1, tv_sel);
  TensorView* tv3 = add(IrBuilder::create<Scalar>(27.0), tv2);
  fusion.addOutput(tv3);
...
TEST_F(NVFuserTest, FusionIndexSelectCanSch_CUDA) {
  ...
  TensorView* tv_sum_add = add(IrBuilder::create<Scalar>(17.0), tv_sum_2);
  ...
  TensorView* tv3_p = add(IrBuilder::create<Scalar>(17.0), tv2_p);
  // Register your outputs
...
TEST_F(NVFuserTest, FusionIndexSelect_Sum_CUDA) {
  ...
  TensorView* tv_sel = index_select(tv0, 0, tv_idx);
  TensorView* tv2 = mul(tv1, tv_sel);
  TensorView* tv_add = add(IrBuilder::create<Scalar>(17.0), tv2);
