    ...
    for (int64_t num_warps : {4, 8}) {
      for (int64_t vec_size : {4, 2, 1}) {
        ...
        for (int64_t B : {8}) {
          ...
          //   iTIDy{num_warps}, iTIDx{32}, iV{vec_size} rBIDz{blocks_z} ]
          auto tv2 = tv0->cacheAfter();
          auto tv3 = tv1->cacheBefore();

          tv3->reorder(
              {{1, 0},
               {0, 1}}); // blocks_x*blocks_y*A*B*num_warps*32*vec_size, H
          tv3->split(
              0, vec_size); // blocks_x*blocks_y*A*B*num_warps, 32, vec_size, H
          tv3->split(0, 32); // blocks_x*blocks_y*A*B*num_warps, 32, vec_size, H
          tv3->split(
              0,
              num_warps); // blocks_x*blocks_y*A*B, num_warps, 32, vec_size, H
          tv3->split(
              0, B); // blocks_x*blocks_y*A, B, num_warps, 32, vec_size, H
          tv3->split(
              0, A); // blocks_x*blocks_y, A, B, num_warps, 32, vec_size, H
          tv3->split(
              0,
              blocks_y); // blocks_x, blocks_y, A, B, num_warps, 32, vec_size, H
          ...
          if (serial) {
            fe.registerPostLoweringHook([](kir::Kernel* kernel) {
              ...
              // This is a poor approximation of a traversal that would appear
              // in a lowering pass to both set the isSerial() flag on grid
              ...
              // Store the TensorIndex for the output tensor T1_g, so that we
              // can re-use its index
              auto t1_idx =
                  output_store_expr->output(0)->as<kir::TensorIndex>();
              ...
              // here, but this works for this test since the T1 store is
              // inlined fully with the serial grid reduction.
              ...
              new_grop =
                  new_grop->withWritePredicate(old_grop->writePredicate())
                      ->as<kir::GridReduction>();
