...
// Test non-persistent softmax
// kernel duration reported by nsys on ipp2-0123 is 0.935 ms.
TEST_F(NVFuserTest, NonPersistentSoftmax) {
  ...
  // load and calc sum(exp(x-bcast_max))
  ...
  // load and calc exp(x-xmax)/bcast_sum
  auto x3 = set(x, CacheOp::Streaming);
  ...
  auto y_out = set(y_cast, CacheOp::Streaming);
