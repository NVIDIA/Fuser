...
TEST_F(NVFuserTest, FusionConstCheck_CUDA) {
  ...
  auto one = IrBuilder::create<Scalar>(1);
...
TEST_F(NVFuserTest, FusionUnrollWithAlloc_CUDA) {
  ...
  TensorView* tv1 = add(tv0, IrBuilder::create<Scalar>(0.0));
  TensorView* tv2 =
      reductionOp(BinaryOpType::Add, {1}, IrBuilder::create<Scalar>(0.0), tv1);
...
TEST_F(NVFuserTest, FusionIsZeroInt_CUDA) {
  ...
  Scalar* x = IrBuilder::create<Scalar>(0);
  Scalar* y = IrBuilder::create<Scalar>(1);
  Val* z = mul(x, y);
...
TEST_F(NVFuserTest, FusionIsOneInt_CUDA) {
  ...
  Scalar* x = IrBuilder::create<Scalar>(1);
  Scalar* y = IrBuilder::create<Scalar>(1);
  Val* z = mul(x, y);
...
TEST_F(NVFuserTest, FusionComputeAtNonterminatingOutput_CUDA) {
  ...
  // tv1 -> tv3 -> tv4
  auto tv3 = add(tv1, IrBuilder::create<Scalar>(3.0));
...
TEST_F(NVFuserTest, FusionTraversalOrder2_CUDA) {
  ...
  TensorView* tv3 = add(tv0, IrBuilder::create<Scalar>(3.0));
...
TEST_F(NVFuserTest, FusionTraversalOrder4_CUDA) {
  ...
  fusion.addInput(tv4);
  TensorView* tv5 = add(tv4, IrBuilder::create<Scalar>(5.0));
  TensorView* tv6 = add(tv5, IrBuilder::create<Scalar>(6.0));
  TensorView* tv7 = add(tv5, IrBuilder::create<Scalar>(7.0));
  fusion.addOutput(tv6);
...
TEST_F(NVFuserTest, FusionThreadPredicate_CUDA) {
  ...
  TensorView* tv3 = add(tv0, IrBuilder::create<Scalar>(2.0));
...
TEST_F(NVFuserTest, FusionReduceImplicitBroadcast_CUDA) {
  ...
  TensorView* tv1 = reductionOp(
      BinaryOpType::Add, {red_dim, 2}, IrBuilder::create<Scalar>(0.0), tv0);
  fusion.addOutput(tv1);
...
TEST_F(NVFuserTest, FusionReduceImplicitBroadcast2_CUDA) {
  ...
  TensorView* tv1 =
      reductionOp(BinaryOpType::Add, {2}, IrBuilder::create<Scalar>(0.0), tv0);

  TensorView* tv2 = reductionOp(
      BinaryOpType::Add, {red_dim}, IrBuilder::create<Scalar>(0.0), tv1);
  fusion.addOutput(tv2);
...
TEST_F(NVFuserTest, FusionBiasGeluFwd_CUDA) {
  ...
  auto t5 = add(t4, t3);
  auto t6 = mul(t5, IrBuilder::create<Scalar>(0.5));
  auto t7 = mul(t5, IrBuilder::create<Scalar>(k_079));
  auto t8 = mul(t5, IrBuilder::create<Scalar>(k_004));
  auto t9 = mul(t8, t5);
  auto t10 = add(t9, IrBuilder::create<Scalar>(1));
  auto t11 = mul(t7, t10);
  auto t12 = unaryOp(UnaryOpType::Tanh, t11);
  auto t13 = add(t12, IrBuilder::create<Scalar>(1));
  auto t14 = mul(t6, t13);
...
TEST_F(NVFuserTest, FusionIssue459_CUDA) {
  ...
  fusion.addOutput(tv5);
  auto tv6 = add(tv4, IrBuilder::create<Scalar>(1.0));
  fusion.addOutput(tv6);
...
TEST_F(NVFuserTest, FusionSmemIndexing_CUDA) {
  ...
  Scalar* symbolic_m_tile_dim = IrBuilder::create<Scalar>(DataType::Int);
  Scalar* symbolic_split_k_tile_dim = IrBuilder::create<Scalar>(DataType::Int);
  Scalar* symbolic_block_k_tile_dim = IrBuilder::create<Scalar>(DataType::Int);
...
TEST_F(NVFuserTest, FusionIssue484_CUDA) {
  ...
  fusion.addInput(tv0);
  auto tv1 = sum(tv0, {1});
  auto tv2 = add(tv1, IrBuilder::create<Scalar>(0.0));
  fusion.addOutput(tv2);
...
TEST_F(NVFuserTest, FusionIssue532_CUDA) {
  ...
  TensorView* tv0 = makeSymbolicTensor(1);
  TensorView* tv1 = add(tv0, IrBuilder::create<Scalar>(1.0));
  TensorView* tv2 = add(tv1, IrBuilder::create<Scalar>(1.0));
  fusion.addInput(tv0);
  fusion.addOutput(tv2);
...
TEST_F(NVFuserTest, FusionWelfordOp_CUDA) {
  ...
  auto tv0 = makeSymbolicTensor(2);
  fusion.addInput(tv0);
  auto tv1 = mul(tv0, IrBuilder::create<Scalar>(1.0));
...
void testWelford(DataType dtype, int red_axis, int odim, int rdim) {
  ...
  fusion.addInput(tv0);
  auto tv1 = mul(tv0_cast, IrBuilder::create<Scalar>(1.0));
...
TEST_F(NVFuserTest, FusionDAGMerging_CUDA) {
  ...
  // Branch 1
  auto tv6 = add(tv1, IrBuilder::create<Scalar>(1.0)); // 4
...
TEST_F(NVFuserTest, FusionDAGScalarMerging_CUDA) {
  ...
  auto tv0 = makeSymbolicTensor(3);
  auto i0 = IrBuilder::create<Scalar>(DataType::Double);

  fusion->addInput(tv0);
  fusion->addInput(i0);

  auto i1 = add(i0, IrBuilder::create<Scalar>(1.0));
  auto i2 = mul(i1, i1);
  auto i3 = add(i2, i1);
...
TEST_F(NVFuserTest, FusionIssue728_CUDA) {
  ...
  auto tv4 = add(tv3, tv1);
  auto tv5 = add(tv4, IrBuilder::create<Scalar>(1.0));
  auto tv6 = add(tv2, IrBuilder::create<Scalar>(1.0));
  fusion.addOutput(tv5);
  fusion.addOutput(tv6);
...
TEST_F(NVFuserTest, FusionSingleElement_CUDA) {
  ...
  auto tv1 = add(tv0, IrBuilder::create<Scalar>(2.5));

  auto tv2 = add(tv1, IrBuilder::create<Scalar>(3.5));
  fusion.addOutput(tv2);
...
TEST_F(NVFuserTest, FusionBNBackwardRepro_CUDA) {
  ...
  auto gt_bool = binaryOp(BinaryOpType::GT, gt_0, IrBuilder::create<Scalar>(1));
  auto gt_float = castOp(DataType::Float, gt_bool);

  auto grad_out = mul(grad_out_prev, gt_float);

  Val* eps_ptr = IrBuilder::create<Scalar>(1e-5);
...
TEST_F(NVFuserTest, FusionBNRepro2_CUDA) {
  ...
  Val* momentum_ptr = IrBuilder::create<Scalar>(kMomentum);
  Val* eps_ptr = IrBuilder::create<Scalar>(kEps);
...
TEST_F(NVFuserTest, FusionZeroSizeTensorPW_CUDA) {
  ...
  auto tv2 = add(tv0, IrBuilder::create<Scalar>(2.5));
  fusion.addOutput(tv2);
...
TEST_F(NVFuserTest, FusionSegmentIoAlias_CUDA) {
  ...
  // fusion->addOutput(tv6);
  TensorView* tv7 = add(tv6, IrBuilder::create<Scalar>(1.0)); // Group 0
  fusion->addOutput(tv7);
...
TEST_F(NVFuserTest, FusionTrivialWarpReduction_CUDA) {
  ...
  auto tv1 = reductionOpRaw(
      BinaryOpType::Add, {1, 2, 3}, IrBuilder::create<Scalar>(0.0), tv0);
...
TEST_F(NVFuserTest, FusionSegfaultReduction_CUDA) {
  ...
  Val* N = IrBuilder::create<Scalar>(1.0);
...
TEST_F(NVFuserTest, FusionPredicateElimination8_CUDA) {
  ...
  Val* num_features = IrBuilder::create<Scalar>(tv1->container(), 1.0);
  ...
  auto tv6 = expand(
      tv2,
      {IrBuilder::create<Scalar>(batch_size),
       IrBuilder::create<Scalar>(channel_size),
       IrBuilder::create<Scalar>(hw_size),
       IrBuilder::create<Scalar>(hw_size)});
  auto tv7 = div(tv6, IrBuilder::create<Scalar>(3136.0));
...
TEST_F(NVFuserTest, FusionBufferReuseBroadCastMultiVisit_CUDA) {
  ...
  auto tv2 = mul(tv0, IrBuilder::create<Scalar>(2.0));
  auto tv3 = broadcast(tv2, {false, false, true});
  auto tv4 = add(tv3, tv1);
  auto tv5 = mul(tv4, IrBuilder::create<Scalar>(3.0));
  fusion->addOutput(tv5);
...
TEST_F(NVFuserTest, FusionBufferReuseLargeBuffer_CUDA) {
  ...
  auto tv1 = mul(tv0, IrBuilder::create<Scalar>(2.0));
  auto tv2 = mul(tv1, IrBuilder::create<Scalar>(2.0));
  auto tv3 = mul(tv2, IrBuilder::create<Scalar>(2.0));
  auto tv4 = mul(tv3, IrBuilder::create<Scalar>(2.0));
  auto tv5 = mul(tv4, IrBuilder::create<Scalar>(2.0));
  auto tv6 = mul(tv5, IrBuilder::create<Scalar>(2.0));
...
TEST_F(NVFuserTest, FusionBufferReuseNo2hop_CUDA) {
  ...
  auto tv5 = mul(tv4, IrBuilder::create<Scalar>(3.0));
  auto tv6 = mul(tv5, IrBuilder::create<Scalar>(3.0));
  fusion->addOutput(tv6);
...
TEST_F(NVFuserTest, FusionBufferReuseLiveInterval_CUDA) {
  ...
  auto tv1 = mul(tv0, IrBuilder::create<Scalar>(3.0));
  auto tv2 = mul(tv1, IrBuilder::create<Scalar>(2.0));
  auto tv3 = mul(tv2, IrBuilder::create<Scalar>(2.0));
...
TEST_F(NVFuserTest, FusionBufferReuseNoAcrossBroadcast_CUDA) {
  ...
  auto tv7 = mul(tv6, IrBuilder::create<Scalar>(7.0));
  fusion->addOutput(tv7);
...
TEST_F(NVFuserTest, FusionParallelDimensionMap3_CUDA) {
  ...
  auto tv4 = add(tv0, IrBuilder::create<Scalar>(1.0));
  fusion->addOutput(tv4);
...
TEST_F(NVFuserTest, FusionSegmenterCombineReductionsCycleRepro_CUDA) {
  ...
  auto d56 = IrBuilder::create<Scalar>(DataType::Double);
  ...
  auto d59 =
      mul(t1->getRootDomain()[2]->extent(), IrBuilder::create<Scalar>(1.0));
  auto t26 = mul(d59, t25);
  auto txx = mul(t26, IrBuilder::create<Scalar>(1.0));
...
TEST_F(NVFuserTest, FusionPredicateParallelizedDomains_CUDA) {
  ...
  auto tv5 = add(tv4, IrBuilder::create<Scalar>(1.0));
  auto tv6 = add(tv5, IrBuilder::create<Scalar>(1.0));
  auto tv7 = add(tv6, IrBuilder::create<Scalar>(1.0));
...
TEST_F(NVFuserTest, FusionSmemPredicateUnswitch_CUDA) {
  ...
  // Just to make TIDx/y/z non-exact
  auto tvx = add(tv1, IrBuilder::create<Scalar>(1.0));
  auto tvy = add(tvx, IrBuilder::create<Scalar>(1.0));
  auto tvz = add(tvy, IrBuilder::create<Scalar>(1.0));
...
TEST_F(NVFuserTest, FusionIssue1223_CUDA) {
  ...
  auto tv3 = add(tv0, IrBuilder::create<Scalar>(0.0));
  fusion.addOutput(tv3);
