...
#define __CALL_GET_STARTS_KERNEL_BLOCKSCALE(                     \
    ELEMENT_AB_TYPE, TENSOR_C_TYPE, C_TYPE)                      \
  else if (out_tensors.dtype() == TENSOR_C_TYPE) {               \
    __get_group_gemm_starts<ELEMENT_AB_TYPE, C_TYPE, float>      \
...
void run_get_group_gemm_starts(
  ...
  __CALL_GET_STARTS_KERNEL_BLOCKSCALE(
      cutlass::bfloat16_t, torch::kBFloat16, cutlass::bfloat16_t)
  __CALL_GET_STARTS_KERNEL_BLOCKSCALE(
      cutlass::half_t, torch::kFloat16, cutlass::half_t)
  else {
...
void run_grouped_mm(
  ...
  using ElementType = TensorType;
  using ElementA = TensorType;
  using ElementB = TensorType;

  using ElementC = TensorType;
...
#define CHECK_TH_CUDA(x, m) \
  NVF_CHECK(x.is_cuda(), m, ": must be a CUDA tensor.")
...
void grouped_mm(
  ...
  if (sm_version == 100) {
    // Input validation
    CHECK_INPUT(a, at::ScalarType::BFloat16, "a");
    CHECK_INPUT(b, at::ScalarType::BFloat16, "b");
    ...
    if (output.scalar_type() == torch::kBFloat16) {
      run_grouped_mm<cutlass::bfloat16_t>(
          output,
          a,
          b,
          alphas,
  ...
  NVF_CHECK(
      can_implement,
      "grouped_mm is not implemented for current compute "
      "capability: ",
      sm_version);
}
