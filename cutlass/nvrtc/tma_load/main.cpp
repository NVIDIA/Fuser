...
int main() {
  ...
  auto d_in = at::randn({128, 64}, options);
  auto d_out = at::empty({128, 64}, options);

  Layout gmem_layout = make_layout(make_shape(128, 64));
  Layout smem_layout = make_layout(Shape<_64, _32>{});
  auto cta_tile = Shape<_64, _32>{};
  int32_t smem_size =
      static_cast<int32_t>(sizeof(SharedStorage<float, decltype(smem_layout)>));

  Tensor gA = make_tensor(
      make_gmem_ptr<float>(raw_pointer_cast(d_in.data_ptr())), gmem_layout);
  auto tma = make_tma_copy<float>(
      SM90_TMA_LOAD{}, gA, smem_layout, cta_tile, Int<1>{});

  // 1. Read the CUDA source file
  std::string filename = "tma_load.cu";
  ...
  std::string name_expression = "cute_tma_copy<float," +
      type_to_string<decltype(tma)>() + "," +
      type_to_string<decltype(cta_tile)>() + "," +
  ...
  data.push_back(convertToBytes(tma));
  pointers.emplace_back(data.back().data());

  data.push_back(convertToBytes(cta_tile));
  pointers.emplace_back(data.back().data());
