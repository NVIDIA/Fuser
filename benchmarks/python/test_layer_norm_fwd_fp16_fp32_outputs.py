...
def layernorm_fwd_fp16_fp32_outputs_iobytes(size: tuple, dtype: torch.dtype):
    ...
    return int(
        ...
        + torch.float.itemsize * (2 * size[0] + np.prod(size))
    )
...
@pytest.mark.parametrize("size", generate_input_sizes(dims=2))
@pytest.mark.parametrize("dtype", FP16_DTYPES)
def test_layernorm_fwd_fp16_fp32_outputs_baseline_benchmark(
    benchmark,
    size: tuple,
    dtype: torch.dtype,
    compile: bool,
    ...
    run_benchmark(
        benchmark,
        torch.compile(layernorm_fwd_fp16_fp32_outputs) if compile else layernorm_fwd_fp16_fp32_outputs,
        inputs,
        iobytes=layernorm_fwd_fp16_fp32_outputs_iobytes(size, dtype),
    )
