...
import torch
from .global_params import generate_input_sizes, FP16_DTYPES, PROMOTE_DTYPES
import numpy as np


def layernorm_bwd_fp16_fp32_outputs_fusion(
    fd: FusionDefinition,
    dtype: DataType,
) -> None:
    T0 = fd.define_tensor(
    ...
    if dtype in PROMOTE_DTYPES:
        ...
        T90 = fd.ops.cast(T90, dtype=dtype) # 2D fp16 output
...
def test_layernorm_bwd_nvf_benchmark(
    ...
    with FusionDefinition() as fd:
        layernorm_bwd_fp16_fp32_outputs_fusion(fd, torch_dtype_to_nvfuser_dtype(dtype))
