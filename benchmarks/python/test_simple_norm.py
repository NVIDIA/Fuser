...
def nvfuser_simple_norm_fusion(fd: FusionDefinition, batch_size: int, vocab_size: int) -> None:
    T0 = fd.define_tensor(shape=[batch_size, vocab_size], contiguity=[True, True], dtype=DataType.BFloat16, is_cpu=False, stride_order=[1, 0])
    ...
    T11 = fd.ops.broadcast_in_dim(T4, shape=[batch_size, vocab_size], broadcast_dims=[0, 1])
