...
def reduction_fusion(
    ...
    T5 = fd.ops.add(T2, T3)
    T6 = fd.ops.add(T4, T5)
    T7 = fd.ops.sum(T6, dims=[reduction_axis], keepdim=False)
    if dtype in PROMOTE_DTYPES:
        T7 = fd.ops.cast(T7, dtype=dtype)
    fd.add_output(T7)


def reduction_fwd_fn(inputs: list):  # in_tensor, reduction_axis
    return torch.sum(inputs[0]+inputs[1]+inputs[2]+inputs[3], dim=inputs[4])
...
def test_reduction_nvf_benchmark(
    ...
    input0 = torch.randn(size, device="cuda", dtype=dtype)
    input1 = torch.randn(size, device="cuda", dtype=dtype)
    input2 = torch.randn(size, device="cuda", dtype=dtype)
    input3 = torch.randn(size, device="cuda", dtype=dtype)
    ...
    if not disable_validation:
        eager_output = reduction_fwd_fn([input0, input1, input2, input3, reduction_axis])
...
def test_reduction_baseline_benchmark(
    ...
    run_benchmark(
        ...
        [input0, input1, input2, input3, reduction_axis],
    )
