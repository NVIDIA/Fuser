...
def chunk_silu_fwd_fusion(fd: FusionDefinition, dtype: DataType, size: tuple):
    T0 = fd.define_tensor(
        shape=[-1, -1],
...
def chunk_silu_fwd_fn(inputs: list):  # [in_tensor]
...
def test_chunk_silu_fwd_nvf_benchmark(
    benchmark,
    size: tuple,
    dtype: torch.dtype,
    ...
    with FusionDefinition() as fd:
        chunk_silu_fwd_fusion(fd, torch_dtype_to_nvfuser_dtype(dtype), size=size)
    if not disable_validation:
        eager_output = chunk_silu_fwd_fn(inputs)
...
@pytest.mark.parametrize("size", generate_input_sizes(dims=2))
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_chunk_silu_fwd_baseline_benchmark(
    benchmark,
    size: tuple,
    dtype: torch.dtype,
    compile: bool,
    ...
    run_benchmark(
        benchmark,
        torch.compile(chunk_silu_fwd_fn) if compile else chunk_silu_fwd_fn,
        inputs,
    )
