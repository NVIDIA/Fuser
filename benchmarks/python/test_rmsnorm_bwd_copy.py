...
def rmsnorm_bwd_fusion(fd : FusionDefinition) -> None :
    T0 = fd.define_tensor(shape=[-1, -1], contiguity=[True, True], dtype=DataType.BFloat16, is_cpu=False)
    T1 = fd.define_tensor(shape=[-1, -1], contiguity=[True, True], dtype=DataType.BFloat16, is_cpu=False)
    T2 = fd.ops.cast(T0, dtype=DataType.Float)
    T3 = fd.ops.cast(T1, dtype=DataType.Float)
    T4 = fd.ops.mul(T3, T2)
    T5 = fd.ops.sum(T4, dims=[0], keepdim=False, dtype=DataType.Null)
    T6 = fd.ops.sum(T2, dims=[1], keepdim=False, dtype=DataType.Null)
    V7 = fd.ops.shape(T2)
    T8 = fd.ops.broadcast_in_dim(T6, shape=V7, broadcast_dims=[0])
    T9 = fd.ops.mul(T8, T2)
    T10 = fd.ops.add(T3, T9)
    T11 = fd.ops.cast(T10, dtype=DataType.BFloat16)
    T12 = fd.ops.cast(T5, dtype=DataType.BFloat16)
    fd.add_output(T11)
    fd.add_output(T12)
