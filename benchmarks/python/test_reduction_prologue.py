...
def reduction_fusion(
    ...
    T1 = fd.ops.tanh(T0)
    T2 = fd.ops.sum(T1, dims=[reduction_axis], keepdim=False)
    if dtype in PROMOTE_DTYPES:
...
def reduction_fwd_fn(inputs: list):  # in_tensor, reduction_axis
    return torch.sum(torch.tanh(inputs[0]), dim=inputs[1])
...
def test_reduction_nvf_benchmark(
    ...
    if not disable_validation:
        eager_output = reduction_fwd_fn([input, reduction_axis])
        fd.validate([input], [eager_output.to(dtype)])

    if not disable_benchmarking:
        run_benchmark(benchmark, fd.execute, [input])
