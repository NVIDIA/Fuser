...
bool isSamePadOp(Val* use, PadOp* p) {
  ...
  if (use_pad->getPaddedAxes() != padde_axes) {
    return false;
  }
...
void propagatePad(Fusion* fusion) {
  ...
  while (!frontier.empty()) {
    ...
    auto pad_replay_check = [&pad_dependencies](Expr* expr) {
      return std::if_all(expr->inputs().begin(), expr->inputs().end(), [&pad_dependencies = std::as_const(pad_dependencies)](Val* val) {
        if (!val->isA<TensorView>()) {
          return false;
        }
        if (std::any_of(
