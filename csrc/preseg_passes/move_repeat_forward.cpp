...
class CanMovePast : public OptOutConstDispatch {
 public:
  CanMovePast(std::unordered_map<TensorView*, IterDomain*>& repeat_id_map,
              Expr* expr): repeat_id_map_(repeat_id_map) {
    std::cerr<< "CanMovePast: " << expr->toString();

    // If no input has info on repeat IDs, this expr should not matter
    if (std::ranges::none_of(expr->inputs(),
                             [&] (Val* inp) {
  ...
  void handleNoSideEffectOp(const Expr* op) {
    ...
    auto inp_it = std::ranges::find_if(inp_tvs,
                                       [&] (TensorView* inp) {
                                         return repeat_id_map_.find(inp) !=
                                             repeat_id_map_.end();
                                       });
    NVF_ERROR(inp_it != inp_tvs.end());

    TensorView* first_repeat_inp = *inp_it;
    
    const auto inp_map = PairwiseLogicalDomainMap(first_repeat_inp, out_tv).mapProducerToConsumer();
    auto inp_map_it = inp_map.find(repeat_id_map_.at(first_repeat_inp));
    NVF_ERROR(inp_map_it != inp_map.end(), "Cannot find a p2c mapping for ", repeat_id_map_.at(first_repeat_inp)->toString());
    IterDomain* out_repeat_id = inp_map_it->second;
    
    for (auto inp_tv: ir_utils::filterByType<TensorView>(op->inputs())) {
      if (inp_tv == first_repeat_inp) {
        continue;
      }

      const auto c2p_map = PairwiseLogicalDomainMap(inp_tv, out_tv).mapBroadcast(true).mapConsumerToProducer();
      ...
      if (auto repeat_id_map_it = repeat_id_map_.find(inp_tv);
        ...
        // Only possible to move when the other inputs are broadcast
  ...
  void handleResizeBasedOp(const Expr* op) {
    ...
    auto it = std::ranges::find(inp_tv->getLogicalDomain(),
                                inp_repeat_id);
    NVF_ERROR(it != inp_tv->getLogicalDomain().end(),
              "Repeat ID not found in logical domain. ",
              inp_tv->toString(), ", logical: ",
              toDelimitedString(inp_tv->getLogicalDomain()),
              ", repeat ID: ", inp_repeat_id->toString());
  ...
  void handle(const CatOp* cat_op) override {
    for (auto inp: cat_op->inputs()) {
      ...
      auto repeat_id_pos = std::distance(logical.begin(),
                                         std::ranges::find(logical,
                                                           repeat_id_map_it->second));
    ...
    repeat_id_map_[out_tv] = out_tv->getLogicalDomain().at(cat_op->concatenatedDim());
...
class MoveRepeatForward {
 ...
 private:
  std::optional<scheduler_tools::StaticRepeatInfo> getMaybeStaticRepeat() {
    ...
    auto getStraightUseExprs = [] (Val* inp, int num_exprs) -> std::vector<Expr*> {
    ...
    for (auto tv: fusion_->allTvs()) {
      ...
      auto static_repeat_info = scheduler_tools::getMaybeStaticRepeatInfo(repeat_out);
  ...
  std::tuple<TensorView*, IterDomain*,
  ...
  findMoveTarget(const scheduler_tools::StaticRepeatInfo& info) {
    ...
    auto all_exprs = DependencyCheck::getAllExprsBetween({info.repeat_output_tv},
                                                         fusion->outputs());
    ...
    repeat_id_map.emplace(info.reshape_output_tv,
                          repeat_out_id);
    ...
    for (const auto& post_dominator: post_dominators | std::views::reverse) {
      NVF_ERROR(post_dominator->isA<TensorView>(),
                "Expected to be a tensor: ", post_dominator->toString());
  ...
  std::vector<Val*> getAllPostDominators(TensorView* tv) const {
    std::deque<std::deque<Val*>> all_use_chains = DependencyCheck::getAllUseChains(tv);
    ...
    for (const auto& use_chain: all_use_chains) {
      all_use_val_sets.emplace_back(
          use_chain.begin(), use_chain.end());
    }

    const auto& use_chain = all_use_chains.at(0);
    std::vector<Val*> post_dominators;
    for (const auto& val: use_chain) {
      if (std::ranges::all_of(arange(all_use_chains.size()),
                              [&] (int i) {
  ...
  void moveTo(const StaticRepeatInfo& info,
    ...
    auto repeat_factor = info.reshape_repeat_id->extent()->evaluate().as<int64_t>();
    auto target_logical = TensorDomain::noReductions(target_tv->getLogicalDomain());
    ...
    for (auto val_to_update :DependencyCheck::getAllValsBetween({info.reshape_output_tv},
                                                                {target_tv})) {
      ...
      NVF_ERROR(tv_to_update != nullptr,
                "Unexpected val: ", val_to_update->toString());

      IterDomain* repeat_id = repeat_id_map.at(tv_to_update);
      auto new_id = IterDomainBuilder(repeat_id).extent(
          info.repeat_input_id->extent()).build();
    ...
    ir_utils::replaceValInAllExprInputsAndFusionOutputs(target_tv,
                                                        repeated_tv);

    // Remove the original repeat exprs
    ir_utils::replaceValInAllExprInputsAndFusionOutputs(info.repeat_output_tv,
                                                        info.repeat_input_tv);
    std::cout << std::endl;
    std::cerr << "Moved\n";
    fusion_->printMath();
    std::cout << std::endl;
