...
void ParallelDimensionMap::adjustMappingsForWarpSpecialization() {
  ...
  for (ParallelType pt : other_active_pts) {
    int64_t thread_count_for_pt = get_threads_count_in_dim(pt);
    NVF_ERROR(
  ...
  int64_t pad_n_threads =
      scheduler_utils::safeDiv(128, other_active_pts_threads);
  int64_t after_pad = get_threads_count_in_dim(ws_pt) + pad_n_threads;
  NVF_ERROR(
  ...
  auto offset = IrBuilder::create<Val>(pad_n_threads, DataType::Index);
