...
class FusionTranslator : public OptInConstDispatch {
 ...
 private:
  ...
  // Determine if this PadOp is associated with a CatOp. If so, avoid adding the
  // PadOp to the FusionDefinition.
  bool skipPadOp(const Expr* e) {
    if (!e->isA<PadOp>()) {
      return false;
    }
    const PadOp* pad_op = e->as<PadOp>();
    TensorView* tv = pad_op->out()->as<TensorView>();
    return tv->uses().size() == 1 && tv->uses().front()->isA<CatOp>() &&
        (map_val_to_fd_index_.count(pad_op->in()) > 0);
  }
  ...
  void translate() {
    ...
    while (!to_visit.empty()) {
      ...
      if (skipPadOp(e)) {
        visited.insert(e);
        skip_count = 0;
        size_t input_fid = map_val_to_fd_index_.at(e->inputs().front());
        map_val_to_fd_index_.emplace(e->outputs().front(), input_fid);
        continue;
      }
