...
void PrecomputedValues::bindTensorMetaData(
  ...
  for (const auto dim : c10::irange(root_domain.size())) {
    auto value = tensor.size((int64_t)dim); // Possible sharded size. 
    auto root_id = root_domain[dim];
    auto extent = root_id->extent();

    // TODO: Lots of duplication with expr_evaluator::bind. 
    const auto all_exp = DependencyCheck::getAllExprsBetween(
        {root_id, root_id}, {alloc_domain.begin(), alloc_domain.end()});
    if (!all_exp.empty()) {
    ...
    else if (root_id->hasExpandedExtent()) {
      auto expanded_extent = root_id->expandedExtent();
