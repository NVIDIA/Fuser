...
// Parse the environment to retrieve MPI rank and MPI world size and sets rank and size accordingly
...
c10::intrusive_ptr<c10d::Backend> createBackend(CommunicatorBackend backend,
                                  ::c10::intrusive_ptr<c10d::TCPStore> store,
                                  RankType rank, size_t size) {
#ifdef USE_C10D_NCCL
...
Communicator::Communicator(CommunicatorBackend backend,
                            RankType server_rank) {
...
void Communicator::sendRecv(RankType receiver_rank,
