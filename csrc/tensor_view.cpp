...
void TensorView::buildAllocationDomainFromLoopIds() {
  const auto& logical = getLogicalDomain();
  const auto& alloc = getMaybeAllocationDomain();
  NVF_ERROR(
      std::is_permutation(
          logical.begin(), logical.end(), alloc.begin(), alloc.end()),
      "buildAllocationDomainFromLoopIds expects the allocation domain to be a "
      "permutation of the logical domain");
  const auto& loop = getLoopDomain();
  ...
  setAllocationDomain(new_alloc_domain, true);
}
