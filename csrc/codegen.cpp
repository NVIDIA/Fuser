...
class CudaKernelGenerator : private kir::ConstIrVisitor {
 ...
 private:
  ...
  void handle(const GroupedBlockScalingFactorLayoutOp* layout_op) final {
    ...
    ArgumentBuilder func_args;
    func_args.arg("&")
        .append(genVariableName(output->view()) + "[0]");
    func_args.arg("&")
        .append(genInline(input));
    ...
    // how do I access an entire tensor on global/shared memory?! maybe check out scatter example
    func_args.arg("&")
        .append(genVariableName(layout_op->expertOffsets()) + "[0]");
    func_args.arg("&")
        .append(genVariableName(layout_op->scalingFactorOffsets()) + "[0]");

    func_args.arg(genInline(layout_op->mn()));
    func_args.arg(genInline(layout_op->k()));
    func_args.arg(genInline(layout_op->g()));

    indent() << genCall("block_layout::groupedBlockLayout", template_args, func_args) << ";\n";
