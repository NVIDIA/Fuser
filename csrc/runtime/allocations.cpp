...
KernelArgumentHolder inferContiguousOutputSizes(
  ...
  for (Val* output : fusion->outputs()) {
    ...
    const auto& [sizes, strides] = inferContiguousShapeAndStrideOfOutput(output_tv, expr_eval);
...
std::pair<std::vector<int64_t>, std::vector<int64_t>> inferContiguousShapeAndStride(
    const TensorView* tv,
    const std::vector<Val*>& symbolic_sizes,
    const std::vector<bool>& expand_flags,
    const ExpressionEvaluator& expr_eval) {
  FUSER_PERF_SCOPE("fusion_executor::allocations::inferContiguousShapeAndStride");
...
std::pair<std::vector<int64_t>, std::vector<int64_t>> inferContiguousAllocationShapeAndStride(
    TensorView* tv,
    const ExpressionEvaluator& expr_eval) {
  ...
  return inferContiguousShapeAndStride(tv, symbolic_sizes, expand_flags, expr_eval);
}
...
std::pair<std::vector<int64_t>, std::vector<int64_t>> inferContiguousShapeAndStrideOfOutput(
    TensorView* tv,
    const ExpressionEvaluator& expr_eval) {
  FUSER_PERF_SCOPE("fusion_executor::allocations::inferContiguousShapeAndStrideOfOutput");
...
TensorShapeInfo inferTensorShapes(
  ...
  // Non-alias handling:
  auto allocation_size_stride = inferContiguousAllocationShapeAndStride(tv, expr_eval);
  if (!tv->hasAllocation()) {
    return TensorShapeInfo{
        allocation_size_stride.first,
        allocation_size_stride.second,
