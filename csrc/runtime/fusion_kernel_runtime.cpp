...
std::optional<std::unique_ptr<HeuristicParamsList>> FusionKernelRuntime::
  ...
  for (auto [run_order_id, group_to_run] :
    ...
    std::cout << "group_runtime_outputs: " << group_runtime_outputs.size() << std::endl;
    for (auto out : group_runtime_outputs) {
      if (out.is<at::Tensor>()) {
        const auto& tensor = out.as<at::Tensor>();
        if (tensor.defined()) {
          std::cout << "group_runtime_outputs: " << tensor.device() << std::endl;
        } else {
          std::cout << "group_runtime_outputs: tensor not defined" << std::endl;
        }
      } else {
        std::cout << "group_runtime_outputs: not a tensor" << std::endl;
      }
    }
