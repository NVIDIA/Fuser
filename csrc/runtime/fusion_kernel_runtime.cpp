...
std::vector<KernelArgumentHolder> FusionKernelRuntime::prepareInputs(
  ...
  for (int64_t run_order_id = 0; run_order_id < num_groups; ++run_order_id) {
    ...
    group_runtime_outputs = inferOutputSizes(
        fusion_to_run.get(), group_runtime_inputs, nullptr /* precomputed */);
    bool need_update_contiguity =
    ...
    args_manager.updateWithSegmentOutputs(
        group_to_run->outputs(), group_runtime_outputs, run_order_id, need_update_contiguity);
  }
...
std::optional<std::unique_ptr<HeuristicParamsList>> FusionKernelRuntime::
  ...
  for (auto [run_order_id, group_to_run] :
    ...
    // generated by NVFuser fusions, we assume the outputs are contiguous, and
    ...
    KernelArgumentHolder group_runtime_outputs;
    // For codegen fusion we allocate contiguous outputs; for expr-eval we use EE.
    group_runtime_outputs = inferOutputSizes(
        fusion_to_run, group_runtime_inputs, evaluator_precomputed_values.get());
    bool need_update_contiguity =
        group_to_run->schedulerType() == SchedulerType::ExprEval;
    ...
    NVF_ERROR(
        need_update_contiguity == has_evaluate_outputs,
        "need_update_contiguity mismatches has_evaluate_outputs in getMaybeHeuristicsFor");
