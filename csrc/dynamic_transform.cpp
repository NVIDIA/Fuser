...
void DynamicTransformConcretizer::mutate(TensorDomain* td) {
  ...
  const auto& maybe_alloc = td->hasAllocation() ? alloc_dom
      : td->hasRFactor()                        ? rfactor_dom
                                                : root_dom;

  for (const auto i : c10::irange(maybe_alloc.size())) {
    auto original_id = maybe_alloc.at(i);
    ...
    auto updated_id = maybe_alloc.at(i);
