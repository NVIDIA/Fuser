...
std::shared_ptr<ReductionParams> innerOuterPersistentHeuristic(
  ...
  struct InnerOuterParams {
    ...
    void verify() {
      ...
      TORCH_INTERNAL_ASSERT(
          tmp_gmem_read_vect != -1, "tmp_gmem_read_vect is not set.");
    }
  };
  ...
  if (inner_dim_numel <= 1024) {
    ...
    // previous step). unroll_factor is set to 2 as we only process vectorized
    // cases.
...
std::shared_ptr<ReductionParams> getPersistentHeuristics(
  ...
  if (combined_inner_outer_reduction) {
    ...
    if (!project_persistent_buffers) {
      ...
      // results on A100 for case of 8x1024x14336
      //   ---- projected:
      // ptxas . 176 bytes stack frame, 176 bytes spill stores, 348 bytes spill
      // loads ptxas info : Used 255 registers, 16 bytes smem, 640 bytes
      // cmem[0], 8 bytes cmem[2] shape= 8 x 1024 x 14336 x , time_us mean(var)=
      // 1471.49 (8.25575)
      // ---- unprojected:
      // ptxas . 0 bytes stack frame, 0 bytes spill stores, 0 bytes spill loads
      // ptxas info : Used 255 registers, 16 bytes smem, 640 bytes cmem[0], 8
      // bytes cmem[2] shape= 8 x 1024 x 14336 x , time_us mean(var)= 1504.97
      // (2.89813)
...
void schedulePersistentKernelInnerOuter(
  ...
  // special vectorization of temp gmem
  ...
  // if the read of temp gmem is vectorized, each thread needs to process these
  // vectorized loaded items, then we can also vectorize the outputs of outer
  // reductions. Since the output of inner reductions are already vectorized,
  // so we don't need to bother filtering them out in this for loop.
  if (rparams.vectorization_factor_tmp_gmem_read > 1) {
