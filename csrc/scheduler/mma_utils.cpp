...
RolesMapOpt getTensorsRoles(Fusion* fusion) {
  ...
  const auto findRolesByDomains = [](const DependenciesMap& deps_map,
    ...
    for (const auto& entry : deps_map) {
      ...
      bool has_m =
          (end != std::find(begin, end, MatmulDomain::M)) ? true : false;
      bool has_n =
          (end != std::find(begin, end, MatmulDomain::N)) ? true : false;
      bool has_k =
          (end != std::find(begin, end, MatmulDomain::K)) ? true : false;
      ...
      if (!processing_output && has_m && !has_n && !has_k) {
        roles_map[MatmulRole::BIAS].push_back(entry.first);
        continue;
      }
