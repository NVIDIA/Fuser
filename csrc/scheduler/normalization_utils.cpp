...
// TODO: Should be a compile time check for registry.cpp
// NVF_ERROR(
//     std::any_of(
//         fusion->inputs().begin(),
//         fusion->inputs().end(),
...
int64_t getMaxRegOrSharedMemorySizeForPersistentBuffer(
  ...
  for (auto tv : persistent_buffers) {
    max_buffer_dtype_size = std::max(
        ...
        dataTypeSize(tv->getDataType().value(), index_type));
  }
...
PersistentKernelProperties getPersistentKernelProperties(
  ...
  project_persistent_buffers = isProjectBufferToInputs(
      fusion,
      runtime_info.getIndexType(),
      ...
      heuristic,
      can_use_smem_persistent);
  max_persistent_buffer_size = project_persistent_buffers
      ? persistent_buffer_size_info.projected_persistent_buffer_size
      : persistent_buffer_size_info.persistent_buffer_size;
