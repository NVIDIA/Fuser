...
void schedulePersistentKernel(
  ...
  if (rparams->use_tma_load) {
    ...
    // add dummy output to bypass tma tvs in transform propagation
      ...
      for (auto tv : tma_tvs) {
        ...
        for (auto tv : consumers) {
          if (DependencyCheck::isDependencyOf(tv, reduction_tv)) {
            std::cout << "Found pre-reduction consumer: " << tv->toString()
                      << std::endl;
            pre_redu_tv = tv;
            continue;
          }
          if (DependencyCheck::isDependencyOf(reduction_tv, tv)) {
            std::cout << "Found post_redu_tv consumer: " << tv->toString()
                      << std::endl;
            post_redu_tv = tv;
            continue;
          }
