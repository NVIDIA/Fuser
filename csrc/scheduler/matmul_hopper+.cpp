  ...
  for (TensorView* tv : tvs) {
    ...
    if (params_->tiling_strategy ==
      ...
      if (isCooperative()) {
        ...
        int64_t outer_split = num_sms * num_compute_warp_groups;

        // outer
        tv->split(num_device_dims_, outer_split);
        // outer / (num_sms * num_wgs), (num_sms * num_wgs)

        tv->split(num_device_dims_ + 1, num_sms);
        // outer / (num_sms * num_wgs), num_wgs, num_sms

        if (!ir_utils::isCpAsyncBulkLoad(tv->definition())) {
          tv->axis(num_device_dims_ + 1)->parallelize(ParallelType::TIDy);
        }
        // outer / (num_sms * num_wgs), num_wgs (TIDy), num_sms
        ...
        // outer / (num_sms * num_wgs), num_sms, num_wgs (TIDy)
      }
    } else {
