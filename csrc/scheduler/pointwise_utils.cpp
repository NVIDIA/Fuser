...
namespace {
// Count the number of non-broadcast dims
int64_t numDeviceDims(nvfuser::TensorView* tv) {
  int64_t count = 0;
...
void reorderParallelizedToFront(nvfuser::TensorView* tv) {
  std::vector<int64_t> reorder_map;
  std::vector<int64_t> non_device_dims;

  for (int64_t i = 0; i < tv->nDims(); i++) {
    if (tv->axis(i)->isDeviceDim()) {
      reorder_map.push_back(i);
    } else {
      non_device_dims.push_back(i);
    }
  }

  reorder_map.insert(
      reorder_map.end(), non_device_dims.begin(), non_device_dims.end());
...
std::optional<FusionRuntimeProperties> getPointwiseInitializationData(
    ...
    SchedulerRuntimeInfo& runtime_info,
    HeuristicDataCache* data_cache) {
  FusionRuntimeProperties init_data;

  init_data.largest_out = getLargestOutputTensor(fusion, data_cache);

  init_data.device_multiprocessor_count = static_cast<int64_t>(
      at::cuda::getCurrentDeviceProperties()->multiProcessorCount);

  init_data.has_reshapes = !ir_utils::getReshapeOps(fusion).empty();

  init_data.ref_loop = getReferenceLoop(
      init_data.largest_out, init_data.has_reshapes, data_cache);

  auto [elem_counts, n_elems] =
      getElementCounts(init_data.ref_loop, runtime_info);
  init_data.elem_counts = std::move(elem_counts);
  init_data.n_elems = n_elems;

  // Check for zero dimensional or zero size tensors
  if (std::ranges::empty(
          init_data.largest_out->getLoopDomain() | TensorDomain::kNoReductions |
  ...
  auto vectorizable_inputs_outputs_entry = HeuristicDataCacheEntry<
      ...
      data_cache, [&init_data]() {
        return std::make_unique<std::vector<TensorView*>>(
            scheduler_utils::getInputsOutputsWithInnerDim(
                init_data.largest_out, true, true));
      });
  ...
  for (auto tv : init_data.vectorizable_inputs_outputs) {
  ...
  init_data.max_dtype_size_bit_for_vectorization = max_dtype_size_bit;
  init_data.min_dtype_size_bit_for_vectorization = min_dtype_size_bit;
  init_data.index_type = index_type;
