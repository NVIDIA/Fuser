...
void scheduleMatmul(Fusion* fusion, const MatmulParams& params) {
  ...
  //   and needs more configurability.
  // ------------------------------------------------------------------
  mma_utils::orderTiledConcreteIdAsRoot(acw_smem);
  // [... M, K]
  acw_smem->merge(-2);
  mma_utils::scheduleContiguousVectorLoad(acw_smem, gemm_tile, 8, false);

  // [... N, K]
  mma_utils::orderTiledConcreteIdAsRoot(bcw_smem);
  bcw_smem->merge(-2);
  mma_utils::scheduleContiguousVectorLoad(bcw_smem, gemm_tile, 8, false);
