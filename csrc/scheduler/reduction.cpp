...
std::shared_ptr<ReductionParams> innerReductionHeuristic(
  ...
  redu_unroll_factor = vectorize_factor > 1 ? (int64_t)vectorize_factor : 1;

  // Grab what we can out of reduction domain, but don't go over a warp size yet
  bdimx = std::min(
      std::max(
          ceilDiv(inner_most_dimension_numel, redu_unroll_factor),
          (int64_t)min_warp_size),
      target_threads_in_block);

  // If we're not just barely covering the dimension, round to a more friendly
  // number
  if (bdimx * redu_unroll_factor != inner_most_dimension_numel) {
  ...
  // bdimx
  if (bdimx * bdimy * bdimz < min_warp_size && redu_unroll_factor > 1) {
    bdimx = std::min(
  ...
  int64_t remainder_in_reduction = ceilDiv(
      total_reduction_numel,
      bdimx * redu_unroll_factor * bdimz * outer_reduction_unroll_factor *
          target_iterations);

  int64_t remainder_in_inner_dim = ceilDiv(
      inner_most_dimension_numel,
      bdimx * redu_unroll_factor * target_iterations);

  // If we haven't gotten to the max_unroll case, try to take it out of the
  // iteration domain
  if (redu_unroll_factor * outer_reduction_unroll_factor < max_unroll) {
    // Don't go over a combined inner/outer unroll of max_unroll
    auto unroll_available =
        ceilDiv(max_unroll, redu_unroll_factor * outer_reduction_unroll_factor);
  ...
  pad_bdimx =
      pad_bdimx && bdimx * redu_unroll_factor != inner_most_dimension_numel;
  ...
  rparams->unroll_factor_inner_reduction = redu_unroll_factor;
...
std::shared_ptr<ReductionParams> outerReductionHeuristicNEW(
    const int64_t total_reduction_numel,
    const int64_t total_iteration_numel,
    const int64_t n_tensor_inputs,
    const int64_t max_input_dtype_size,
  ...
  // Experiment shows many cases have higher performance with bdimx = 8
  ...
  // unrolled and vectorized data. assume 50% occupancy, with 512 threads per
  // block, each thread can use 128 registers. 64 registers for
  // unroll/vectorization, 64 for other usage. For example, if inputs are fp16,
  // max_unroll_vect = 128, the heuristic will split 128 between unroll and
  // vectorization, e.g. unroll = 32, vectorization = 4.
  auto const max_buffer_register = 128L;
  auto const max_unroll_vect = ceilDiv(
      // Available unrolling based on size of data type
  ...
  // At least 4 serial reductions per thread ontop of reduction unroll
  // constexpr int64_t min_serial_redu = 4l;
  // with 512 threads per block and min_bdimx = 8 --> max_bdimy = 64
  // reduction_unroll = 64, min_serial_redu = 4
  // max block reduction size = 64 * 64 * 4 = 16K
  ...
  // vectorization
  // Threads are currently at a warp (8 or 32)
  ...
  while (
      ...
      (
          ...
          || target_unroll < int64_t(max_unroll_vect))) {
  ...
  if (target_unroll < max_unroll_vect && available_parallelism() > 1) {
    target_unroll = std::min(available_parallelism(), max_unroll_vect);
  }
  ...
  // at 16K x 18K, unroll = 64, vect = 2, bdimx = 32, bdimy = 16
  ...
  if (char* user_unroll_env = std::getenv("UNROLL")) {
    redu_unroll_factor = atoi(user_unroll_env);
  ...
  if (rDimAvail() > 16 &&
    ...
    int64_t bytes_stride_remainder = max_input_dtype_size * bdimx * bdimy *
        iter_unroll_factor * redu_unroll_factor;
    ...
    int64_t grdim_for_stride = ceilDiv(
        total_reduction_numel, max_remainder_size * bdimy * redu_unroll_factor);
  ...
  std::cout << "final_paras bdimx: " << bdimx << ", bdimy: " << bdimy
            ...
            << ", inner_reduction_unroll_factor: " << redu_unroll_factor
            << std::endl;
...
std::shared_ptr<ReductionParams> reductionHeuristic(
  ...
  if (fastest_dim_reduction) {
    ...
    if (std::getenv("USE_NEW")) {
      return outerReductionHeuristicNEW(
          total_reduction_numel,
          total_iteration_numel,
          (int64_t)n_tensor_inputs,
          (int64_t)max_input_dtype_size,
...
void scheduleReduction(Fusion* fusion, const ReductionParams& rparams) {
  ...
  // allow iter domain grouped reduction for non-welford block/grid outer
  // reduction. the var name is confusing, should rename
  // [cross_grid/block_inner_reduction] to [cross_grid/block_reduction]
  const bool has_welford = ir_utils::hasOpsOfType<WelfordOp>(fusion);
  const bool use_grouped_reduction = has_welford
      ? (!rparams.fastest_dim && rparams.cross_grid_inner_reduction &&
