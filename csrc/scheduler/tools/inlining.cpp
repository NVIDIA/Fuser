...
size_t MaxPosCalculator::getMaxPosAll(
  ...
  if (check_siblings) {
    for (auto sibling_tv : ir_utils::siblingTvsOf(tv)) {
      ...
      std::cout << "max_pos: " << max_pos
                << " sibling_tv: " << sibling_tv->toString() << std::endl;
    }
  }
  return max_pos;
}
