...
bool InnerPersistentKernelScheduler::canScheduleRunTime(
  ...
  if (required_sm_per_norm > scheduler_utils::safeDiv(sm_count_, 2)) {
    scheduler_debug_utils::canScheduleRejectReason(
  ...
  if ( // Large reduction dim
      ...
      properties.total_iteration_numel <
          (properties.fastest_dim_reduction
               ? scheduler_utils::safeDiv(sm_count_, 8)
               // Make sure we at least use a quarter of the device * a
               // half warp
               : (warp_size / 8) * sm_count_)) {
    scheduler_debug_utils::canScheduleRejectReason(
...
class HeuristicGeneratorBase {
 public:
  ...
  HeuristicGeneratorBase(PersistentKernelProperties pk_prop)
    ...
    max_threads_per_sm_ = (int64_t)dev_prop->maxThreadsPerMultiProcessor;
    sm_count_ = (int64_t)dev_prop->multiProcessorCount;

    // Test min_threads_per_block_ using 3 values:
    ...
    // SMs.
    max_multi_reduction_factor_ = std::min(
 ...
 protected:
  ...
  // don't try to maximize it further. As long as it's as large as a given
  // target occupancy, we consider it's good enough. The idea is that as long as
  // we have a good enough occupancy, we should be able to saturate the memory
  // bandwidth. Within these constraints, we try to maximize the number of
  // registers each thread can use. Para [target_warps_per_sm_]: required
  // occupancy to saturate memory bandwidth. Para [register_overhead_]:
  // registers except those for persistent buffers.
  std::pair<int64_t, int64_t> getMaxRegisterCountPerThreadAndOccupancy(
      const int64_t buffer_size_per_thread,
      const int64_t threads_per_block) {
    // convert [target_warps_per_sm_] to [target_blocks_per_sm]
    // ensure higher than target by round up
    int64_t target_blocks_per_sm =
        ceilDiv(target_warps_per_sm_ * threads_per_warp_, threads_per_block);
    // ensure lower than hardware limit by round down
    if (threads_per_block * target_blocks_per_sm > max_threads_per_sm_) {
      target_blocks_per_sm = max_threads_per_sm_ / threads_per_block;
    }
    ...
    if (register_per_thread_target >= register_per_thread_min) {
      return {register_per_thread_target, threads_per_sm / threads_per_warp_};
    }

    //(2) can't achieve target occupancy. Estimate occupancy from minimum
    // register
    ...
    int64_t blocks_per_sm_max = scheduler_utils::safeDiv(
        ...
        threads_per_block);
    threads_per_sm =
        std::min(blocks_per_sm_max * threads_per_block, max_threads_per_sm_);

    return {
        getRegPerThreadGivenThreadsPerSM(threads_per_sm),
        blocks_per_sm_max * threads_per_block / threads_per_warp_};
  }

  // Returns the maximum persistent batch size.
  // For example: assuming we have 64K registers per SM and 28 warps (864
  // threads) per SM. Each thread can use up to 72 registers. Then minus the
  // register overhead 16, there are 56 registers or 224 bytes to store the
  // persistent buffer. (1) If each reduction element has 1 fp32 buffer and
  // vectorized by 8,
  ...
  int64_t getMaxPersistentBatch(
    ...
    int64_t register_for_buffer = total_register - register_overhead_;
...
class HeuristicGeneratorReg2D : public HeuristicGeneratorBase {
 ...
 protected:
  ...
  //      batch leads to the largest occupancy.
 ...
 private:
  ...
  HeuristicParams getHeuristicParamsGivenPerisisentBatchSize(
    ...
    NVF_ERROR(
        params.bdimx <= max_threads_per_block_,
        "persistent batch size too small! bdimx should be less than ",
        max_threads_per_block_,
        ", but got ",
        params.bdimx);
    params.bdimy = std::min(
        scheduler_utils::safeDiv(min_threads_per_block_, params.bdimx),
        max_multi_reduction_factor_);
    params.padded_bdimx = params.bdimx % threads_per_warp_ == 0
        ? params.bdimx
        : params.bdimx + (threads_per_warp_ - params.bdimx % threads_per_warp_);
    params.is_pad_bdimx = params.bdimx > 16 &&
        params.padded_bdimx * params.bdimy <= max_threads_per_block_;
    ...
    auto reg_occ = getMaxRegisterCountPerThreadAndOccupancy(
        persistent_buffer_size, threads_per_block);
    ...
    // Reduction elements after vectorization are split into multiple
    // persistent batches. when [reduction_count_after_vectorize] is not
    // divisible by [persistent_val], the last batch is not be fully utilized,
    // the wasted threads in the last batch is quantified as
    // [n_persistent_tails].
  ...
  bool compareTwoHeuristics(
    ...
    if (prioritize_occupancy) {
      ...
      score = compare(
          ha.occupancy >= target_warps_per_sm_,
          hb.occupancy >= target_warps_per_sm_);
      if (score != 0) {
        return score > 0;
      }
      ...
      // optimize generated SASS code and avoids register spills. But don't
      // want to use a very large block size which increaes inter-thread
      // communication. This condition influence dropout_layer_norm at 7K and
      // softmax_dropout at 6K and 16K.
      constexpr int64_t opt_max_threads_per_block = 512l;
      score = compare(
          ha.register_overhead > register_overhead_ &&
              ha.padded_bdimx <= opt_max_threads_per_block,
          hb.register_overhead > register_overhead_ &&
      ...
      // Tiebreaker, use large persistent batch size so more registers are
      // used for the persistent buffer.
 ...
 protected:
  std::shared_ptr<ReductionParams> generateHeuristic() override {
    ...
    const int64_t outer_reduction_numel =
        prop.total_reduction_numel / prop.inner_most_dimension_numel;

    const auto dev_prop = at::cuda::getCurrentDeviceProperties();

    auto const max_unroll = ceilDiv(
        // Available unrolling based on size of data type
        16l / prop.max_dtype_size,
        // Reduce unrolling if we have many inputs, start reduction at 4 inputs
        scheduler_utils::lastPow2(std::max(prop.n_tensor_inputs >> 2, 1l)));

    // Conservative value, could be set to larger based on arch if necessary.
    constexpr int64_t l1_cache = 32l * 1024l;
    // Could change per generation, but for l1 we want to consider active
    // threads, not resident
    ...
    const bool fits_in_l2 =
        n_elems * prop.max_dtype_size * prop.n_tensor_inputs <
    ...
    const int64_t warp_size_based_on_l2 =
        fits_in_l2 ? 32l / prop.max_dtype_size : 16l;
    ...
    const int64_t warp_size_based_on_l1 = std::min(
        ceilDiv(
            ...
            scheduler_utils::safeDiv(
                l1_cache,
                prop.n_tensor_inputs * prop.max_dtype_size * active_threads)),
        16l);
    ...
    int64_t min_target_iterations =
        scheduler_utils::safeDiv(32, prop.max_dtype_size);

    // Start trying to break parallelization up across threads,
    // unrolling/iterations, and blocks.

    // max_threads_per_block_ is the cap on a thread block, the minimum is based
    // on warp_size
    int64_t max_threads_per_block_ = std::max(
        warp_size, ceilDiv(prop.total_reduction_numel, min_target_iterations));
    ...
    // target iterations
    if (target_blocks > sm_count_) {
      auto available_unroll =
          scheduler_utils::safeDiv(n_elems, warp_size * sm_count_);

      // Spread across unrolling and iterations, want a balance of the two so
      // flip back and forth to alternate adding to them.
      bool flip = true;
      ...
      while (available_unroll > 1 &&
        ...
        available_unroll = scheduler_utils::safeDiv(
            n_elems, warp_size * sm_count_ * target_unroll * target_iterations);
        flip = !flip;
      }
    ...
    target_blocks = std::min(target_blocks, sm_count_ * 4);
    ...
    if (target_blocks * target_unroll * target_iterations < n_elems) {
      if (outer_reduction_numel == 1) {
        ...
        // reductions
        max_threads_per_block_ = std::min(
        ...
        max_threads_per_block_ = std::min(
            ceilDiv(n_elems, target_blocks * target_unroll),
            ceilDiv(max_threads_per_sm_, (int64_t)4));
      }
    }

    // Round up to nearest warp.
    if (max_threads_per_block_ % warp_size != 0) {
      max_threads_per_block_ += warp_size - max_threads_per_block_ % warp_size;
      max_threads_per_block_ = std::min(
          max_threads_per_block_, (int64_t)dev_prop->maxThreadsPerBlock);
    }
    // Compute maximum number of reductions we could do in the same kernel based
    // on persistent buffer size. Bounded by the wave count for utilization of
    // SMs.
    const int64_t max_multi_reduction_factor_ = std::min(
        scheduler_utils::safeDiv(
            scheduler_utils::register_file_size,
            prop.max_persistent_buffer_size),
        ceilDiv(prop.total_iteration_numel, sm_count_));
    ...
    inner_reduction_unroll_factor =
        prop.vectorize_factor > 1 ? (int64_t)prop.vectorize_factor : 1;

    // Grab what we can out of reduction domain, but don't go over a warp size
    // yet
    bdimx = std::min(
        std::max(
            ceilDiv(
                prop.inner_most_dimension_numel, inner_reduction_unroll_factor),
            (int64_t)warp_size),
        max_threads_per_block_);
    ...
    bdimy = std::min(
        scheduler_utils::safeDiv(warp_size, bdimx),
        max_multi_reduction_factor_);
    // If 3D fill the rest of the threads into bdimz
    bdimz = std::min(
        std::min(
            scheduler_utils::safeDiv(max_threads_per_block_, bdimx * bdimy),
    ...
    if (prop.vectorize_factor > 1 && inner_reduction_unroll_factor > 1) {
      vectorize = true;
    ...
    // since V100.
    if (bdimx * bdimy * bdimz < min_threads_per_block_) {
      bdimy = std::min(
          scheduler_utils::safeDiv(min_threads_per_block_, bdimx * bdimz),
          max_multi_reduction_factor_);
    }
    ...
    int64_t batches_per_block_inner_reduction = ceilDiv(
        prop.inner_most_dimension_numel, bdimx * inner_reduction_unroll_factor);
    ...
    godim = ceilDiv(prop.total_iteration_numel, bdimy);
    ...
    while (outer_reduction_unroll_factor < max_unroll &&
      ...
      batches_per_block_outer_reduction =
          scheduler_utils::roundUpPow2Or8(ceilDiv(
              outer_reduction_numel, bdimz * outer_reduction_unroll_factor));
    }

    // Adjust bdimx based on batches_per_block and unroll factor set as they
    // could have moved a bit since they're the free variables, not the buffers
    bdimx = ceilDiv(
        prop.inner_most_dimension_numel,
    ...
    while (
        ...
        bdimx * bdimy * bdimz * 2 <= max_threads_per_block_ &&
        // And batches_per_block_inner_reduction can be divided by two
        (batches_per_block_inner_reduction >
             batches_per_block_inner_reduction_max ||
         batches_per_block_outer_reduction >= 2)) {
      // Try to decrease per thread register allocation persistence size on
      // inner reduction by double bdimx.
    ...
    if (batches_per_block_outer_reduction * batches_per_block_inner_reduction *
        ...
        bdimx * bdimy * bdimz * 2l <= max_threads_per_block_ &&
        batches_per_block_inner_reduction >
            batches_per_block_inner_reduction_max) {
    ...
    if (batches_per_block_outer_reduction * batches_per_block_inner_reduction *
                ...
                4l >
            scheduler_utils::max_registers_per_thread * 3l &&
        bdimx * bdimy * bdimz * 2l <= max_threads_per_sm_ &&
        batches_per_block_outer_reduction >= 2l) {
      batches_per_block_outer_reduction /= 2l;
    }

    auto threads_per_warp_ = (int64_t)at::cuda::warp_size();
    auto padded_bdimx = bdimx % threads_per_warp_ == 0
        ? bdimx
        : bdimx + (threads_per_warp_ - bdimx % threads_per_warp_);
    ...
    // we've seen unexpectedly high register counts with vectorization factor
    // less than 4, which would make the below estimate inaccurate.
    // TODO: support the non vectorized case. consider shmem.
    // only need to balance register and occupancy ratio if there are enough
    // blocks and buffers
    if (vectorize && blocksPerKernel > sm_count_ &&
        batches_per_block_inner_reduction > 1) {
      // Estimate register per thread based on buffer size, since inner
      // reduction dim is fully parallelized, the buffer size of each element
      // equals the total buffer size divide by inner_most_dimension_numel. Each
      // thread will hold batches_per_block_inner_reduction *
      // inner_reduction_unroll_factor elements.
      const int64_t persistent_buffer_size = prop.max_persistent_buffer_size /
          prop.inner_most_dimension_numel * batches_per_block_inner_reduction *
          inner_reduction_unroll_factor;
      const int64_t threads_per_block =
...
std::shared_ptr<ReductionParams> getInnerPersistentHeuristics(
  ...
  if (isDebugDumpEnabled(DebugDumpOption::SchedulerDebug)) {
    debug() << "\n===== Reduction Stats ========\n"
            << "total_reduction_numel: " << prop.total_reduction_numel << "\n"
            << "total_iteration_numel: " << prop.total_iteration_numel << "\n"
            << "inner_most_dimension_numel: " << prop.inner_most_dimension_numel
            << "\n"
