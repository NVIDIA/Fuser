...
std::shared_ptr<ReductionParams> innerPersistentHeuristic2D(
  ...
  const int64_t n_elems = inner_most_dimension_numel * total_iteration_numel;
  ...
  int64_t max_threads_in_block = std::max(
      warp_size, ceilDiv(inner_most_dimension_numel, min_target_iterations));
  ...
  while (bdimx * bdimy * bdimz * 2 <= max_threads_in_block &&
  ...
  if (batches_per_block_inner_reduction * inner_reduction_unroll_factor * 4l >
