...
void HostIrContainer::insertDeallocations() {
  std::unordered_map<TensorView*, std::vector<nvfuser::Expr*>::size_type> lastUse;
  for (std::vector<nvfuser::Expr*>::size_type i = 0; i < top_level_exprs_.size(); i++) {
    Expr *expr = top_level_exprs_[i];
  ...
  for (auto iter = rmap.rbegin(); iter != rmap.rend(); ++iter) {
    ...
    for (TensorView* tv : iter->second) {
      ...
      top_level_exprs_.insert(top_level_exprs_.begin() + iter->first + 1, deallocate);
    }
  }
}
