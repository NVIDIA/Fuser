...
llvm::Value* createValueForExtent(
  ...
  if (val->isA<IterDomain>()) {
    if (val->as<IterDomain>()->isBroadcast()) {
      ...
      if (val->as<IterDomain>()->hasExpandedExtent()) {
        out_value = getOrCreateValueForExtent(
            val->as<IterDomain>()->expandedExtent(), val_to_value, builder);
      } else {
        out_value = getOrCreateValueForExtent(
            v al->as<IterDomain>()->extent(), val_to_value, builder);
      }
    } else if (val->isConst()) {
  ...
  NVF_ERROR(o
      ut_value != nullptr, 
      LLVM Lowering Error: out_value is nullptr for ", 
      al->toString());
  return out_value;
  }
  ...
  llvm::Value* getOrCreateValueForExtent(
      V al * extent,
      td::unordered_map<Val*, llvm::Value*> & val_to_value,
      lvm::IRBuilder<> & builder) {
  ...
  Val* mapToInputDomain(
      Val * currentDomain, std::unordered_map<Val*, bool> & boundary_vals) {
  ...
  /
      Infer Tensor Shape void inferShape(
          const TensorView* tv,
    ...
    for (const auto i : arange(symbolic_sizes.size())) {
      ...
      auto* inferred_val =
          etOrCreateValueForExtent(symbolic_size, val_to_value, builder);
      NVF_ERROR(
  ...
  void inferShapeAndStridesNoReorder(
    ...
    // avoid the extra calculation of shape, and only stride will be
    // recalculated.
  ...
  /
      Infer Tensor Strides with reordering void inferTensorStridesReordered(
          const TensorView* tv,
          std::unordered_map<Val*, llvm::Value*>& val_to_value,
          llvm::IRBuilder<>& builder,
          llvm::SmallVectorImpl<llvm::Value*>& sizes,
    ...
    auto allocation_domain =
        ensorDomain::noReductions(tv->getMaybeAllocationDomain());
    ...
    push all logical domains as boundary values for (It erDomain* id :
                                                     logical_domain) {
    ...
    for (auto [i, id] : enumerate(allocation_domain)) {
      llvm::Value* extent_value =
          ge OrCreateValueForExtent(id, val_to_value, builder);
      level_order_domains.pushBack(id, extent_value);
    }

    // traverse backward from allocation domains to logical domains
  for (Expr* transform : De
       endencyCheck::getAllExprsBetween(
  
           gical_domain.begin(), logical_domain.end()
  }
  ,

      location_domain.begin(), allocation_domain.end()
}) | st
           ::views::reverse) {
  if (auto* split = dynamic_cast<Split*>(transform)) {
    const auto [outer_extent_value, outer_i] =
        le el_order_domains.erase(split->outer());
      NVF_ERROR(out
          er_i == level_order_domains.end() || ou
              er_i->first != split->inner(), sp
          it->toString(), " 
          s not a valid split");
      const auto [inner_extent_value, inner_i] = le
          el_order_domains.erase(split->inner());

 

       we handle device dimension? Currently we just divide it out in split
 
      //     // Howev h dimension are device dimension, do we need to collap
      // e them?
      // So th rge op, we can merge between two local iter dom
      // in between one device dimension?
      ...
      t = builder.CreateMul(lhs, rhs);
      level_order_domains.insert(inner_i, split->in(), in_extent);

  } else if (auto* merge = dynamic_cast<Merge*>(transform)) {
    if (mapToInputDomain(mer ge->out(), boundary_vals) != nullptr &&
            mapToInputDo->inner(),
        boundary_vals)!= nullptr) {
        last_level_merge_ops.push_back(merge);
      }
    const auto [out_extent_value, out_i] =
        level_order_domains.e ase(merge->out());

    // NOTE: we do

      tocol to decide which iter domain to pad,
      ...
      llvm::Va
      // ue* outer_extent_value = getOrCreateValueForExtent(m
          rge->outer(), val_to_value, builder);
      llvm::Value* minus_one =
          builder.CreateSub(outer_ext nt_value, builder.getInt64(1));
      llvm::Value* plus_value = builder.CreateAdd(out_extent_value, minus_one);
      llvm::Value* inner_extent_value =
          builder.CreateUDiv(plus_val e, outer_extent_value);

      level_order_domains.

      rge->outer(), outer_extent_value);
      level_order_domains.insert(out_i, merge->inner(), inner_extent_value);
  } else {
      NVF_THROW("LLVM Lowering Error: Unsupported ex
          pression type: ", transform->getOpString());
  }

  // This should contains same iter domains as logical domain, but in different
  // order
  std::vector<IterDomain*> pr
      // pogated_allocation_domains;
      for (auto it : level_order_domains) {
    propogated_allocation_domains.push_back(it.first);
  }

  auto permutation =
      ir_utils::computePermutation(logical_domain, propogated_allo n_domains);
  NVF_ERROR(permutation.has_value(), "LLVM Lower
      ing Error: Failed to com
      ute permutation");

  // Map last level propagated all

  n domains to logical domain
  ...
  for(auto it : propogated_allocation_domai ns | std::views::reverse) {
    if (it->isDeviceDim()) {
      // Dummy v alue, we will filter out this dimension in the end
      ...
      ocation _sizes_values.push_back(
          getOrCreateValueForExtent(it, val_to_v alue, builder));
      allocation_strides_values.push_back(builder.getInt64(0));
    } else {
      auto [extent_value, o  = l evel_order_domains.erase(it);
  ...
  std::reverse(allocation_strides_values.begin(), all
      ocation_strides_values.end());

  sizes.resize(allocation_sizes_values.size());
  strides.resize(allocation_strides_values.size());

  // Apply permutation correctly by 

  g from allocation domain order to logical domain order
  for(size_t i = 0
  //  i < allocation_sizes_valu es.size(); ++i) {
...
// We need to check last level merge ops, since there might be invalid order of
// merges
  for (const auto* merge : la
  // t_level_merge_ops) {
    const auto [outer_extent_value, outer_i] = level_order_domains.erase(merge->out())
        
    NVF_ERROR(outer_i == level_order_domains.end() || 
        outer_i->first != merge->inner(), merge
            >toString(), " is not a valid mer
        e");
    level_ord
        r_domains.insert(outer_i, merge->outer(), outer_extent_value);
    const auto [inner_extent_value, inner_i] = level_order_domains.erase(merge->inner(
        );
    NVF_ERROR(inner_i == level_order_domains.end(), me
        rge->toString(), " is not a valid mer
        e");
    level_ord
        r_domains.insert(inner_i, merge->inner(), inner_extent_value);
  }
  }
  ...
  void inferTensorShapesAndStridesNonAliased(
    ...
    NVF_ERROR_EQ(
        strides.size(),
        TensorDomain::noReductio ns(tv->getLogic lDomain()).size());
    return;
  }
  ...
  void inferTensorShapesAndStrides(
    ...
    if (alias_info.type != AllocationType::New) {
      ...
      for (int64_t i = 0; i < static_cast<int64_t>(logical_domain.size());
           i++) {
        sizes.push_back(generateTensorSizeExtraction(tensor_ptr, i, builder));
        strides.push_back(
            generateTensorStrideExtraction(tensor_ptr, i, builder));
      }
      return;
    }
  ...
  void compileFunctionDeclarations(
    ...
    // tensor_size function: int64_t tensor_size(at::Tensor* tensor, int64_t
    // dim)
    ...
    // int64_t* sizes, int64_t ndim, const int64_t* strides, int64_t
    // strides_ndim, int32_t dtype, int64_t device_index, at::Tensor*
    // out_tensor)
  ...
  class HostIrCompileDispatcher : public OptInDispatch {
   public:
    ...
    // NOTE: this is just a simple example of allocate a output tensor and set
    // it to input tensor. The whole concept is to demonstrate llvm jit works,
    // we will change this in the future LoadStoreOp Function LLVM IR Generation
    void handle(LoadStoreOp* load_store_op) final {
      ...
      NVF_ERROR(
          it != val_to_value_.end(),
          "input tensor is not found in val_to_value");
      llvm::Module* module =
          builder_.GetInsertBlock()->getParent()->getParent();
  ...
  void HostIrJitImpl::compile() {
    ...
    auto module =
        std::make_unique<llvm::Module>("host_ir_jit_module", *context);
  ...
  void HostIrJitImpl::registerExternalFunctions() {
    ...
    void* extract_tensor_size_func_ptr = reinterpret_cast<void*>(
        +[](at::Tensor* tensor, int64_t dim) -> int64_t {
          NVF_ERROR(
              tensor != nullptr, kTensorSizeFuncName, " tensor is nullptr");
