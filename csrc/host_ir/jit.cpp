...
using AllocateFunc = std::function<void(
...
template <typename T> T throwIfError(llvm::Expected<T> &&E) {
  if (!E) {
...
void dimCheck(llvm::Value *ndim_arg, llvm::Value *ndim_val,
...
void generateAllocateFunc(const kir::Allocate *allocate, llvm::Module *mod) {
  ...
  at::ScalarType data_type =
      data_type_to_aten(allocate->buffer()->dtype() == DataType::Index
                            ? PrimDataType::Int
  ...
  if (at_empty_strided_cuda_func == nullptr) {
    llvm::FunctionType *at_empty_strided_cuda_func_type =
        llvm::FunctionType::get(builder.getVoidTy(),
                                {int64_ptr_type, int64_type, int64_ptr_type,
                                 int64_type, int32_type, int64_type,
                                 void_ptr_type},
                                false);
    at_empty_strided_cuda_func = llvm::Function::Create(
        at_empty_strided_cuda_func_type, llvm::Function::ExternalLinkage,
        kHostIrJitAtEmptyStridedCudaFuncName, mod);
  }

  // Call at::native::empty_strided_cuda with constants
  builder.CreateCall(at_empty_strided_cuda_func,
                     {sizes_arg, shape_ndim_arg, strides_arg, strides_ndim_arg,
                      dtype_constant, device_index_constant, out_tensor_arg});
  ...
  NVF_ERROR(!llvm::verifyModule(*mod, &error_stream),
...
void compile(
    const hir::HostIrContainer *container, llvm::orc::LLJIT *jit,
  ...
  for (auto *allocate : allocate_exprs) {
    ...
    allocate_funcs[allocate] =
        reinterpret_cast<void (*)(const int64_t *, int64_t, const int64_t *,
                                  int64_t, at::Tensor &)>(func_addr.getValue());
  }
}
...
HostIrJit::HostIrJit(hir::HostIrContainer *container, int num_threads)
  ...
  void *at_empty_strided_cuda_func_ptr = reinterpret_cast<void *>(
      +[](const int64_t *sizes, int64_t ndim, const int64_t *strides,
          int64_t strides_ndim, int32_t dtype, int64_t device_index,
        ...
        out_tensor = at::native::empty_strided_cuda(aten_sizes, aten_strides,
                                                    scalar_type, c10::nullopt,
                                                    device, c10::nullopt);
      });
  ...
  symbolMap[mangler(kHostIrJitAtEmptyStridedCudaFuncName)] =
      llvm::orc::ExecutorSymbolDef(at_empty_strided_cuda_addr,
...
at::Tensor HostIrJit::allocate(const kir::Allocate *allocate,
                               const std::vector<int64_t> &input_sizes,
                               const std::vector<int64_t> &input_strides) {
  FUSER_PERF_SCOPE("HostIrJit::allocate");
  auto allocate_func_iter = pimpl_->allocate_funcs_.find(allocate);
  NVF_ERROR(allocate_func_iter != pimpl_->allocate_funcs_.end(),
            "allocate function not found for ", allocate);
  const auto &func_ptr = allocate_func_iter->second;

  at::Tensor tensor;
  func_ptr(input_sizes.data(), input_sizes.size(), input_strides.data(),
           input_strides.size(), tensor);
