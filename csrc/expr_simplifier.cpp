...
Val* eliminateTrivialComputation(Val* value, const Context& context) {
  ...
  if (auto fop = dynamic_cast<FOp*>(value->definition())) {
    ...
    if (op == BinaryOpType::Add) { // a + (-a) -> 0
      ...
      for (size_t idx : std::views::iota(0LL, fop->inputs().size())) {
      ...
      for (auto [orig, inv, idx] : inv_inputs) {
        for (size_t idx2 : std::views::iota(0LL, fop->inputs().size())) {
...
Val* distributeDivisibleDivMod(Val* value, const Context& context) {
  ...
  for (auto i : std::views::iota(0LL, fop->inputs().size())) {
    ...
    for (auto j : std::views::iota(0LL, fop->inputs().size())) {
      if (j == i) {
        continue;
      }
...
Val* fundamentalDivisionWithRemainderProperty(
  ...
  auto get_a_op_b_mul_c = [](BinaryOpType op, Val* x) {
    ...
    for (auto j : std::views::iota(0LL, fmul->inputs().size())) {
      ...
      if (bop->getBinaryOpType() == op) {
        ...
        auto b = bop->rhs();
        std::vector<Val*> other_terms;
        for (auto k : std::views::iota(0LL, fmul->inputs().size())) {
          if (j == k) {
            continue;
          }
  ...
  for (auto i : std::views::iota(0LL, fadd->inputs().size())) {
  ...
  for (auto& [i, a1, b1, bc] : divmuls) {
    for (auto& [j, a2, b2, c] : modmuls) {
      ...
      if (quotient != nullptr && quotient->isOne()) {
        ...
        for (auto k : std::views::iota(0LL, fadd->inputs().size())) {
          if (k == i || k == j) {
            continue;
          }
...
Val* cancelTermsInPredicate(Val* value, const Context& context) {
  ...
  for (const auto lhs_i : std::views::iota(0LL, lhs_terms.size())) {
    for (const auto rhs_i : std::views::iota(0LL, rhs_terms.size())) {
  ...
  for (const auto i : std::views::iota(0LL, lhs_terms.size())) {
  ...
  for (const auto i : std::views::iota(0LL, rhs_terms.size())) {
