...
std::unordered_map<ValGroup, IterDomain*> LoopPromotionMapBuilder::build() {
  ...
  //
  // NOTE: need to use permissive because EXACT and LOOP graph might contain
  // different ValGroup
  const ValGraph iel_graph = id_model_.buildIntersection(
      idGraph(IdMappingMode::EXACT),
      idGraph(IdMappingMode::LOOP),
      /*propagate_exprs=*/false,
      /*permissive=*/true,
      /*exclude_allocation_domain=*/true);
    ...
    for (auto loop_id : *group) {
      ...
      for (auto use : uses_it->second) {
        if (std::ranges::any_of(use->outputs(), [&](Val* out) -> bool {
              // NOTE: not all consumer of a given ID is guaranteed to be
              // included in LOOP graph, because LOOP graph excludes IDs only on
              // the path to allocation domain.
