...
void IdGraphVisitor::traverse() {
  ...
  ExprGroups all_exprs;
  {
    ...
    if (sub_selection_.empty()) {
      ...
      for (const ValGroup& id_group : all_ids) {
        const ExprGroups* defs = graph().getDefinitions(id_group);
        NVF_ERROR(defs);
  ...
  auto is_id_ready = [&](const ValGroup& id_group) {
    ...
    return std::all_of(defs->begin(), defs->end(), [&](ExprGroup expr_group) {
    ...
    while (!to_visit_ids.empty()) {
      ...
      if (is_id_ready(current_id_group)) {
        ...
        if (!terminating_outputs.has(current_id_group)) {
          if (const ExprGroups* uses = graph().getUses(current_id_group)) {
