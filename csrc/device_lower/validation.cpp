...
class ExprValidator : public OptOutDispatch {
 ...
 private:
  ...
  // domain, and that, these IDs are contiguous in the logical domain.
  ...
  void handle(BlockQuantizationOp* bqop) final {
    ...
    auto last_split_seen =
        checkGroupIDDerivedFromLastLogicalIDs(grouped_id, quantized_output);
    ...
    if (last_split_seen == nullptr) {
      auto ids_in_logical = IterVisitor::getInputsTo({grouped_id});
