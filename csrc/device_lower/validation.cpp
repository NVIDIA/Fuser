...
void validateScans(Fusion* fusion) {
  for (auto sop : ir_utils::getOpsOfType<ScanOp>(fusion)) {
    ...
    IterDomain* scan_id = out->getLogicalDomain().at((size_t)sop->dim());

    if (scan_id->isParallelized()) {
      // The following checks are only valid for serial scans
      continue;
    }
    ...
    // If not found, ensure this is a parallel scan
    if (scan_pos == -1L) {
      NVF_ERROR(
          std::any_of(
              ...
              [](IterDomain* id) {
                return id->isParallelized() && id->isScan();
              }),
          "Could not find scan dimension ",
          scan_id->toString(),
          " in loop domain. Expect a parallel scan but can't find parallel "
          "scan dimension.");
      continue;
    }
    NVF_ERROR(
