...
class ReadAfterWriteSyncs : public kir::ExprMutator {
 private:
  ...
  void dispatch(Expr* expr) final {
    ...
    if (auto mma = dynamic_cast<MmaOp*>(expr)) {
      if (mma->isHopper()) {
        ...
        fill_async_mma_pipeline = true;
        // async mma pipeline has not been flushed yet.
        flush_async_mma_pipeline = false;
      }
    ...
    for (auto inp : expr->inputs()) {
      ...
      NVF_ERROR(
          !flush_async_mma_pipeline || !fill_async_mma_pipeline,
      ...
      if (async_type == AsyncOpType::WgMma && !fill_async_mma_pipeline &&
          flush_async_mma_pipeline) {
        continue;
      }
  ...
  bool checkAsyncMmaPipeline() {
    return fill_async_mma_pipeline == false;
  }

 private:
  //! fill_async_mma_pipeline is true when any mma expression is issued. A RAW
  //! sync is required before any consumer operations use the results of mma
  //! expression.
  //!
  //! flush_async_mma_pipeline is true when a RAW sync is issued for async mma
  ...
  //!
  //! fill_async_mma_pipeline and flush_async_mma_pipeline cannot be true
  //! simultaneously.
  //!
  ...
  bool fill_async_mma_pipeline = false;
