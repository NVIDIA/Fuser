 ...
 private:
  ...
  void insertClusterReductionMBarrier(Expr* expr) {
    ...
    if (cluster_reduction_count == 1) {
      ...
      std::cout << "inserting cluster reduction mbarrier: " << pred_mbarrier_init->toString() << std::endl;
      ...
      std::cout << "inserting cluster reduction mbarrier: " << fl->toString() << std::endl;
    ...
    // Insert clusterSync after mbarrier initialization to ensure mbarrier is visible to other CTAs
    auto cluster_sync = IrBuilder::create<kir::ClusterSync>();
    std::cout << "inserting clusterSync: " << cluster_sync->toString() << std::endl;
