...
class AllocationInserter : public kir::ExprMutator {
 private:
  ...
  std::vector<Val*> getNonGlobalAllocExpr(AllocationInformation& info) {
    ...
    bool is_mma = info.buffer->definition()->isA<MmaOp>() &&
        isHopper(info.buffer->definition()->as<MmaOp>()->macro());

    if (!getenv("OLD") && !is_mma) {
      alloc_domains =
          GpuLower::current()->getAllocationInfo(info.buffer).ids;
      alloc_dims.reserve(alloc_domains.size());
      info.allocation_domains->reserve(alloc_domains.size());
      for (const auto& id : alloc_domains) {
        alloc_dims.push_back(id->extent());
        info.allocation_domains->push_back(id);
      }
      return alloc_dims;
