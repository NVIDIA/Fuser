...
struct InplaceAliasInfo {
  ...
  std::unordered_map<DisjointSets<TensorView*>::DisjointSet,
                     TensorView*> real_alloc_map;
};
 ...
 public:
  ...
  void handle(ScatterOp* sop) final {
    ...
    if (in_tv_alias_it != info_.aliased_tvs.end()) {
      ...
      info_.real_alloc_map.emplace(in_tv_alias_it->second,
                                   in_tv);
    }
  ...
  void handle(kir::Allocate* alloc) final {
    NVF_ERROR(info_.alloc_map.emplace(alloc->buffer()->as<TensorView>(), alloc).second);
  }
...
class InplaceAliasMutator : public kir::ExprMutator {
 public:
  InplaceAliasMutator(const InplaceAliasInfo& info): 
                                                     info_(info) {}
 protected:
