...
template <typename TimeType, typename IndexType>
class CenteredIntervalTree {
 public:
  ...
  std::vector<IndexType> getIntervalsOverlappingPoint(TimeType x) const {
    ...
    root->insertIntervalsOverlappingPoint(output, x);
    return output;
  }

  //! Get a vector of indices of intervals that overlap an arbitrary given interval
  std::vector<IndexType> getIntervalsOverlappingPoint(
      TimeType start,
      TimeType stop) const {
    std::vector<IndexType> output;
    root->insertIntervalsOverlappingPoint(output, start, stop);
    return output;
  }
 ...
 private:
  struct CenteredIntervalTreeNode {
    ...
    //! comparison.
    void insertIntervalsOverlappingPoint(
        std::vector<IndexType>& output,
        TimeType x) const {
      if (x == center_point) {
        // if x is the center point, we just insert the entire center subset
        output.reserve(output.size() + local_to_global.size());
        output.insert(
            output.end(), local_to_global.begin(), local_to_global.end());
        return;
      }
      ...
      if (x < center_point) {
        ...
        for (auto local_idx : center_start_sorted) {
          auto global_idx = local_to_global.at(local_idx);
          auto start = intervals.at(global_idx).first;
          if (start > x) {
            break;
          }
        ...
        for (auto local_idx : center_stop_reverse_sorted) {
          auto global_idx = local_to_global.at(local_idx);
          auto stop = intervals.at(global_idx).second;
          if (stop < x) {
            break;
          }
          output.push_back(global_idx);
      ...
      if (child_node) {
        child_node->findIntervalsOverlappingPoint(x, output);
      }
    }
    ...
    //! Save a reference to the intervals passed. Indices should be relative to
    //! this vector.
    const std::vector<std::pair<TimeType, TimeType>>& intervals_;

    CenteredIntervalTreeNode* root = nullptr;
};
