...
void IndexLowering::handle(const kir::ClusterReductionOp* cop) {
  ...
  if (cop->mbarrier()->isA<TensorView>()) {
    mbarrier_addr = lower_utils::u32IndexScalarSmemTv(cop->mbarrier()->as<TensorView>());
  } else if (cop->mbarrier()->isA<kir::TensorIndex>()) {
    mbarrier_addr = lower_utils::u32IndexScalarSmemTv(cop->mbarrier()->as<kir::TensorIndex>());
  } else {
