...
class GpuLower : public NonCopyable {
 ...
 public:
  ...
  const std::unordered_map<TensorView*, IndexingAllocationInfo>& allocationInfo()
      const {
    return allocation_info_;
  }

  std::unordered_map<TensorView*, IndexingAllocationInfo>& allocationInfo() {
    return allocation_info_;
  }

  const IndexingAllocationInfo& getAllocationInfo(TensorView* tv) const;
 ...
 private:
  ...
  std::unordered_map<TensorView*, IndexingAllocationInfo> allocation_info_;
