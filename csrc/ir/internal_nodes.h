...
//! Grouped matrix multiplication operation for horizontal fusions.
//! Similar to batched GEMMs, multiple independent matrix multiplications
//! are performed together. The main benefit is improved performance through
...
class GroupedMMOp : public Expr {
 public:
  ...
  GroupedMMOp(
      ...
      std::vector<Val*> mat1s,
      std::vector<Val*> mat2s);
  ...
  //! Number of expressions grouped horizontally.
  size_t numHorizontallyGroupedExprs() const {
    return outputs().size();
  }

  Val* output(size_t index) const {
    return outputs().at(index);
  }

  Val* mat1(size_t index) const {
    return inputs().at(index * 2);
  }

  Val* mat2(size_t index) const {
    return inputs().at(index * 2 + 1);
  }

  IterDomain* getKIDOfMat1(size_t index) const {
    return TensorDomain::noReductions(mat1(index)->getLogicalDomain()).at(2);
  }

  IterDomain* getKIDOfMat2(size_t index) const {
    return TensorDomain::noReductions(mat2(index)->getLogicalDomain()).at(1);
  }
};
