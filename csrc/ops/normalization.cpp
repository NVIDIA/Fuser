  ...
  if (kTraining || running_mean == nullptr) {
    ...
    if (running_mean != nullptr && running_var != nullptr) {
      ...
      auto cast_to_input_dtype = [fusion](
        ...
        fusion->aliasOutputToInput(cast_output, input_to_cast, true);
      };

      if (running_mean->isFusionInput()) {
        fusion->aliasOutputToInput(new_mean_hat, running_mean, true);
      } else {
        cast_to_input_dtype(running_mean, new_mean_hat);
      }

      if (running_var->isFusionInput()) {
        fusion->aliasOutputToInput(new_var_hat, running_var, true);
      } else {
...
ForwardNormResult instance_norm(
  ...
  if (kUseInputStats || running_mean == nullptr) {
    ...
    if (running_mean != nullptr && running_var != nullptr) {
      ...
      fusion->aliasOutputToInput(new_mean_channels_only, running_mean, true);
      ...
      fusion->aliasOutputToInput(new_var_channels_only, running_var, true);
    }
