...
Val* numFeatures(TensorView* x, const std::vector<int>& dims, size_t ndims) {
  Val* num_features = IrBuilder::create<Scalar>(x->container(), 1.0);
...
TensorView* variance(
  ...
  if (correction > 0) {
    num_features =
        sub(num_features, IrBuilder::create<Scalar>(x->container(), correction));
  }
...
ForwardNormResult batch_norm(
  ...
  if (kTraining || running_mean == nullptr) {
    ...
    if (running_mean != nullptr && running_var != nullptr) {
      ...
      auto rev_momentum =
          sub(IrBuilder::create<Scalar>(x->container(), 1.0), momentum);
...
ForwardNormResult instance_norm(
  ...
  Val* N = IrBuilder::create<Scalar>(x->container(), 1.0);
  for (const auto axis : c10::irange(kNumberOfDims)) {
  ...
  Val* B = IrBuilder::create<Scalar>(x->container(), 1.0);
