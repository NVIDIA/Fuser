    ...
    if (running_mean != nullptr && running_var != nullptr) {
      ...
      auto cast_to_input_dtype = [fusion](
        ...
        fusion->aliasOutputToInput(
            cast_output, input_to_cast, AliasType::SameShapeDifferentData);
      };

      if (running_mean->isFusionInput()) {
        fusion->aliasOutputToInput(
            new_mean_hat, running_mean, AliasType::SameShapeDifferentData);
      } else {
      ...
      if (running_var->isFusionInput()) {
        fusion->aliasOutputToInput(
            new_var_hat, running_var, AliasType::SameShapeDifferentData);
      } else {
  ...
  if (kUseInputStats || running_mean == nullptr) {
    ...
    if (running_mean != nullptr && running_var != nullptr) {
      ...
      fusion->aliasOutputToInput(
          new_mean_channels_only,
          running_mean,
          AliasType::SameShapeDifferentData);
