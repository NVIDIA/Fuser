...
BlockQuantizationResults block_quantize(TensorView* input) {
  ...
  // auto reshaped_quantized_tensor =
  //     reshape(quantized_tensor, [](auto& x) { x.merge(-2); });
  // auto reshaped_block_scales =
  //     reshape(block_scales, [](auto& x) { x.merge(-2); });

  // return BlockQuantizationResults(
  //     reshaped_block_scales, reshaped_quantized_tensor);
}
