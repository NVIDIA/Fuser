...
std::vector<PolymorphicValue> GetMetaData::evaluate(
  ...
  metadata->logical_size = input.sizes().vec();
  metadata->logical_stride = input.strides().vec();
  if (tv->hasAllocation()) {
    auto allocation_data =
        inferAndValidateAllocationSizesAndStrides(input, tv, ee);
    metadata->alloc_size = std::move(allocation_data.first);
    metadata->alloc_stride = std::move(allocation_data.second);
  } else {
    metadata->alloc_size = input.sizes().vec();
    metadata->alloc_stride = input.strides().vec();
  }
