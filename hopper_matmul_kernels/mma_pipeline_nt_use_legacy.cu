...
__global__ void __launch_bounds__(/*MAX_THREADS_PER_BLOCK=*/384) nvfuser_none_f0_c0_r0_g0(Tensor<__half, 3, 3> T0, Tensor<__half, 3, 3> T1, const __grid_constant__ TensorMap var0, const __grid_constant__ TensorMap var1, const __grid_constant__ TensorMap var2, Tensor<__half, 2, 2> T3) {
  ...
  if ((((nvfuser_index_t)threadIdx.y) == 2)) {
    ...
    // Release all stages for Load WarpGroup
    // All stages are prefetched.
    // Call mbarrier::arrive for all stages.
    #pragma unroll
    ...
    // Prefill
    // Launch 0th wgmma to prefill mma pipeline.
    // * Mma waits for 0th stage. 
    // * Compute wgmma for 0th stage. 0th stage cannot be written.
    ...
    // Compute
    // for remaining k loop iterations:
    // * Wait for stage (N % pipeline) to arrive.
    // * Launch wgmma for iteration N
    // * Wait for prev stage wgmma to finish.
  ...
  for(nvfuser_index_t i48 = 0; i48 < 16; ++i48) {
    if ((b20 && (i21 < (-(16 * i48))))) {
      asm volatile(
        ...
        :
        :"r"((uint32_t)((toSmem(T7) + ((((nvfuser_index_t)threadIdx.y) * 32768) + (((i48 / 4) * 8192) + ((i11 * 128) + (((((((nvfuser_index_t)threadIdx.x) % 32) / 16) + ((i48 % 4) * 2)) ^ (i11 % 8)) * 16))))))),
         "r"((*reinterpret_cast<Array<uint32_t, 4, 1>*>(&T6[(8 * i48)]))[0]),
         "r"((*reinterpret_cast<Array<uint32_t, 4, 1>*>(&T6[(8 * i48)]))[1]),
         "r"((*reinterpret_cast<Array<uint32_t, 4, 1>*>(&T6[(8 * i48)]))[2]),
         "r"((*reinterpret_cast<Array<uint32_t, 4, 1>*>(&T6[(8 * i48)]))[3])
      );
    }
  }
  __syncthreads();
