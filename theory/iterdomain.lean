...
def inner_split (factor : Nat) : IterDomainTransformations 1 2 :=
  { extent_map := fun v => Vector.cons (v[0] ⌈/⌉ factor) (Vector.cons factor Vector.nil),
    index_map := fun v => Vector.cons (Nat.div v[0] factor) (Vector.cons (Nat.mod v[0] factor) Vector.nil) }
...
theorem split_split {n m : Nat}:
  ...
  let t2 := compose (extend (inner_split n) 1 0) (inner_split (Nat.mul m n))
  equiv t1 t2
:=
  let t1 := compose (extend (inner_split m) 0 1) (inner_split n)
  let t2 := compose (extend (inner_split n) 1 0) (inner_split (Nat.mul m n))
  have hem : t1.extent_map = t2.extent_map := sorry
  have him : t1.index_map = t2.index_map := sorry
  And.intro hem him
