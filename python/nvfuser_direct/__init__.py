...
def validate(
    ...
    **kwargs,
    ...
    ----------
    fd : FusionDefinition
        The fusion definition to validate
    ...
    **kwargs : dict, optional
    ...
    None
    """
    fusion_outputs = fd.execute(inputs, device=device, **kwargs)
    assert (
        hasattr(fd, "fec") and fd.fec is not None
    ), "FusionExecutorCache is not initialized"

    if reference_outputs is None:
        return fd.fec.validate_with_auto_inferred_outputs(fusion_outputs, inputs)

    assert len(fusion_outputs) == len(
        reference_outputs
    ), f"Expected {len(fusion_outputs)} reference outputs for validation."

    tolerance_values = fd.fec.get_val_tolerances(inputs)
