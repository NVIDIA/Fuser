...
def get_compiler_info() -> Optional[Tuple[str, Tuple[int, int, int]]]:
    ...
    try:
        ...
        if 'clang' in output:
            ...
            if match:
                return ("clang", (int(match.group(1)), int(match.group(2)), int(match.group(3))))
            return None
        
        # Parse as GCC
        match = re.search(r'(\d+)\.(\d+)\.(\d+)', first_line)
        if match:
            return ("gcc", (int(match.group(1)), int(match.group(2)), int(match.group(3))))
...
def validate_compiler() -> Tuple[str, Tuple[int, int, int]]:
    ...
    if not check_format_support(compiler_type):
        print(f"[nvFuser] WARNING: {req.name} {format_version(version)} detected but <format> header not available")
        print(f"[nvFuser] Build may fail. Please verify compiler installation is complete.")
