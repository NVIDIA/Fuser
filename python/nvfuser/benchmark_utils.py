...
class CUDAEventTimer(Timer):
    ...
    def __call__(self):
        """
        Custom CUDA event-based timer used by pytest-benchmark.
        At every timer call, if the timer is running, it records the end event and computes elapsed time.
        If not running, it records the start event and starts timing.

        Returns:
            self.current_time: Global monotonic clock variable
        """
        if self.is_running:
            # Timer is running, record end event and compute elapsed time
            self.end_event.record()
            torch.cuda.synchronize()  # Ensure events are completed
            elapsed_ms = self.start_event.elapsed_time(self.end_event)
            self._increment_global_time(elapsed_ms / 1000.0)  # Convert ms to seconds
            self.is_running = False
        else:
            # Start new timing
    ...
    def cleanup(self):
        """
        Ensures timer is not running and synchronizes CUDA
        """
        if self.is_running:
            print(f"Cleanup: Recording end event on device {torch.cuda.current_device()}")
            self.end_event.record()
            print(f"Cleanup: Synchronizing device {torch.cuda.current_device()}")
