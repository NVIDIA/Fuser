...
class PythonTranslator : public OptInConstDispatch {
 ...
 private:
  ...
  void handle(const GroupedBlockQuantizationOp* grouped_bqop) final {
    ...
    static const auto default_args = std::make_tuple(
        KeywordArgument<decltype(grouped_bqop->globalScale())>{"global_scale", nullptr},
        KeywordArgument<int64_t>{"block_size", 16},
        KeywordArgument<DataType>{"dtype", DataType::Float4_e2m1fn});
    ...
    printer_.generateKwargsOperation(
        ...
        std::make_tuple(grouped_bqop->in(), grouped_bqop->inputOffsets(), grouped_bqop->outputOffsets()),
        default_args,
        std::make_tuple(
            grouped_bqop->globalScale(),
            grouped_bqop->blockSize(),
            dtype),
        std::vector<const nvfuser::Val*>{grouped_bqop->output(0), grouped_bqop->output(1)});
  }
