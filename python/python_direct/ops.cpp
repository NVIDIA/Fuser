...
TensorView
  ...
  ops.def(
      ...
      [](TensorView* mat1,
         ...
         TensorView* scale1,
         TensorView* scale2,
         std::optional<TensorView*> alpha,
         std::optional<TensorView*> bias,
        ...
        auto [output, block_scaling_factor, global_scaling_factor] = grouped_mm(
            ...
            scale1,
            scale2,
            alpha.value_or(nullptr),
            bias.value_or(nullptr),
            beta.value_or(nullptr),
            dtype,
...
void bindSdpaOps(py::module_& ops) {
  ops.def(
      ...
      [](TensorView* query,
         TensorView* key,
         TensorView* value,
         std::optional<Val*> dropout_p,
         std::optional<Val*> is_causal,
         std::optional<Val*> scale) -> decltype(auto) {
        auto [output, log_sumexp, philox_seed, philox_offset] = sdpfa_fwd(
            query,
            key,
            value,
            dropout_p.value_or(nullptr),
            is_causal.value_or(nullptr),
            scale.value_or(nullptr));
...
TensorView* random_dist_op_fn(
    ...
    std::optional<Val*> rng_seed,
    std::optional<Val*> rng_offset,
    PrimDataType dtype) {
  NVF_CHECK(
