...
// Register a caster for CPP c10::ArrayRef<T> to Python list
// Skip conversion from Python Sequence to C++ c10::ArrayRef<T>
template <typename T>
struct type_caster<c10::ArrayRef<T>> {
  // Define the Python type name, e.g., "list[int]"
  NB_TYPE_CASTER(
      c10::ArrayRef<T>,
  ...
  static handle from_cpp(
    ...
    for (const auto& value : src) {
      // Convert each element using Nanobind's existing casters
      nb::handle h = make_caster<T>::from_cpp(value, policy, cleanup);
      if (!h) {
        return nb::handle();
      }
      out.append(h);
