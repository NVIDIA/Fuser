...
# kernel path: /tmp/torchinductor_root/x4/cx4peng5ge4a35oilidhydhcvn6rwoyxbj633qup547wxrfecikx.py
...
def triton_red_fused_native_group_norm_0(in_ptr0, out_ptr0, out_ptr1, out_ptr2, out_ptr3, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp0 = tl.load(in_ptr0 + (r1 + (2048*x0)), rmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
...
# kernel path: /tmp/torchinductor_root/qp/cqpncb74sddanolipz6fzmkjjekjnfvaoqlgqhdfdc5boxamdd4y.py
# Source Nodes: [y], Original ATen: [aten.native_group_norm]
...
def call(args):
    ...
    assert_size_stride(primals_3, (1024, 256, 16, 16), (65536, 256, 16, 1))
    with torch.cuda._DeviceGuard(0):
        ...
        # Source Nodes: [y], Original ATen: [aten.native_group_norm]
        triton_poi_fused_native_group_norm_1.run(primals_3, buf0, buf1, primals_2, primals_1, buf3, 67108864, grid=grid(67108864), stream=stream0)
        del buf0
        del buf1
        del primals_1
...
def benchmark_compiled_module(times=1, repeat=0):
    ...
    primals_3 = rand_strided((1024, 256, 16, 16), (65536, 256, 16, 1), device='cuda:0', dtype=torch.float16)
