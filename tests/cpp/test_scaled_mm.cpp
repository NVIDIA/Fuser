...
// Test basic scaled_mm operation functionality
TEST_F(NVFuserTest, ScaledMm_BasicOperation) {
...
// Test scaled_mm with different matrix sizes
TEST_F(NVFuserTest, ScaledMm_DifferentSizes) {
...
// Test scaled_mm with alpha parameter
TEST_F(NVFuserTest, ScaledMm_WithAlpha) {
  // Skip if not on SM100 or above
  if (at::cuda::getCurrentDeviceProperties()->major < 10) {
    GTEST_SKIP() << "Skipping test on pre-SM100 GPUs";
  }
...
// Test scaled_mm with bias
TEST_F(NVFuserTest, ScaledMm_WithBias) {
  // Skip if not on SM100 or above
  if (at::cuda::getCurrentDeviceProperties()->major < 10) {
    GTEST_SKIP() << "Skipping test on pre-SM100 GPUs";
  }
...
// Test scaled_mm with different output data types
TEST_F(NVFuserTest, ScaledMm_DifferentOutputTypes) {
  // Skip if not on SM100 or above
  if (at::cuda::getCurrentDeviceProperties()->major < 10) {
    GTEST_SKIP() << "Skipping test on pre-SM100 GPUs";
  }
  ...
  EXPECT_EQ(output.sizes()[0], M);
  EXPECT_EQ(output.sizes()[1], N);
  EXPECT_EQ(output.scalar_type(), at::ScalarType::Half);
}

// Test scaled_mm performance comparison with regular matmul
TEST_F(NVFuserTest, ScaledMm_PerformanceComparison) {
  // Skip if not on SM100 or above
  if (at::cuda::getCurrentDeviceProperties()->major < 10) {
    GTEST_SKIP() << "Skipping test on pre-SM100 GPUs";
  }
