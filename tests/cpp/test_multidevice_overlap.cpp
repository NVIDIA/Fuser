...
class OverlapTest : public MultiDeviceTest {
  protected:
    ...
    void SetUp() {
        ...
        // Set up input tensors. We create the full unsharded tensors and define the actual input as the shard corresponding to the current device. Having the full unsharded input on each rank makes it possible to compute the expected result locally, hence, this way of doing is convenient for validating data correctness.
        at::TensorOptions options = at::TensorOptions().dtype(at::kFloat).device(communicator->device());
        ...
        // We pre-allocate the output and some intermediate buffers so we do not rely on torch allocator, which do not behave well with multi-stream programming.
        ...
        auto tc_unsharded_expected_reshaped = at::reshape(tc_unsharded_expected, {params.S, num_devices, params.M/(params.S*num_devices), params.N});
        tc_expected = tc_unsharded_expected_reshaped.index({at::indexing::Slice(),
                            at::indexing::Slice(my_device_index, my_device_index+1), "..."});
    ...
    c10d::Backend* getWorldCommunicator() {
        return world_communicators.at((communicator_running_counter++) % world_communicators.size());
    }
...
// This test implements a reduce-scattered based pipelining overlapping technic, as used in NEMO-megatron transformer, precisely at the second layer of the MLP consisting of a GEMM+Reduce-scatter
...
TEST_F(OverlapTest, SimpleComputeComm) {
    ...
    // validation
    ASSERT_TRUE(tc.allclose(tc_expected, 1e-3, 1e-3)) << "Unexpected results, obtained:"<< tc << "\n expected: " << tc_expected;
}
