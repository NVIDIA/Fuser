...
TEST_F(PresegTest, TranslateRepeatToExpand5) {
  ...
  auto tv1 = pad(tv0, {fusion.zeroVal(), tv0->axis(-1)->extent()});
  auto tv2 = pad(tv0, {tv0->axis(-1)->extent(), fusion.zeroVal()});
...
// Addition-based concatenation followed using BFloat16
TEST_F(PresegTest, TranslateRepeatToExpand6) {
  auto fusion_ptr = std::make_unique<Fusion>();
  Fusion& fusion = *fusion_ptr;
  FusionGuard fg(&fusion);

  auto tv0 = makeContigConcreteTensor({32}, DataType::BFloat16);
  fusion.addInput(tv0);

  auto tv1 = pad(tv0, {fusion.zeroVal(), tv0->axis(-1)->extent()});
  auto tv2 = pad(tv0, {tv0->axis(-1)->extent(), fusion.zeroVal()});
  auto tv3 = add(castOp(DataType::Float, tv1), castOp(DataType::Float, tv2));
...
// Addition-based concatenation followed by a reduction using BFloat16
// inputs
TEST_F(PresegTest, TranslateRepeatToExpand7) {
  auto fusion_ptr = std::make_unique<Fusion>();
  Fusion& fusion = *fusion_ptr;
  FusionGuard fg(&fusion);
  ...
  // Should be scheduled as a reduction kernel
  FusionKernelRuntime* runtime = executor_cache.getMostRecentKernelRuntime();
  EXPECT_FALSE(runtime->isSegmented());
  const auto& heuristic_param =
      runtime->schedulerHeuristics()->heuristicsList().front();
  EXPECT_EQ(heuristic_param->scheduler_type, SchedulerType::Reduction);
}
