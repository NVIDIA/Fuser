...
TEST_F(NVFuserTest, FusionRemoveBcastSqueeze1) {
  auto fusion = std::make_unique<Fusion>();
  FusionGuard fg(fusion.get());
  DataType input_dtype = DataType::Float;
...
TEST_F(NVFuserTest, FusionRemoveBcastSqueeze2) {
  auto fusion = std::make_unique<Fusion>();
  FusionGuard fg(fusion.get());
  DataType input_dtype = DataType::Float;
  auto tv0 = makeContigTensor(2, input_dtype);
...
TEST_F(NVFuserTest, FusionRemoveBcastSqueeze3) {
  auto fusion = std::make_unique<Fusion>();
  FusionGuard fg(fusion.get());
  DataType input_dtype = DataType::Float;
  auto tv0 = makeContigTensor(2, input_dtype);
...
TEST_F(NVFuserTest, FusionRemoveBcastSqueeze4) {
  auto fusion = std::make_unique<Fusion>();
  FusionGuard fg(fusion.get());
  DataType input_dtype = DataType::Float;
  auto tv0 = makeContigTensor(2, input_dtype);
...
TEST_F(NVFuserTest, FusionRemoveBcastSqueeze5) {
  auto fusion = std::make_unique<Fusion>();
  FusionGuard fg(fusion.get());
  DataType input_dtype = DataType::Float;
  auto tv0 = makeContigTensor(2, input_dtype);
...
TEST_F(NVFuserTest, FusionRemoveSqueezeBcast1) {
  auto fusion = std::make_unique<Fusion>();
  FusionGuard fg(fusion.get());
  DataType input_dtype = DataType::Float;
...
TEST_F(NVFuserTest, FusionRemoveSqueezeBcast2) {
  auto fusion = std::make_unique<Fusion>();
  FusionGuard fg(fusion.get());
  DataType input_dtype = DataType::Float;
  const std::vector<bool> is_broadcast_dim{false, true};
...
TEST_F(NVFuserTest, FusionRemoveSqueezeBcast3) {
  auto fusion = std::make_unique<Fusion>();
  FusionGuard fg(fusion.get());
  DataType input_dtype = DataType::Float;
  const std::vector<bool> is_broadcast_dim{false, true};
...
TEST_F(NVFuserTest, FusionRemoveSqueezeBcastSqueezeBcast) {
  auto fusion = std::make_unique<Fusion>();
  FusionGuard fg(fusion.get());
  DataType input_dtype = DataType::Float;
  const std::vector<bool> is_broadcast_dim{false, true};
...
TEST_F(NVFuserTest, FusionRemoveSqueezeBcastBcastSqueeze) {
  auto fusion = std::make_unique<Fusion>();
  FusionGuard fg(fusion.get());
  DataType input_dtype = DataType::Float;
