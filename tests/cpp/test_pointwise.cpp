...
TEST_P(TmaPointwiseBcastTest, InnerOuterBcast) {
  ...
  // Flattening would cause transformation propagation to fail for broadcast
  // tensors because it would merge broadcast and non-broadcast val groups.
...
TEST_P(TmaPointwiseBcastTest, InnerMiddleOuterBcast) {
  int64_t dim0 = 128;
  int64_t dim1 = 256;
  int64_t dim2 = 4096;
  DataType dtype = DataType::Float;
  ...
  auto tv1 = makeContigConcreteTensor({dim0, dim2}, dtype);
  auto tv2 = makeContigConcreteTensor({dim1, dim2}, dtype);
  auto tv3 = makeContigConcreteTensor({dim0, dim1, dim2}, dtype);
  fusion->addInput(tv0);
  fusion->addInput(tv1);
  fusion->addInput(tv2);
  fusion->addInput(tv3);
  // break after dim-0, tv2 is outer broadcast, tv0 and tv1 are inner broadcast
  // tvs.
  auto tv4 = broadcast(tv0, {false, false, true});
  auto tv5 = broadcast(tv1, {false, true, false});
  auto tv6 = broadcast(tv2, {true, false, false});
  auto tv7 = add(tv3, tv4);
  auto tv8 = add(tv5, tv6);
  auto tv9 = add(tv7, tv8);
  fusion->addOutput(tv9);
  auto options =
      at::TensorOptions().dtype(data_type_to_aten(dtype)).device(at::kCUDA, 0);
  auto t0 = at::randn({dim0, dim1}, options);
  auto t1 = at::randn({dim0, dim2}, options);
  auto t2 = at::randn({dim1, dim2}, options);
  auto t3 = at::randn({dim0, dim1, dim2}, options);

  auto [use_auto_scheduler, tma_inner_bcast, tma_outer_bcast] = GetParam();
  if (use_auto_scheduler) {
    // Test using the automatic pointwise scheduler
    auto cg_results =
        scheduleAndRun(fusion, SchedulerType::PointWise, {t0, t1, t2, t3});
    testValidate(
        fusion, cg_results.outputs, {t0, t1, t2, t3}, __LINE__, __FILE__);
    return;
  }
  // CUDA_ERROR_ILLEGAL_INSTRUCTION
  if (tma_inner_bcast) {
    GTEST_SKIP()
        << "TMA domain mixing broadcast and non-broadcast is not supported";
    return;
  }
  // Manual scheduling: Test TMA load/store with broadcast tensors Cache the
  // main input (tv3) and output (tv9) using TMA
  auto tv3_smem = tv3->cacheAfter(LoadStoreOpType::CpAsyncBulkTensorTile);
  tv3_smem->setMemoryType(MemoryType::Shared);
  auto tv9_smem = tv9->cacheBefore(LoadStoreOpType::CpAsyncBulkTensorTile);
  tv9_smem->setMemoryType(MemoryType::Shared);
  std::vector<TensorView*> tma_tvs = {tv3_smem, tv9};
  std::vector<TensorView*> ldg_tvs = {};
  std::unordered_set<TensorView*> vectorizable_ldg_tvs = {};
  ...
  if (tma_inner_bcast) {
    ...
    tv0_smem->setMemoryType(MemoryType::Shared);
    tv1_smem->setMemoryType(MemoryType::Shared);
    tma_tvs.push_back(tv0_smem);
    tma_tvs.push_back(tv1_smem);
  } else {
    auto tv0_regs = tv0->cacheAfter();
    auto tv1_regs = tv1->cacheAfter();
    ldg_tvs.push_back(tv0_regs);
    ldg_tvs.push_back(tv1_regs);
  }
  ...
  // Use tv9_smem as reference for transformation propagation
  // Note: We can't use tv9 directly because TMA store operations only have
  // TMA tile domains, not computation domains. tv9_smem has both.
  auto reference_tv = tv9_smem;

  // ============================================================================
  // TMA BROADCAST: Why flattening FAILS in this test (NOW CAUGHT!)
  // ============================================================================
  //
  // IMPORTANT UPDATE: As of the fix to tma.cpp, flattening is now correctly
  // REJECTED in this test because it leads to incorrect indexing!
  //
  // THE PROBLEM:
  // - Input gmem tensors have NO broadcast in their allocation domain
  //   (tv2: {256, 4096})
  // - Broadcast operations happen AFTER the TMA load:
  //   tv6 = broadcast(tv2_tma_loaded, {true, false, false})
  // - tv6's logical domain becomes: (bS15{1}, iS16{256}, iS17{4096})
  // - Flattening causes: Merge(bS15{1}, iS16{256}) - mixing broadcast and
  //   non-broadcast!
  //
  // WHY IT WASN'T CAUGHT BEFORE:
  // - TMA analysis only validated the path from smem_tv to gmem_tv
  // - The broadcast happened AFTER the TMA load, outside TMA analysis scope
  // - No error was raised, but it produced SILENT NUMERICAL ERRORS
  //
  // THE FIX:
  // - Added validateTMAConsumerBroadcasts() in tma.cpp
  // - Now checks ALL downstream tensors that transitively depend on TMA-loaded
  //   tensors (not just direct consumers)
  // - Example: t1=tma_load(t0); t2=exp(t1); t3=broadcast(t2) - checks t2, t3,
  // etc.
  // - Catches cases where broadcast dimensions get merged with non-broadcast
  //   dimensions anywhere in the downstream transformation paths
  //
  // CONTRAST with InnerMiddleOuterBcastInputs:
  // - That test has broadcast IN the gmem allocation domain (tv2: {1, 256,
  //   4096})
  // - Was already caught by TMA analysis because broadcast dimensions are
  //   filtered from frontier
  // - Both tests now correctly reject flattening when it would merge broadcast
  //   and non-broadcast dimensions
  //
  // ============================================================================
  //
  // For 2D scheduling, keep the two dimensions separate (don't flatten)
  ...
  // The code below is disabled (false condition) to maintain 2D scheduling.
  // Schedule the TMA domain [I0, I1] (not flattened)
  // If branch: [dim0, dim1, dim2] -> [dim0*dim1*dim2] -> [dim0, dim1*dim2]
  // T13_s_float[iblockIdx.y131{1}, iblockIdx.x133{4096}, iB132{8}, iB134{256}]
  // ca_pos( 2 ) logical domain : (iS34{256}, iS35{4096}) contiguity: t t
  //   Merge: iS34{256} and iS35{4096} -> iS128{1048576}
  //   Split: iS128{1048576} by factor 1048576 -> iS129{1}, iS130{1048576}
  //   Split: iS129{1} by factor 8 -> iblockIdx.y131{1}, iB132{8}
  //   Split: iS130{1048576} by factor 256 -> iblockIdx.x133{4096}, iB134{256}
  // loop domain : (iblockIdx.y131{1}, iblockIdx.x133{4096}, iB132{8},
  // iB134{256}) Else branch: [dim0, | dim1, dim2] -> [dim0, dim1*dim2]
  // T13_s_float[iblockIdx.x93{4096}, iB94{256}] ca_pos( 1 )
  // logical domain : (iS34{256}, iS35{4096})
  // contiguity: t t
  //   Merge: iS34{256} and iS35{4096} -> iS92{1048576}
  //   Split: iS92{1048576} by factor 256 -> iblockIdx.x93{4096}, iB94{256}
  // loop domain : (iblockIdx.x93{4096}, iB94{256})
  if (/*is_1d_scheduler=*/true) {
    // [dim0, dim1, dim2] -> [dim0*dim1*dim2] -> [dim0, dim1*dim2]
    reference_tv->flatten();
    int64_t tma_domain_size_inner = dim1 * dim2;
    reference_tv->split(0, tma_domain_size_inner);
  } else {
    // merge based on break point position of 1
  ...
  // The first 2 dimensions are parallelized by BIDy and BIDx, We need this
  // reorder so we can inline at position-2. Otherwise, we need the producer
  // needs stay in global memory due to unexact parallelization.
  const bool is_reorder = true;
  reference_tv->split(1, tma_tile_inner);
  reference_tv->split(0, tma_tile_outer);
  if (is_reorder) {
    // [I0/to, to, I1/ti, ti] -> [I0/to, I1/ti, to, ti]
  ...
  // Axis 3: BIDx (block X), Axis 5: TIDx (thread X)
  if (is_reorder) {
    // [Do/to, Di/ti, to/y, y, ti/v/x, x, v]
    reference_tv->axis(0)->parallelize(ParallelType::BIDy);
    reference_tv->axis(1)->parallelize(ParallelType::BIDx);
    reference_tv->axis(3)->parallelize(ParallelType::TIDy);
    reference_tv->axis(5)->parallelize(ParallelType::TIDx);
  } else {
    // [Do/to, to/y, y, Di/ti, ti/v/x, x, v]
    reference_tv->axis(0)->parallelize(ParallelType::BIDy);
    reference_tv->axis(2)->parallelize(ParallelType::TIDy);
    reference_tv->axis(3)->parallelize(ParallelType::BIDx);
    reference_tv->axis(5)->parallelize(ParallelType::TIDx);
  ...
  auto out_tensors = ke.run({t0, t1, t2, t3});
  testValidate(fusion, out_tensors, {t0, t1, t2, t3}, __LINE__, __FILE__);
}

TEST_P(TmaPointwiseBcastTest, InnerMiddleOuterBcastInputs) {
  int64_t dim0 = 128;
  int64_t dim1 = 256;
  int64_t dim2 = 4096;
  DataType dtype = DataType::Float;
  ...
  auto fusion = fusion_ptr.get();
  FusionGuard fg(fusion);
  auto tv0 = makeContigConcreteTensor({dim0, dim1, 1}, dtype);
  auto tv1 = makeContigConcreteTensor({dim0, 1, dim2}, dtype);
  auto tv2 = makeContigConcreteTensor({1, dim1, dim2}, dtype);
  auto tv3 = makeContigConcreteTensor({dim0, dim1, dim2}, dtype);
  fusion->addInput(tv0);
  fusion->addInput(tv1);
  fusion->addInput(tv2);
  ...
  auto options =
      at::TensorOptions().dtype(data_type_to_aten(dtype)).device(at::kCUDA, 0);
  auto t0 = at::randn({dim0, dim1, 1}, options);
  auto t1 = at::randn({dim0, 1, dim2}, options);
  auto t2 = at::randn({1, dim1, dim2}, options);
  auto t3 = at::randn({dim0, dim1, dim2}, options);
  ...
  // Manual scheduling: Test TMA load/store with broadcast tensors Cache the
  // main input (tv3) and output (tv7) using TMA
  auto tv3_smem = tv3->cacheAfter(LoadStoreOpType::CpAsyncBulkTensorTile);
  tv3_smem->setMemoryType(MemoryType::Shared);
  auto tv6_smem = tv6->cacheBefore(LoadStoreOpType::CpAsyncBulkTensorTile);
  tv6_smem->setMemoryType(MemoryType::Shared);
  std::vector<TensorView*> tma_tvs = {tv3_smem, tv6};
  std::vector<TensorView*> ldg_tvs = {};
  std::unordered_set<TensorView*> vectorizable_ldg_tvs = {};
  ...
  // ============================================================================
  // TMA BROADCAST RESTRICTION: Why we need 2D scheduling (no flattening)
  // ============================================================================
  //
  // IMPORTANT: This restriction ONLY applies when the gmem tensor being loaded
  // by TMA already has broadcast dimensions in its allocation domain (like the
  // inputs in this test: tv2 has shape {1, 256, 4096}). If broadcast happens
  // AFTER the TMA load via broadcast() ops (see InnerMiddleOuterBcast test),
  // then flattening is fine because TMA never sees the broadcast dimensions.
  //
  // When using TMA with tensors that have broadcast dimensions in their
  // allocation domain, we must carefully avoid merging broadcast and
  // non-broadcast dimensions. This is a fundamental restriction in how TMA
  // analysis handles broadcast dimensions.
  //
  // THE PROBLEM:
  // ------------
  // The TMA analysis in tma.cpp (view_gmem_alloc_domain_with_exprs::run) works
  // by traversing from the consumer's loop domain to the gmem tensor's
  // allocation domain. During this traversal, it maintains a "frontier" that
  // represents the current domain being analyzed.
  //
  // However, in getGmemAllocDomain() (line 636 of tma.cpp), broadcast
  // dimensions are explicitly FILTERED OUT from this frontier:
  //
  //   if (id->isBroadcast()) {
  //     continue;  // Broadcast dimensions are skipped!
  //   }
  //
  // This filtering is necessary because TMA hardware operations don't transfer
  // broadcast dimensions (they have no actual data).
  //
  // THE FAILURE MODE:
  // -----------------
  // If we flatten or merge dimensions in a way that combines broadcast and
  // non-broadcast dimensions, the transformation path will contain merge/split
  // expressions involving broadcast dimensions. When the TMA analysis tries to
  // process these expressions in handleTwoToOne(), it expects to find ALL
  // involved dimensions in the frontier, but the broadcast dimensions were
  // filtered out! This causes the assertion failure:
  //
  //   "The TMA domain must be equivalent to the allocation domain of the
  //    gmem tensor, but { bS33{1}; bS6{1} } is not on the path"
  //
  // EXAMPLE WITH OUR TEST:
  // ----------------------
  // Consider tv2: {1, dim1, dim2} (outer broadcast, used with TMA when
  // tma_outer_bcast=true)
  //
  // If we flatten: [1, dim1, dim2] -> [1*dim1*dim2]
  //   This merge combines broadcast dimension {1} with non-broadcast {dim1}.
  //   TMA analysis will fail because {1} is not in the frontier!
  //
  // If we do 2D merge: [1, | dim1, dim2] -> [1, dim1*dim2]
  //   This keeps broadcast {1} separate from non-broadcast {dim1*dim2}.
  //   The broadcast dimension stays isolated and TMA analysis succeeds!
  //
  // THE SOLUTION: 2D Scheduling
  // ---------------------------
  // We use a "break point" based merge that respects the broadcast structure:
  // - Keep broadcast and non-broadcast dimensions in separate groups
  // - Only merge dimensions within the same group (all broadcast or all
  //   non-broadcast)
  // - For this test with break point at position 1: merge(1) creates
  //   [dim0, dim1*dim2], keeping the structure aligned with broadcast patterns
  //
  // This ensures that when TMA analysis processes the transformation path, it
  // never encounters merge operations that would require finding broadcast
  // dimensions in the frontier.
  //
  // ============================================================================
  ...
  if (/*is_1d_scheduler=*/false) {
    // [dim0, dim1, dim2] -> [dim0*dim1*dim2] -> [dim0, dim1*dim2]
    // WARNING: This would fail with broadcast tensors using TMA!
    // It would merge broadcast and non-broadcast dimensions.
    reference_tv->flatten();
    int64_t tma_domain_size_inner = dim1 * dim2;
    reference_tv->split(0, tma_domain_size_inner);
  } else {
    // merge based on break point position of 1
    // [dim0, | dim1, dim2] -> [dim0, dim1*dim2]
    // This respects the broadcast structure and works with TMA
    reference_tv->merge(1);
  }
