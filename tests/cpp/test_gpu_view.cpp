...
TEST_F(GpuViewTest, FusionViewDtypeSameSizeOutput) {
  ...
  auto cg_outputs =
      scheduleAndRun(&fusion, SchedulerType::PointWise, {at_x, at_bias}).outputs;
...
TEST_F(GpuViewTest, FusionReshapeConcreteDomain3) {
  ...
  FusionExecutorCache executor_cache(std::move(fusion_ptr));
  auto outputs = executor_cache.runFusionWithInputs_deprecated({at_x, at_y, at_z});
...
TEST_F(GpuViewTest, FusionReshapeZeroDimOutput) {
  ...
  auto cg_outputs =
      scheduleAndRun(&fusion, SchedulerType::PointWise, {at_x, at_y, at_z}).outputs;
