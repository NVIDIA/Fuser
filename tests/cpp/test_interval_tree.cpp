...
//! Complexity analysis (where N = number of intervals, k = number of overlapping results):
//!
...
TEST_F(IntervalTreeTest, BasicInsertAndQuery) {
  ...
  for (int point : test_points) {
    ...
    EXPECT_EQ(tree_result, naive_result) << "Point query failed for point " << point;
...
TEST_F(IntervalTreeTest, FromIntervals) {
  auto tree = CenteredIntervalTree<int, std::string>::fromIntervals(test_intervals_);
  NaiveIntervalTree<int, std::string> naive;
...
TEST_F(IntervalTreeTest, Remove) {
  ...
  for (int point : test_points) {
    ...
    EXPECT_EQ(tree_result, naive_result) << "Query failed after removal for point " << point;
  }
}
...
TEST_F(IntervalTreeTest, DifferentCoordinateTypes) {
  ...
  for (double point : test_points) {
    ...
    EXPECT_EQ(tree_result, naive_result) << "Double query failed for point " << point;
  }
}
...
TEST_F(IntervalTreeTest, ComplexPayload) {
  ...
  for (int point = 0; point < 8; ++point) {
    ...
    EXPECT_EQ(tree_result.size(), naive_result.size()) << "Size mismatch for point " << point;
    ...
    EXPECT_EQ(tree_result, naive_result) << "Complex query failed for point " << point;
  }
}
...
TEST_F(IntervalTreeTest, EdgeCases) {
  ...
  EXPECT_EQ(result, std::vector<std::string>{"point", "wide"});
...
TEST_F(IntervalTreeTest, StressTest) {
  ...
  for (int i = 0; i < num_point_queries; ++i) {
    ...
    EXPECT_EQ(tree_result, naive_result) << "Stress test failed for point " << point;
    EXPECT_EQ(tree_result.size(), naive_result.size()) 
        << "Result size mismatch for point " << point;
  }
...
TEST_F(IntervalTreeTest, DependencyMapperCoords) {
  ...
  for (const auto& point : test_points) {
    ...
    EXPECT_EQ(tree_result, naive_result) 
        << "Point query failed for point [" << point[0] << ", " << point[1] << ", " << point[2] << "]";
    EXPECT_EQ(tree_result.size(), naive_result.size()) 
        << "Result size mismatch for point [" << point[0] << ", " << point[1] << ", " << point[2] << "]";
  }
  ...
  for (const auto& [start, end] : test_ranges) {
    ...
    EXPECT_EQ(tree_result, naive_result) 
        << "Range query failed for range [[" << start[0] << "," << start[1] << "," << start[2] 
        << "], [" << end[0] << "," << end[1] << "," << end[2] << "]]";
    EXPECT_EQ(tree_result.size(), naive_result.size()) 
        << "Result size mismatch for range [[" << start[0] << "," << start[1] << "," << start[2] 
        << "], [" << end[0] << "," << end[1] << "," << end[2] << "]]";
  }
...
TEST_F(IntervalTreeTest, DependencyMapperCoordsEdgeCases) {
  ...
  std::sort(result.begin(), result.end());
  EXPECT_EQ(result, std::vector<int>{42, 100, 200});
