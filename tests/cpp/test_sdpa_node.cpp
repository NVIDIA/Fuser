...
auto validateSdpaFwdOutputs = [](std::vector<at::Tensor> nvf_out,
  ...
  EXPECT_TRUE(at::allclose(nvf_out[0].toTensor(), attn));
  EXPECT_TRUE(at::allclose(nvf_out[1].toTensor(), log_sumexp));
};
...
TEST_F(SDPATest, AttnProgram) {
  ...
  EXPECT_TRUE(at::allclose(out[0].toTensor(), expected_out));
}
