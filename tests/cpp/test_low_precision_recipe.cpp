...
TEST_P(NVFP4QuantizeTest, WithoutPerTensorAmaxAgainstDeviceFunction) {
  ...
  auto cols = 32;
  inputs.push_back(
      at::ones({rows, cols}, at::device(at::kCUDA).dtype(at::kFloat))
          .to(data_type_to_aten(data_hp_dtype)));
  auto outputs = fec.runFusionWithInputs(inputs);
  ...
  cudaError_t kernel_err = fp_conversion_kernel_execute<4, 32>(
      d_input, 16, total_elements, &h_output_e2m1, &h_output_e4m3, 0);
