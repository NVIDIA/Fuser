...
TEST_F(AliasTest, NotAllOutputsAlias_Pointwise) {
  ...
  for (SegmentedGroup* group : runtime->fusionSegments()->groups()) {
    if (group->heuristicType() == HeuristicType::PointWise) {
...
TEST_F(AliasTest, DoNotOverSegment_WithForks) {
  ...
  EXPECT_THAT(
      runtime->fusionSegments()->groups(),
      Contains(HeuristicIs(HeuristicType::PointWise)).Times(1));
...
TEST_F(AliasTest, InplaceUpdate) {
  ...
  EXPECT_THAT(
      runtime->fusionSegments()->groups(),
      UnorderedElementsAre(HeuristicIs(HeuristicType::PointWise)));
}
