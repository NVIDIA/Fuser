...
//   TensorView* tv0 = makeContigTensor(4); //[D, DIDx(D), M/D, K/D]
//   TensorView* tv1 = makeContigTensor(3); //[DIDx(D), K/D, N]
//   TensorView* tv1b = broadcast(
//       tv1, {true, false, false, false}); //[1, DIDx(D), K/D, N]
//   TensorView* tv2_unreduced = matmul(tv0, tv1b); //[Stream(D), DIDx(D), M/D, N]
//   TensorView* tv2_unreduced_resharded = set(tv2_unreduced); //[DIDx(D), Stream(D), M/D, N]
//   TensorView* tv2 = sum(tv2_unreduced_resharded, {1}); //[DIDx(D), r(D), M/D, N]
...
//   TensorView* tv2_unreduced = matmul(tv0, tv1b); //[DIDx(D), Stream(D), M/D, N]
//   TensorView* tv2_unreduced_resharded = set(tv2_unreduced); // [Stream(D), DIDx(D), M/D, N]
//   TensorView* tv2 = sum(tv2_unreduced_resharded, {1}); //[r(D), DIDx(D), M/D, N]
  ...
  // tv2_unreduced_resharded [Stream(D), DIDx(D), M/D, N]
  // is inserted here
