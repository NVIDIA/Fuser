...
TEST_F(MultiDeviceTutorial, HostIrLaunchingFusion) {
  ...
  // validate the result
  EXPECT_TRUE(torch::allclose(2 * aten_input + 1, outputs.at(0).toTensor()));
}
...
TEST_F(MultiDeviceTutorial, HostIrLaunchingThreeFusions) {
  ...
  // validate the result
  EXPECT_TRUE(torch::allclose(4 * aten_tv0 + 5, outputs.at(0).toTensor()));
}
...
TEST_F(MultiDeviceTutorial, HostIrGemmReduceScatter) {
  ...
  // "validate" the result
  EXPECT_EQ(outputs.at(0).toTensor().numel(), (M * N) / communicator_->size());
}
...
TEST_F(MultiDeviceTutorial, HostIrKernekPipelining) {
  ...
  // validate the result
  EXPECT_TRUE(torch::allclose(outputs.at(0).toTensor(), aten_tv0 + 3));
}
