...
TEST_P(HostIrTest, TwoFusions) {
  ...
  auto ref_output =
      at::sum(at::relu(t0), at::OptionalIntArrayRef({0, 1})) * 2;
...
TEST_P(HostIrTest, ThreeFusions) {
  ...
  auto outputs =
      hie.runWithInput({{post_on_stream_0->inputs().at(0), t0_0}});
...
TEST_P(SliceHostIrTest, SlicingTensor) {
  ...
  // validate
  std::vector<at::indexing::TensorIndex> ranges_aten(t0.dim(), at::indexing::Slice());
...
TEST_F(MatmulHostIrTest, HostIrMatmulOut) {
  ...
  std::unordered_map<Val*, c10::IValue> concrete_input_buffers = {{tv0, t0}, {tv1, t1}, {tv2, t2}};
