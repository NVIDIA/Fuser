...
TEST_F(PointwiseTest, DomainMapFactory) {
  ...
  auto t3 = (input0 + input1).as_strided({1, 7, 25}, {7*25, 25, 1});
  at::manual_seed(0);
  auto t4 = generate_uniform(4 * 25, at::kFloat).as_strided({4, 1, 25}, {25, 25, 1});
  auto t5 = t4 + input0;
  
  testValidate(fusion, cg_outputs, {input0, input1}, {t3, t5}, __LINE__, __FILE__);
}
