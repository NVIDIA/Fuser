...
TEST_F(NVFuserTest, FusionTrivialReduction2_CUDA) {
  ...
  auto cg_outputs =
      scheduleAndRun(&fusion, SchedulerType::PointWise, {t0, t1});
...
TEST_F(NVFuserTest, FusionBiasGeluFwd_CUDA) {
  ...
  testValidate(&fusion, cg_outputs.outputs, {at_bias, at_input}, __LINE__, __FILE__);
}
...
TEST_F(NVFuserTest, FusionBiasGeluBwd_CUDA) {
  ...
  auto cg_outputs =
      scheduleAndRun(&fusion, SchedulerType::PointWise, {at_grad, at_bias, at_input});
...
TEST_F(NVFuserTest, FusionSmemIndexing_CUDA) {
  ...
  testValidate(
      &fusion, cg_outputs, {t0, t1, 3, 4, 5}, {aten_output}, __LINE__, __FILE__);
}
...
TEST_F(NVFuserTest, FusionIssue367_CUDA) {
  ...
  testValidate(
      &fusion, cg_outputs, {t0, t1, 2, 2, 3}, {aten_output}, __LINE__, __FILE__);
}
...
TEST_F(NVFuserTest, FusionVectorization3_CUDA) {
  ...
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-goto,hicpp-avoid-goto)
  ASSERT_ANY_THROW(ke.run({t0.index({"...", at::indexing::Slice(1)}), t1.index({"...", at::indexing::Slice(1)})}));
...
TEST_F(NVFuserTest, FusionBlockWelfordInSerialLoop_CUDA) {
  ...
  testValidate(
      &fusion, outputs, {t0}, {aten_avg, aten_M2}, __LINE__, __FILE__);
}
...
TEST_F(NVFuserTest, FusionBNBackwardRepro_CUDA) {
  ...
  auto outputs = executor_cache.runFusionWithInputs_deprecated({t0, t1, t2, t3, t4, t5, t6, t7});
}
...
TEST_F(NVFuserTest, FusionBNRepro_CUDA) {
  ...
  FusionExecutorCache executor_cache(std::move(fusion_ptr));
  auto cg_outputs = executor_cache.runFusionWithInputs_deprecated({t1, t2, t3, t4, t5});
...
TEST_F(NVFuserTest, FusionZeroSizeTensorPW_CUDA) {
  ...
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-goto,hicpp-avoid-goto)
  ASSERT_ANY_THROW(SchedulerEntry::scheduleAndRun(&fusion, SchedulerType::PointWise, {t0, t1}));
}
...
TEST_F(NVFuserTest, FusionZeroSizeTensorNormalization_CUDA) {
  ...
  auto cg_results = scheduleAndRun(
      &fusion, SchedulerType::OuterPersistent, {t0, t1}, false);
...
TEST_F(NVFuserTest, FusionSimpleWarp_CUDA) {
  ...
  testValidate(
      fusion.get(), outputs, {t1}, {at_output}, __LINE__, __FILE__);
}
...
TEST_F(NVFuserTest, FusionPointwiseBroadcast_CUDA) {
  ...
  testValidate(&fusion, cg_outputs.outputs, {at_x, at_bias}, __LINE__, __FILE__);
}
