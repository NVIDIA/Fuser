...
TEST_F(TMATest, CpAsyncBulkPredicateCircularBufferStages2) {
  NVFUSER_TEST_CUDA_ARCH_GUARD(9, 0);
  constexpr at::ScalarType dtype = at::ScalarType::Float;
  CompileParams index32bit{DataType::Int32, 255, false};
...
TEST_F(TMATest, CpAsyncBulkPredicateCircularBufferStages3) {
  NVFUSER_TEST_CUDA_ARCH_GUARD(9, 0);
  constexpr at::ScalarType dtype = at::ScalarType::Float;
  CompileParams index32bit{DataType::Int32, 255, false};
...
TEST_F(TMATest, CpAsyncBulkPredicateCircularBufferStages5) {
  NVFUSER_TEST_CUDA_ARCH_GUARD(9, 0);
  constexpr at::ScalarType dtype = at::ScalarType::Float;
  CompileParams index32bit{DataType::Int32, 255, false};

  const int64_t dim0 = 8195;
  const int64_t dim1 = 512;
  const int64_t number_of_stages = 5;
  auto fusion = std::make_unique<Fusion>();
  FusionGuard fg(fusion.get());
  auto tv0 = makeContigTensor(2, aten_to_data_type(dtype));
  fusion->addInput(tv0);
  ...
  if (number_of_stages > 1) {
    tv0a->circularBuffer(
        number_of_stages, 2, Pipelined(true));
  }
