...
TEST_F(TMATest, CpAsyncBulkPredicateCircularBuffer) {
  NVFUSER_TEST_CUDA_ARCH_GUARD(9, 0);
  constexpr at::ScalarType dtype = at::ScalarType::Float;
  CompileParams index32bit{DataType::Int32, 255, false};
  ...
  // inline
  inlineSelectedAt({tv0a}, tv0a, 1);
  inlineMost(std::unordered_set<TensorView*>{tv1});

  tv0a->circularBuffer(
      2, 1, WarpSpecialized(ParallelType::TIDy));
...
TEST_F(TMATest, CpAsyncBulkPredicateCircularBufferUnroll) {
  NVFUSER_TEST_CUDA_ARCH_GUARD(9, 0);
  constexpr at::ScalarType dtype = at::ScalarType::Float;
  CompileParams index32bit{DataType::Int32, 255, false};
...
TEST_P(UblkPredicateTest, testUnrollCircularBuffer) {
  ...
  int64_t sm_count = at::cuda::getCurrentDeviceProperties()->multiProcessorCount;
