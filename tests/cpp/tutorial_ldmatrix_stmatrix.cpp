...
```
Reference: [LdMatrix
PTX](https://docs.nvidia.com/cuda/parallel-thread-execution/#warp-level-matrix-load-instruction-ldmatrix)
...
Reference: [StMatrix
PTX](https://docs.nvidia.com/cuda/parallel-thread-execution/#warp-level-matrix-store-instruction-stmatrix)
...
![Register layout for LdMatrix /
StMatrix](ldstmatrix/ldstmatrix_register_layout.svg)
...
**Step 1:** Split m by two compute warp groups; Split n by four tiles for 128B
swizzle
...
### Figure 3: Map from LdMatrix / StMatrix loop domain to TMA shared memory
allocation domain
 ...
 * Split `miii(8)` into `miiio(8)` and `miiii(1)` by repetitions for 128B
swizzle.
