...
class IdModelTester : public LoopPromotionMapBuilderCallback {
 public:
  ...
  // The test validation depends on the particular visiting order
  // generated by ValGraphStmtSort since different replay order
  // would result in different names of promotion IDs. Since
  // ValGraphStmtSort is no longer used, many of the tests would
  // fail simply because ID names would change.
  //
  // Instead of updating all those tests, replace the ordering by
  // the one produced by ValGraphStmtSort. This should not matter
  // for the algorithmic correctness of the IdModel build processes
  // as long as the default ordering is also topologically sorted.
  ExprGroups updateOrderedExprGroupsForPropagation(
      const ExprGroups& ordered_exprs,
      const ValGraph& graph) override {
    // Make sure the original exprs are topologically sorted too
