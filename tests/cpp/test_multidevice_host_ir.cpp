    ...
    */
    ...
    std::vector<Val*> post_inputs = {ir_cloner.clone(hu->fusion_to_execute()->inputs().front())};
    std::vector<Val*> post_outputs = {ir_cloner.clone(hu->fusion_to_execute()->outputs().front())};
    ...
    hic->top_level_exprs.push_back(post_compute);

    std::vector<Val*> comm_inputs = post_outputs.front();
    auto tv2 = set(comm_inputs.front());
    ...
    hic->top_level_exprs.push_back(post_comm);

    // add global IO to the HostIrContainer. This step could potentially be infered automatically
    hic->addInput(post_compute->inputs().front());
    hic->addOutput(post_comm->outputs().front());
    hic->print();
    ...
    auto options = at::TensorOptions().device(at::kCUDA, communicator->device());
    ...
    GTEST_EXPECT_TRUE(torch::allclose(ref_output, outputs.front()));
    }
