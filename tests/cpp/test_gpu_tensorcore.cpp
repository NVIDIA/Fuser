      ...
      for (bool downcast_output : {false, true}) {
        for (const auto& [M, N, K, alignA, alignB, alignBias] :
          ...
          params.supported_vec_size = {
              .operands =
                  {atenSupportedVectorization(t0),
                   atenSupportedVectorization(t1)},
              .epilogue_inputs = {alignBias},
              .outputs = {std::min(
          ...
          params.async_gmem_load_operands =
              params.supported_vec_size.operands[0] >= 4 &&
              params.supported_vec_size.operands[1] >= 4;
          params.double_buffer_options.double_buffer_smem_write = true;
