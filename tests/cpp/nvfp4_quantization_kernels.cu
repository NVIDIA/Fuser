...
cudaError_t fp_conversion_kernel_execute(
  ...
  err = launch_float_to_fp_kernel<BLOCK_DIM_X, BLOCK_DIM_Y>(
      d_input, d_output_e2m1, d_output_e4m3, num_rows, total_elements, stream);
...
#define INSTANTIATE_FP_CONVERSION_KERNEL(BLOCK_DIM_Y)                \
  template cudaError_t fp_conversion_kernel_execute<4, BLOCK_DIM_Y>( \
      const float* d_input,                                          \
      const int INNER_DIM,                                           \
      int total_elements,                                            \
      nvfp4_types::__e2m1** h_output_e2m1,                           \
...
// always 4)
INSTANTIATE_FP_CONVERSION_KERNEL(1)
INSTANTIATE_FP_CONVERSION_KERNEL(4)
INSTANTIATE_FP_CONVERSION_KERNEL(8)
INSTANTIATE_FP_CONVERSION_KERNEL(32)
