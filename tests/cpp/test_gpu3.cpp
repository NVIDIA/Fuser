...
TEST_F(NVFuserTest, FusionExpandRepro1860_CUDA) {
  ...
  auto outputs = executor_cache.runFusionWithInputs_deprecated({t1, t2, t3, t4});
}
...
TEST_F(NVFuserTest, FusionAvoidRedundantWriteBroadcastedSoftmaxInput_CUDA) {
  ...
  testValidate(executor_cache.fusion(), cg_outputs, {t0, t1}, __LINE__, __FILE__);
}
  ...
  for (auto dtype : {DataType::Float, DataType::Double}) {
    for (auto do_min : {true, false}) {
      ...
      for (auto size : {1, 2, 5}) {
        ...
        auto nvf_outputs = executor_cache.runFusionWithInputs_deprecated({at_x});
...
TEST_F(NVFuserTest, MoveNonConcretizedBroadcastInNormalization) {
  ...
  testValidate(&fusion_copy, cg_outputs.outputs, aten_inputs, __LINE__, __FILE__);
...
TEST_F(NVFuserTest, MoveNonConcretizedBroadcastInPointwise) {
  ...
  auto cg_outputs =
      scheduleAndRun(&fusion, SchedulerType::PointWise, {input0, t1})
          .outputs;
  testValidate(&fusion, cg_outputs, {input0, t1}, __LINE__, __FILE__);
...
TEST_F(NVFuserTest, MoveNonConcretizedBroadcastInReduction) {
  ...
  auto cg_outputs =
      scheduleAndRun(&fusion, SchedulerType::Reduction, {t0, t1});
...
TEST_F(NVFuserTest, Issue2685Repro) {
  ...
  Fusion fusion_copy = fusion;
  auto cg_outputs =
      scheduleAndRun(&fusion, SchedulerType::InnerPersistent, {t0, t1, t2, t10});
  testValidate(&fusion_copy, cg_outputs.outputs, {t0, t1, t2, t10}, __LINE__, __FILE__);
}
