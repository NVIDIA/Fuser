...
def test_transformer_backward(mpi_test):
    ...
    mha_linear1_out = torch.testing.make_tensor(
        d, b, s, e // d, dtype=torch.bfloat16, device="cpu"
    )
    mlp_linear0_weight = torch.testing.make_tensor(
        d, e * 4 // d, e, dtype=torch.bfloat16, device="cpu"
    )
    ...
    ins = [
        ...
        29,
        2722423872872113,
        mha_linear1_out[rank : rank + 1].cuda(),
        mlp_linear0_weight[rank : rank + 1].cuda(),
        torch.testing.make_tensor((e,), dtype=torch.bfloat16, device="cuda"),
    ...
    _assert_shape_dtype(mha_linear1_weight_grad, [d, e, e // d], torch.bfloat16)
    _assert_shape_dtype(mha_linear1_bias_grad, [e], torch.bfloat16)
    _assert_shape_dtype(mha_linear0_weight_grad, [d, e * 3 // d, e], torch.bfloat16)
    _assert_shape_dtype(mha_linear0_bias_grad, [d, e * 3 // d], torch.bfloat16)
