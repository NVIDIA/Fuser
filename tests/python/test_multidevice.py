...
class TransformerForwardFusion(FusionDefinition):
    ...
    def definition(self) -> None:
        ...
        T42 = self.ops.broadcast_in_dim(T36, shape=[1, 2048, 1], broadcast_dims=[0, 1])
        T47 = self.ops.broadcast_in_dim(T37, shape=[1, 2048, 1], broadcast_dims=[0, 1])
        S48 = self.define_scalar(1.00000e-05, dtype=DataType.Double)
        T49 = self.ops.add(T42, S48)
        T54 = self.ops.broadcast_in_dim(
            T47, shape=[1, 2048, 12288], broadcast_dims=[0, 1, 2]
        )
        T55 = self.ops.rsqrt(T49)
        T56 = self.ops.sub(T31, T54)
        T61 = self.ops.broadcast_in_dim(
            T55, shape=[1, 2048, 12288], broadcast_dims=[0, 1, 2]
        )
        T62 = self.ops.mul(T56, T61)
        T67 = self.ops.broadcast_in_dim(
            self.layernorm0_weight, shape=[1, 2048, 12288], broadcast_dims=[2]
        )
        T68 = self.ops.cast(T67, dtype=DataType.Float)
        T69 = self.ops.mul(T62, T68)
        T74 = self.ops.broadcast_in_dim(
            self.layernorm0_bias, shape=[1, 2048, 12288], broadcast_dims=[2]
        )
        ...
        T123 = self.ops.reshape(T104, new_shape=[1, 2048, 96, 128])
        T124 = self.ops.permute(T123, dims=[0, 2, 1, 3])
        T130 = self.ops.reshape(T91, new_shape=[1, 2048, 96, 128])
        T131 = self.ops.permute(T130, dims=[0, 2, 1, 3])
        T137 = self.ops.reshape(T117, new_shape=[1, 2048, 96, 128])
        ...
        T151 = self.ops.reshape(T146, new_shape=[1, 2048, 12288])
        ...
        T165 = self.ops.broadcast_in_dim(
            T159, shape=[1, 2048, 1], broadcast_dims=[0, 1]
        )
        T170 = self.ops.broadcast_in_dim(
            T160, shape=[1, 2048, 1], broadcast_dims=[0, 1]
        )
        ...
        T190 = self.ops.broadcast_in_dim(
            self.layernorm1_weight, shape=[1, 2048, 12288], broadcast_dims=[2]
        )
        T191 = self.ops.cast(T190, dtype=DataType.Float)
        T192 = self.ops.mul(T185, T191)
        T197 = self.ops.broadcast_in_dim(
            self.layernorm1_bias, shape=[1, 2048, 12288], broadcast_dims=[2]
        )
...
def test_transformer_forward(mpi_test):
    ...
    mlp_linear0_weight = torch.randn(
        num_devices, 49152 // num_devices, 12288, dtype=torch.bfloat16, device="cuda"
    )
    mlp_linear0_bias = torch.randn(
        num_devices, 49152 // num_devices, dtype=torch.bfloat16, device="cuda"
    )
    mlp_linear1_weight = torch.randn(
        num_devices, 12288, 49152 // num_devices, dtype=torch.bfloat16, device="cuda"
    )
    ins = [
        29,
        2142642406458297,
    ...
    fd = TransformerForwardFusion(num_devices)
