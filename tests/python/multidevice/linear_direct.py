...
def get_fusion_definition(
    ...
    match compute_type:
        ...
        case ComputeType.BACKWARD:
            with FusionDefinition() as bwd_fd:
                define_linear_backward(linear_config, bwd_fd)
                multidevice_schedule(bwd_fd, in_dtensors)
            get_fusion_definition.cache[key] = bwd_fd
            return bwd_fd
...
class LinearFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output: DTensor):
        ...
        in_dtensors = [input, weight, grad_output]
        op = get_fusion_definition(
            ComputeType.BACKWARD,
            LinearConfig(weight.size(1), weight.size(0), has_batch=(input.dim() == 3)),
            DTensorList(in_dtensors),
        )
        grad_x, grad_w = nvfd.dtensor_execute(op, in_dtensors)
