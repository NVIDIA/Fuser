...
def transformer_forward_tensor_parallel_schedule(
    fd: FusionDefinition, num_devices: int
):
...
def transformer_forward_sequence_parallel_schedule(
    fd: FusionDefinition, num_devices: int
):
    mesh = nvfuser.multidevice.DeviceMesh(range(num_devices))
    inputs = fd.fusion.inputs()
...
@pytest.mark.mpi
def test_transformer_forward_tensor_parallel(multidevice_direct_test, benchmark):
    d = multidevice_direct_test.size
    mesh = nvfuser.multidevice.DeviceMesh(torch.arange(d))
    ...
    with FusionDefinition() as fd:
        ...
        transformer_forward_tensor_parallel_schedule(fd, d)
...
@pytest.mark.mpi
def test_transformer_forward_sequence_parallel(multidevice_direct_test, benchmark):
    d = multidevice_direct_test.size
    mesh = nvfuser.multidevice.DeviceMesh(torch.arange(d))
    ...
    # and `define_tensor`s.
    ins = [
        multidevice_direct_test.shard_tensor(inp, 1, mesh),
        torch.testing.make_tensor((e,), dtype=torch.bfloat16, device="cuda"),
        torch.testing.make_tensor((e,), dtype=torch.bfloat16, device="cuda"),
        multidevice_direct_test.shard_tensor(mha_linear0_weight, 0, mesh),
        multidevice_direct_test.shard_tensor(mha_linear0_bias, 0, mesh),
    ...
    with FusionDefinition() as fd:
        transformer_forward_definition(fd, b, s, h, e)
        transformer_forward_sequence_parallel_schedule(fd, d)
    ...
    _assert_shape_dtype(layernorm0_mean, [b, s // d], torch.float32)
    _assert_shape_dtype(layernorm0_rstd, [b, s // d, 1], torch.float32)
    _assert_shape_dtype(mha_linear0_out, [b, s, e * 3 // d], torch.bfloat16)
    _assert_shape_dtype(sdpa_out, [b, h // d, s, e // h], torch.bfloat16)
    _assert_shape_dtype(sdpa_logsum_exp, [b, h // d, s], torch.float32)
    ref_philox_seed, ref_philox_offset = create_sdpa_rng_tensors()
    _assert_shape_dtype(sdpa_seed, ref_philox_seed.shape, ref_philox_seed.dtype)
    _assert_shape_dtype(sdpa_offset, ref_philox_offset.shape, ref_philox_offset.dtype)
    _assert_shape_dtype(mha_linear1_out, [b, s // d, e], torch.bfloat16)
    _assert_shape_dtype(mha_dropout_mask, [b, s // d, e], torch.bool)
    _assert_shape_dtype(layernorm1_mean, [b, s // d], torch.float32)
    _assert_shape_dtype(layernorm1_rstd, [b, s // d, 1], torch.float32)
    _assert_shape_dtype(mlp_linear0_out, [b, s, e * 4 // d], torch.bfloat16)
    _assert_shape_dtype(mlp_dropout_mask, [b, s // d, e], torch.bool)
    _assert_shape_dtype(out, [b, s // d, e], torch.bfloat16)
