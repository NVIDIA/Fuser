...
def test_issue4888():
    ...
    def nvfuser_fusion_id2(fd : FusionDefinition) -> None :
        T0 = fd.define_tensor(shape=[4096, 4097], contiguity=[True, True], dtype=DataType.BFloat16, is_cpu=False, stride_order=[1, 0])
        T1 = fd.define_tensor(shape=[4096, 4097], contiguity=[True, True], dtype=DataType.Bool, is_cpu=False, stride_order=[1, 0])
        T2 = fd.define_tensor(shape=[4096, 4097], contiguity=[True, True], dtype=DataType.Bool, is_cpu=False, stride_order=[1, 0])
        T3 = fd.define_tensor(shape=[1, 32, 4096, 4096], contiguity=[None, True, True, True], dtype=DataType.BFloat16, is_cpu=False, stride_order=[3, 2, 1, 0])
        T4 = fd.ops.cast(T0, dtype=DataType.Float)
        ...
        T21 = fd.ops.broadcast_in_dim(T15, shape=[1, 1, 4096, 4097], broadcast_dims=[0, 2, 3])
        T27 = fd.ops.broadcast_in_dim(T21, shape=[1, 1, 4096, 4097], broadcast_dims=[0, 1, 2, 3])
        T43 = fd.ops.slice(T27, start_indices=[0, 0, 0, 0], end_indices=[1, 1, 4096, 4096], strides=[1, 1, 1, 1], manual_normalization=0)
        T49 = fd.ops.broadcast_in_dim(T43, shape=[1, 32, 4096, 4096], broadcast_dims=[0, 1, 2, 3])
        ...
        T61 = fd.ops.broadcast_in_dim(T55, shape=[1, 32, 4096, 1], broadcast_dims=[0, 1, 2])
        T67 = fd.ops.broadcast_in_dim(T61, shape=[1, 32, 4096, 4096], broadcast_dims=[0, 1, 2, 3])
        T68 = fd.ops.sub(T54, T67)
        T69 = fd.ops.exp(T68)
        T70 = fd.ops.sum(T69, dims=[3], keepdim=False, dtype=DataType.Null)
        T76 = fd.ops.broadcast_in_dim(T70, shape=[1, 32, 4096, 1], broadcast_dims=[0, 1, 2])
        T82 = fd.ops.broadcast_in_dim(T76, shape=[1, 32, 4096, 4096], broadcast_dims=[0, 1, 2, 3])
    ...
    inputs = [
        ...
        torch.testing.make_tensor((4096, 4097), dtype=torch.bool, device='cuda:0'),
        torch.testing.make_tensor((1, 32, 4096, 4096), dtype=torch.bfloat16, device='cuda:0'),
    ]
    fd.execute(inputs)
