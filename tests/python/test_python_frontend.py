...
class TestNvFuserFrontend(TestCase):
    ...
    def test_issue_2258(self):
        ...
        # LayerNorm params
        ln_weight = torch.rand(
            w_shape, dtype=input_dtype, device="cuda", requires_grad=True
        )
        ln_bias = torch.rand(
            w_shape, dtype=input_dtype, device="cuda", requires_grad=True
        )
        ...
        with FusionDefinition() as fd:
            ...
            if zero_centered_gamma:
                G0 = fd.define_scalar(1, dtype=DataType.Int)
            ...
            V6 = fd.define_vector([T1.size(0), 1], dtype=DataType.Int)
            ...
            fd.add_output(
                T30, alias_input=amax_f
            )  # Amax output, TODO: fill input tensor instead?

        bda_out, ln_out = fd.execute(
