...
import torch
import pytest
from nvfuser import direct, DirectFusionDefinition, DataType


def test_direct_fusion_definition():
    fd = DirectFusionDefinition()
    tv0 = (
        direct.TensorViewBuilder().num_dims(3).shape([2, 4, 8]).contiguity(True).build()
    )
    tv1 = (
        direct.TensorViewBuilder().num_dims(3).shape([2, 4, 8]).contiguity(True).build()
    )
