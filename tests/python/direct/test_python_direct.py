...
def test_fusion_profiler():
    ...
    try:
        with PythonProfiler() as profiler:
            fd.execute(inputs)
        profile_results = profiler.get_fusion_profile()
        self.assertEqual(profile_results.segments, 2)
        self.assertEqual(len(profile_results.kernel_profiles), 2)
...
def test_fusion_profiler_with_noncodegen_kernels():
    ...
    try:
        ...
        profile_results = profiler.get_fusion_profile()
        self.assertEqual(len(profile_results.kernel_profiles), 2)
        self.assertGreaterEqual(len(profile_results.kernel_profiles[0].name), 0)
        self.assertGreaterEqual(len(profile_results.kernel_profiles[1].name), 0)
    except Exception as e:
        raise RuntimeError(
            "FusionDefinition's execute() did not run correctly with profile enabled!"
        )
