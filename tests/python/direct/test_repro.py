...
def test_register_aliasing_bug(nvfuser_direct_test):
    def nvfuser_fusion_id5(fd: FusionDefinition, dim0, dim1) -> None:
        T0 = fd.define_tensor(
            shape=[dim1],
            contiguity=[True],
            dtype=DataType.Float,
        ...
        T34 = fd.ops.broadcast_in_dim(T29, shape=[1, dim0, 1], broadcast_dims=[0, 1])
        T39 = fd.ops.broadcast_in_dim(
    ...
    dim1 = 32
    with FusionDefinition() as fd:
        nvfuser_fusion_id5(fd, dim0, dim1)

    inputs = [
        torch.testing.make_tensor(
            (dim1,), dtype=torch.float32, device="cuda:0", low=LOW_VAL, high=HIGH_VAL
        ),
        torch.testing.make_tensor(
            (1, dim0, dim1),
            dtype=torch.float32,
