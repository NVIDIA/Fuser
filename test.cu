...
__device__ __forceinline__ void batchWarpReduceSum(Array<T, K, K>& val, Func reduction_op)
{
    constexpr uint32_t FINAL_MASK = 0xffffffff;
    #pragma unroll
    for (int mask = 16; mask > 0; mask >>= 1) {
        Array<T, K, K> remote;
        *reinterpret_cast<uint64_t *>(remote.array) = __shfl_xor_sync(FINAL_MASK, *reinterpret_cast<uint64_t *>(val.array), mask, 32);
        #pragma unroll K
        for (int i = 0; i < K; i++) {
...
template <
    ...
    typename Func,
    typename BlockDimT>
__device__ void iterGroupedWarpReduceBcast(
    Array<T, N, 1>& out,
    const Array<T, N, 1>& inp_val,
    Func reduction_op,
    T* shared_mem,
    bool read_write_pred,
    T init_val,
  ...
  // the original N elements are packed into [N / K] elements
  constexpr int K = sizeof(uint64_t) / sizeof(T);
  // array of K elementw with type T and alignment of sizeof(T) * K
...
__global__ void nvfuser_none_f0_c0_r0_g0(Tensor<float, 2, 2> T0, Tensor<float, 1, 1> T1) {
  alignas(16) extern __shared__ char array[];
  void* shared_mem = array;
  NVFUSER_DEFINE_MAGIC_ZERO;
  nvfuser_index_t i0;
  i0 = ceilDiv((ceilDiv((ceilDiv(T0.logical_size[1LL], 4)), ((nvfuser_index_t)blockDim.x))), 2);
  nvfuser_index_t i1;
  i1 = 4 * ((nvfuser_index_t)threadIdx.x);
  nvfuser_index_t i2;
  i2 = i1 + ((4 * T0.logical_size[1LL]) * ((nvfuser_index_t)blockIdx.x));
  nvfuser_index_t i3;
  i3 = ((nvfuser_index_t)blockDim.x) * 8;
  nvfuser_index_t i4;
  i4 = 4 * ((nvfuser_index_t)blockDim.x);
  nvfuser_index_t i5;
  i5 = 4 * ((nvfuser_index_t)blockIdx.x);
  bool b6;
  b6 = (3 + i5) < T0.logical_size[0LL];
  nvfuser_index_t i7;
  i7 = ((((nvfuser_index_t)blockDim.x) * 4) + i1) + 3;
  nvfuser_index_t i8;
  i8 = (3 - T0.logical_size[1LL]) + i1;
  bool b9;
  b9 = ((nvfuser_index_t)threadIdx.x) == 0;
  Array<float, 4, 1> T4;
  #pragma unroll
  for(nvfuser_index_t i10 = 0; i10 < 4; ++i10) {
    T4[i10] = 0.000000000e+00f;
  }
  NVFUSER_UPDATE_MAGIC_ZERO;
  #pragma unroll 1
  for(nvfuser_index_t i11 = 0; i11 < i0; ++i11) {
    nvfuser_index_t i12;
    i12 = i3 * i11;
    nvfuser_index_t i13;
    i13 = i2 + i12;
    nvfuser_index_t i14;
    i14 = i8 + i12;
    if ((b6 && ((i7 + i12) < T0.logical_size[1LL]))) {
      Array<float, 32, 4> T2;
      #pragma unroll
      ...
      for(nvfuser_index_t i15 = 0; i15 < 4; ++i15) {
        nvfuser_index_t i18;
        i18 = i13 + (T0.logical_size[1LL] * i15);
        nvfuser_index_t i19;
        i19 = 8 * i15;
        #pragma unroll
        for(nvfuser_index_t i17 = 0; i17 < 2; ++i17) {
          loadGlobalToLocal<float, /*vec_size=*/4, /*is_volatile=*/false, CacheOp::Streaming>(&T2[(i19 + (4 * i17))],  &T0[(i18 + (i4 * (i17 + nvfuser_zero)))]);
        }
      }
      NVFUSER_UPDATE_MAGIC_ZERO;
      #pragma unroll
      ...
      for(nvfuser_index_t i15 = 0; i15 < 4; ++i15) {
        nvfuser_index_t i25;
        i25 = i13 + (T0.logical_size[1LL] * i15);
        nvfuser_index_t i26;
        i26 = 8 * i15;
        bool b27;
        b27 = (i5 + (i15 + nvfuser_zero)) < T0.logical_size[0LL];
        #pragma unroll
        for(nvfuser_index_t i17 = 0; i17 < 2; ++i17) {
          nvfuser_index_t i28;
          i28 = i4 * (i17 + nvfuser_zero);
          if ((b27 && (i14 < (-i28)))) {
            loadGlobalToLocal<float, /*vec_size=*/4, /*is_volatile=*/false, CacheOp::Streaming>(&T2[(i26 + (4 * i17))],  &T0[(i25 + i28)]);
          }
        }
      }
      NVFUSER_UPDATE_MAGIC_ZERO;
  ...
  if (b6) {
    ...
    #pragma unroll
    for(nvfuser_index_t i10 = 0; i10 < 4; ++i10) {
      T3[i10] = 0.000000000e+00f;
    }

    warp::iterGroupedWarpReduceBcast<false, true, 4>(T3, T4, [](float &a, float b) { a = a + b; }, static_cast<float*>(shared_mem), true, static_cast<float>(0.000000000e+00f), DefaultBlockDim());

    #pragma unroll
    for(nvfuser_index_t i31 = 0; i31 < 4; ++i31) {
      // Array<float, 1, 1> T3;
      // T3[0] = 0.000000000e+00f;
      // warp::warpReduceTIDX<false, true>(T3[0], T4[i31], [](float &a, float b) { a = a + b; }, static_cast<float*>(shared_mem), true, static_cast<float>(0.000000000e+00f), DefaultBlockDim());
      if (b9) {
        T1[(i5 + (i31 + nvfuser_zero))]
           = T3[i31];
      }
    }
    NVFUSER_UPDATE_MAGIC_ZERO;
    ...
    for(nvfuser_index_t i31 = 0; i31 < 4; ++i31) {
      nvfuser_index_t i32;
      i32 = i5 + (i31 + nvfuser_zero);
      Array<float, 1, 1> T3;
      T3[0] = 0.000000000e+00f;
      warp::warpReduceTIDX<false, true>(T3[0], T4[i31], [](float &a, float b) { a = a + b; }, static_cast<float*>(shared_mem), true, static_cast<float>(0.000000000e+00f), DefaultBlockDim());
      if ((b9 && (i32 < T0.logical_size[0LL]))) {
        T1[i32]
           = T3[0];
      }
    }
