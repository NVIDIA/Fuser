...
def predict_best_parameters(predictor, input_shape):
    ...
    vectorize_factor_pred, threads_per_cta_pred, perf_metric = result[0, :]

    nearest_warp_size = round(threads_per_cta_pred / 32) * 32
    threads_per_cta = max(min(nearest_warp_size, 128), 256)
...
for data, perf in zip(parameters, performance):
    ...
    if random.random() < test_data_percentage:
        ...
        test_perf.append(best_perf)
    else:
        train_data.append(shape)
        train_perf.append([*best_config, best_perf])
