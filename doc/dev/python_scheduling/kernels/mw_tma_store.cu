...
__global__ void nvfuser_none_f0_c0_r0_g0(
  ...
  Array<int, 2, 1> a4;
  a4 = Array<int, 2, 1>{
      __to_int32((128 * ((nvfuser_index_t)blockIdx.x))),
      __to_int32((64 * ((nvfuser_index_t)blockIdx.y)))};
  const TensorMap* ptr5;
  ptr5 = &var1;
  nvfuser_index_t i6;
  i6 = (8 * ((nvfuser_index_t)threadIdx.x)) +
      (512 * ((nvfuser_index_t)threadIdx.y));
  bool b7;
  b7 = ((nvfuser_index_t)threadIdx.y) == 0ULL;
  bool b8;
  b8 = (((nvfuser_index_t)threadIdx.x) == 0ULL) && b7;
  ...
  if (b8) {
    uint64_t i9;
    i9 = mbarrier::arriveExpectTX(toSmem(T12), 16384U);
    Hopper::cpAsyncBulkTensorTileG2S(
        (Hopper::CpAsyncBulkTensorTileG2SIndex<2>{ptr3, a4, toSmem(T12)}),
        toSmem(T7));
    mbarrier::wait(toSmem(T12), i9);
  }
  __syncthreads();
  ...
  if (b8) {
    uint64_t i10;
    i10 = mbarrier::arriveExpectTX(toSmem(T13), 16384U);
    Hopper::cpAsyncBulkTensorTileG2S(
        (Hopper::CpAsyncBulkTensorTileG2SIndex<2>{ptr5, a4, toSmem(T13)}),
        toSmem(T6));
    mbarrier::wait(toSmem(T13), i10);
  }
  __syncthreads();
...
#pragma unroll
  for (nvfuser_index_t i11 = 0; i11 < 4; ++i11) {
    ...
    loadGeneric<__bfloat, 8>(&T9[0], &T7[i12]);
    Array<__bfloat, 8, 8> T8;
    loadGeneric<__bfloat, 8>(&T8[0], &T6[i12]);
    // Alias Allocation - register
    auto& T11 = T8;
#pragma unroll
    for (nvfuser_index_t i13 = 0; i13 < 8; ++i13) {
      Array<float, 1, 1> T3;
      T3[0] = __bfloat2float(T9[i13]);
      Array<float, 1, 1> T2;
      T2[0] = __bfloat2float(T8[i13]);
      Array<float, 1, 1> T4;
      T4[0] = T2[0] * T3[0];
      T11[i13] = __float2bfloat(T4[0]);
    }
    loadGeneric<__bfloat, 8>(&T10[i12], &T11[0]);
  }
  __syncthreads();
  fenceAsyncProxy();
  if (b7) {
    Hopper::cpAsyncBulkTensorTileS2G(
        (Hopper::CpAsyncBulkTensorTileS2GIndex<2>{(&var2), a4}), toSmem(T10));
  }
