...
__global__ void nvfuser_pointwise_f0_c1_r0_g0(Tensor<__bfloat, 2, 2> T0, Tensor<__bfloat, 2, 2> T1, Tensor<__bfloat, 2, 2> T5) {
  nvfuser_index_t i0;
  ...
  int bx = blockIdx.x;
  bool valid = true;
  uint32_t arvtx = threadIdx.x == 0 ? sizeof(uint4) : 0;  // Only thread 0 has non-zero
  int iteration = 0;
  ...
  do {
    ...
    if ((((i0 + 7) + i1) < 67108864)) {
      ...
      T7.set(__bfloat(0));
      loadGlobalToLocal<__bfloat, /*vec_size=*/8, /*is_volatile=*/false, CacheOp::Streaming>(&T7[0],  &T1[i2]);
      Array<__bfloat, 8, 8> T6;
      T6.set(__bfloat(0));
      loadGlobalToLocal<__bfloat, /*vec_size=*/8, /*is_volatile=*/false, CacheOp::Streaming>(&T6[0],  &T0[i2]);
      ...
      for(nvfuser_index_t i4 = 0; i4 < 8; ++i4) {
        Array<float, 1, 1> T3;
        T3[0]
           = __bfloat2float(T7[i4]);
        Array<float, 1, 1> T2;
        T2[0]
           = __bfloat2float(T6[i4]);
        Array<float, 1, 1> T4;
        T4[0]
          = T2[0]
          * T3[0];
      ...
      loadLocalToGlobal<__bfloat, /*vec_size=*/8, /*is_volatile=*/false>( &T5[i2], &T8[0]);
    }
