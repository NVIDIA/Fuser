...
def create_fusion_func(which_fusion, inputs):
    ...
    if which_fusion == FUSION.SUM:
        return sum_fusion
    elif which_fusion == FUSION.GELU_BIAS_BWD:
...
def test_model(clf, script_config):
    ...
    plt.savefig(
        f"outer_reduction_{script_config.selected_fusion.name}_empirical_batch_size_{script_config.empirical_batch_size}.png"
    )
    plt.close("all")
