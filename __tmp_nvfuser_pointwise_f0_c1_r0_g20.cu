...
__global__ void nvfuser_pointwise_f0_c1_r0_g20(Tensor<float, 2, 2> T19, Tensor<__bfloat, 3, 3> T9, Tensor<float, 3, 3> T20, Tensor<float, 3, 3> T165, Tensor<float, 3, 3> T206, Tensor<float, 3, 3> T212, Tensor<__bfloat, 3, 3> T245) {
  nvfuser_index_t i0;
  ...
  if ((((i0 + 3) + i1) < 12288)) {
    ...
    T285.set(__bfloat(0));
    loadGlobalToLocal<__bfloat, /*vec_size=*/4, /*is_volatile=*/false, CacheOp::Streaming>(&T285[0],  &T9[i2]);
    Array<float, 1, 1> T288;
    T288[0] = 0;
    T288[0]
       = T206[((nvfuser_index_t)blockIdx.y)];
    Array<float, 4, 4> T289;
    T289.set(float(0));
    loadGlobalToLocal<float, /*vec_size=*/4, /*is_volatile=*/false, CacheOp::Streaming>(&T289[0],  &T212[i2]);
    ...
    loadLocalToGlobal<__bfloat, /*vec_size=*/4, /*is_volatile=*/false>( &T245[i2], &T290[0]);
  } else {
    Array<float, 1, 1> T286;
    T286[0] = 0;
    T286[0]
