            ...
            for USE_STREAM in "${USE_STREAM_VALUES[@]}"; do
                for USE_MATMUL_OUT in "${USE_MATMUL_OUT_VALUES[@]}"; do
                    for DISABLE_MEMORY_CACHE in "${DISABLE_MEMORY_CACHE_VALUES[@]}"; do
                        FILE_NAME="LOG2_M=${LOG2_M}__LOG2_N=${LOG2_N}__LOG2_K=${LOG2_K}__USE_STREAM=${USE_STREAM}__USE_MATMUL_OUT=${USE_MATMUL_OUT}__DISABLE_MEMORY_CACHE=${DISABLE_MEMORY_CACHE}"
