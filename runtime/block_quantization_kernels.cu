...
__device__ void block_quantize_bf16_to_nvfp4(
  ...
  __bfloat local_max = NEG_INFINITY;
#pragma unroll
  for (int i = 0; i < ITEMS_PER_THREAD; ++i) {
    local_max = fmax(local_max, abs(vec4[i]));
  }
  ...
  float block_max = NEG_INFINITY;
  quadMaxReduction(local_max);
  block_max = __bfloat2float(local_max);
...
#pragma unroll
  for (int i = 0; i < 8; ++i) {
    float upcast_in = __bfloat2float(vec4[i]);
    float scaled_val = upcast_in / clamped_max_converted;
    clamped_vals[i] = clamp(scaled_val, -6.000000000e+00f, 6.000000000e+00f);
  }
