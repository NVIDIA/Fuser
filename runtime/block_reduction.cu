...
__device__ void blockIterGroupedYdimReduce(
  ...
  unsigned int peer_stride = blockDim.x * N;
  if (read_pred) {
...
#pragma unroll
    for (int i = 0; i < N; ++i) {
      reduction_op(
          ...
          shared_mem[smem_offset + np2 * peer_stride + i]);
    }
  }
...
#pragma unroll
      for (int i = 0; i < N; ++i) {
        reduction_op(
            shared_mem[smem_offset + i],
            shared_mem[smem_offset + factor * peer_stride + i]);
      }
    }
    block_sync::sync<Aligned>();
  }
...
#pragma unroll
    for (int i = 0; i < N; ++i) {
      ...
      if (reduction_size > 1) {
        reduction_op(result, shared_mem[smem_offset + peer_stride + i]);
      }
      out[i] = result;
    }
  }
