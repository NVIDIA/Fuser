...
function toggleOldCode(testnum, kernelnum) {
  ...
  const sass_button = document.getElementById(`sassbutton_${testnum}_${kernelnum}`)
  ...
  const old_sass_div = document.getElementById(`oldsass_${testnum}_${kernelnum}`)
  const new_sass_div = document.getElementById(`newsass_${testnum}_${kernelnum}`)
  const diff_sass_div = document.getElementById(`sassdiff_${testnum}_${kernelnum}`)
  const old_div = cuda_button.disabled
    ? old_cuda_div
    : (ptx_button.disabled
      ? old_ptx_div
      : old_sass_div)
  const diff_div = cuda_button.disabled
    ? diff_cuda_div
    : (ptx_button.disabled
      ? diff_ptx_div
      : diff_sass_div)
  const new_div = cuda_button.disabled
    ? new_cuda_div
    : (ptx_button.disabled
      ? new_ptx_div
      : new_sass_div)
...
function switchDivToType(testnum, kernelnum, type) {
  const oldcode_button = document.getElementById(`oldcodebutton_${testnum}_${kernelnum}`)
  const diff_button = document.getElementById(`diffbutton_${testnum}_${kernelnum}`)
  const newcode_button = document.getElementById(`newcodebutton_${testnum}_${kernelnum}`)
  const cuda_button = document.getElementById(`cudabutton_${testnum}_${kernelnum}`)
  ...
  const new_sass_div = document.getElementById(`newsass_${testnum}_${kernelnum}`)
  const diff_sass_div = document.getElementById(`sassdiff_${testnum}_${kernelnum}`)
  cuda_button.disabled = type === "cuda"
  ptx_button.disabled = type === "ptx"
  sass_button.disabled = type === "sass"
  if (old_cuda_div.style.display === "block" ||
      old_ptx_div.style.display === "block" ||
      old_sass_div.style.display === "block") {
    old_cuda_div.style.display = type === "cuda" ? "block" : "none"
    old_ptx_div.style.display = type === "ptx" ? "block" : "none"
    old_sass_div.style.display = type === "sass" ? "block" : "none"
  } else if (new_cuda_div.style.display === "block" ||
      new_ptx_div.style.display === "block" ||
      new_sass_div.style.display === "block") {
    new_cuda_div.style.display = type === "cuda" ? "block" : "none"
    new_ptx_div.style.display = type === "ptx" ? "block" : "none"
    new_sass_div.style.display = type === "sass" ? "block" : "none"
  } else if (diff_cuda_div.style.display === "block" ||
      diff_ptx_div.style.display === "block" ||
      diff_sass_div.style.display === "block") {
    diff_cuda_div.style.display = type === "cuda" ? "block" : "none"
    diff_ptx_div.style.display = type === "ptx" ? "block" : "none"
    diff_sass_div.style.display = type === "sass" ? "block" : "none"
  }
}
function switchToCUDA(testnum, kernelnum) {
  switchDivToType(testnum, kernelnum, "cuda")
}
function switchToPTX(testnum, kernelnum) {
  switchDivToType(testnum, kernelnum, "ptx")
}
function switchToSASS(testnum, kernelnum) {
  switchDivToType(testnum, kernelnum, "sass")
}
