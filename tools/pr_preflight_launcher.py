...
from tools.claude_wrapper import launch
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Preflight launcher for PR quality checks. "
...
def build_context_from_env_and_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    # Head SHA: prefer PR_HEAD_SHA, fallback to PR_SHA (legacy name in this repo)
...
def main() -> int:
    ...
    default_dir = f"artifacts/claude_review/{(context.pr_number or 'local')}-{context.head_sha}"
    output_dir = get_env("CLAUDE_OUTPUT_DIR") or default_dir

    # Invoke Claude and propagate exit code
    exit_code = launch(prompt, VERDICT_MARKER, output_dir)

    # Short summary for logs
    print(f"Claude review artifacts: {output_dir}")
    print(f"Claude review exit code: {exit_code}")
