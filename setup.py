      ...
      with concat_third_party_license() as tp_licenses:
        ...
        if not tp_licenses.empty():
          with open('LICENSE', 'a') as f:
            f.write('\n\n')
            f.write('NVIDIA/fuser depends on libraries with license listed below:')
        ...
        for project_name, license_files in tp_licenses:
          ...
          if not identical_flag:
            raise RuntimeError("inconsistent license found for project: ", project_name, " check its license files under: ", license_files)
...
def main():
  ...
  cmd_str = [get_cmake_bin(), "--build", build_dir_name, "--target", "install", "--", "-j", max_jobs]
  subprocess.check_call(cmd_str)
