...
#pragma unroll K
  ...
  unsigned int thread_id_x = (threadIdx.x - (barrier_id-1) * block_dim.x);
  // if(blockIdx.x==0){
  //   printf("thread_id_x: %d, threadIdx.x: %d, barrier_id: %d, inp_val[0]= %f\n", thread_id_x, threadIdx.x, barrier_id, inp_val[0]);
  // }
  unsigned int warp_idx = thread_id_x  / WARP_SIZE;
  unsigned int lane_idx = thread_id_x % WARP_SIZE;
...
__global__ void __launch_bounds__(/*MAX_THREADS_PER_BLOCK=*/384, 1) nvfuser_inner_persistent_f0_c1_r0_g0(Tensor<__half, 2, 2> T0, Tensor<__half, 2, 2> T5) {
  alignas(16) extern __shared__ char array[];
  void* shared_mem = array;
  const unsigned smem_offset = alignBufferSize(((ceilDiv((ceilDiv((ceilDiv(T0.logical_size[1LL], 8)), 4)), 32)) * 32) * 1 * 1 * sizeof(float), 16);
  nvfuser_index_t i0;
  i0 = ceilDiv((ceilDiv(T0.logical_size[0LL], 4)), 148);
  nvfuser_index_t i1;
  i1 = 8 * T0.logical_size[1LL];
  uint32_t i2;
  i2 = (uint32_t)(i1);
  nvfuser_index_t i3;
  i3 = 2 * T0.logical_size[1LL];
  uint32_t i4;
  i4 = (uint32_t)(i3);
  nvfuser_index_t i5;
  ...
  i6 = toSmem(T8);
  nvfuser_index_t i7;
  i7 = 4 * T0.logical_size[1LL];
  nvfuser_index_t i8;
  i8 = ceilDiv((ceilDiv(T0.logical_size[1LL], 8)), 4);
  nvfuser_index_t i9;
  i9 = 8 * i8;

  nvfuser_index_t local_tidx = threadIdx.x % 128;
  nvfuser_index_t i10;
  i10 = 8 * local_tidx;
  nvfuser_index_t i11;
  i11 = i10 + (i7 * ((nvfuser_index_t)blockIdx.x));
  nvfuser_index_t i12;
  i12 = 592 * T0.logical_size[1LL];
  bool b13;
  b13 = ((nvfuser_index_t)threadIdx.x) < 32ULL;

  nvfuser_index_t i15;
  i15 = (-T0.logical_size[0LL]) + i5;
  nvfuser_index_t i16;
  i16 = (7 - T0.logical_size[1LL]) + i10;
  bool b17;
  b17 = ((i10 + 7) + (24 * i8)) < T0.logical_size[1LL];
  nvfuser_index_t i18;
  i18 = 3 + i5;
  uint64_t* T12 = reinterpret_cast<uint64_t*>(array + smem_offset + 0);
  #pragma unroll
  for(nvfuser_index_t i19 = 0; i19 < 4; ++i19) {
    if ((Hopper::electSync(4294967295U) && b13)) {
      mbarrier::init(toSmem((&T12[i19])), 1U);
    }
  }
  #pragma unroll
  for(nvfuser_index_t i20 = 0; i20 < 4; ++i20) {
    if ((Hopper::electSync(4294967295U) && b13)) {
      mbarrier::init(toSmem((&T12[(i20 + 4LL)])), (uint32_t)(((ceilDiv((ceilDiv((ceilDiv(T0.logical_size[1LL], 8)), 4)), 32)) * 32)));
    }
  }
  __syncthreads();
  if ((((nvfuser_index_t)threadIdx.x) >= (((nvfuser_index_t)blockDim.x) + -128))) {
    decreaseRegisters<56>();
    if ((((nvfuser_index_t)threadIdx.x) == (((nvfuser_index_t)blockDim.x) + -1))) {
      #pragma unroll 3
      for(nvfuser_index_t i21 = 0; i21 < i0; ++i21) {
        ...
        uint32_t i23;
        i23 = i6 + (i1 * (i21 % 4));
        mbarrier::waitParity(toSmem((&T12[((i21 % 4) + 4LL)])), (uint32_t)(((i21 / 4) % 2)));
        mbarrier::arriveExpectTX(toSmem((&T12[(i21 % 4)])), i2);
        #pragma unroll
        for(nvfuser_index_t i24 = 0; i24 < 4; ++i24) {
          Hopper::cpAsyncBulkG2S((Hopper::CpAsyncBulkG2SIndex{ (T0.data + (T0.logical_size[1LL] * ((i22 + i24) % T0.logical_size[0LL]))), i4, toSmem((&T12[(i21 % 4)])) }), (i23 + (i3 * i24)));
        }
      }
    }
    return;
  } else {
    increaseRegisters<224>();

    
    nvfuser_index_t warp_group_id = threadIdx.x / 128;
    bool b14;
    b14 = local_tidx < i8;

    // only need 128 threads arrive at this barrier
    if(warp_group_id == 0){
      #pragma unroll
      for(nvfuser_index_t i25 = 0; i25 < 4; ++i25) {
        mbarrier::arrive(toSmem((&T12[(i25 + 4LL)])));
      }
    }

    #pragma unroll 1
    for(nvfuser_index_t i26 = warp_group_id; i26 < i0; i26 += 2) {
      ...
      mbarrier::waitParity(toSmem((&T12[(i26 % 4)])), (uint32_t)(((i26 / 4) % 2)));
      #pragma unroll
      for(nvfuser_index_t i31 = 0; i31 < 4; ++i31) {
        nvfuser_index_t i34;
        i34 = i27 + (T0.logical_size[1LL] * i31);
        nvfuser_index_t i35;
        i35 = 32 * i31;
        bool b36;
        b36 = b14 && (i30 < (-i31));
        ...
        for(nvfuser_index_t i33 = 0; i33 < 4; ++i33) {
          ...
          i37 = i9 * i33;
          if ((b36 && (i16 < (-i37)))) {
            loadGeneric<__half, 8>( &T10[(i35 + (8 * i33))],  &T8[(i34 + i37)]);
            // if(blockIdx.x==0 && local_tidx == 0){
            //   printf("tidx: %03d, i26=%d T10= %f, T8=%f, T8id= %d\n", threadIdx.x, i26, __half2float(T10[(i35 + (8 * i33))]),
            //          __half2float(T8[(i34 + i37)]), (i34 + i37));
            // }            
          }
          // else{
          //   if(blockIdx.x==0 && local_tidx == 0){
          //     printf("failed predicate tidx: %03d, i26=%d b36=%d i16=%d i37=%d \n", threadIdx.x, i26,
          //           b36, i16, i37);
          //   } 
          // }
        }
      ...
      mbarrier::arrive(toSmem((&T12[((i26 % 4) + 4LL)])));

      Array<float, 4, 1> T11;
      #pragma unroll
      for(nvfuser_index_t i38 = 0; i38 < 4; ++i38) {
        T11[i38] = 0.000000000e+00f;
      }
      #pragma unroll
      for(nvfuser_index_t i38 = 0; i38 < 4; ++i38) {
        nvfuser_index_t i39;
        ...
        for(nvfuser_index_t i41 = 0; i41 < 4; ++i41) {
          ...
          b43 = b40 && (i16 < (-(i9 * i41)));
          #pragma unroll
          for(nvfuser_index_t i44 = 0; i44 < 8; ++i44) {
            Array<float, 1, 1> T1;
            T1[0]
               = __half2float(T10[(i42 + i44)]);
            if (b43) {
              T11[i38]
                = T11[i38]
                + T1[0];
      ...
      for(nvfuser_index_t i45 = 0; i45 < 4; ++i45) {
        T2[i45] = 0.000000000e+00f;
      }
      warp::iterGroupedWarpReduce<false, false, true, 4>(T2.array, T11.array, [](float &a, float b) { a = a + b; },
       static_cast<float*>(shared_mem), true, static_cast<float>(0.000000000e+00f), 
       dim3(((ceilDiv((ceilDiv((ceilDiv(T0.logical_size[1LL], 8)), 4)), 32)) * 32), 1, 1), warp_group_id + 1);
      ...
      for(nvfuser_index_t i46 = 0; i46 < 4; ++i46) {
        ...
        for(nvfuser_index_t i48 = 0; i48 < 4; ++i48) {
          ...
          i49 = i47 + (8 * i48);
          #pragma unroll
          for(nvfuser_index_t i50 = 0; i50 < 8; ++i50) {
            nvfuser_index_t i51;
            i51 = i49 + i50;
            Array<float, 1, 1> T6;
            T6[0]
               = __half2float(T10[i51]);
            Array<float, 1, 1> T4;
            T4[0]
              = T6[0]
              + T3[0];
      ...
      if ((b17 && ((i18 + i29) < T0.logical_size[0LL]))) {
        #pragma unroll
        for(nvfuser_index_t i52 = 0; i52 < 4; ++i52) {
          nvfuser_index_t i53;
          i53 = 32 * i52;
          nvfuser_index_t i54;
          i54 = i28 + (T0.logical_size[1LL] * i52);
          #pragma unroll
          for(nvfuser_index_t i55 = 0; i55 < 4; ++i55) {
            loadLocalToGlobal<__half, /*vec_size=*/8, /*is_volatile=*/false>( &T5[(i54 + (i9 * i55))], &T9[(i53 + (8 * i55))]);
          }
        }
      } else {
        #pragma unroll
        for(nvfuser_index_t i52 = 0; i52 < 4; ++i52) {
          ...
          i56 = 32 * i52;
          nvfuser_index_t i57;
          i57 = i28 + (T0.logical_size[1LL] * i52);
          bool b58;
          b58 = b14 && (i30 < (-i52));
          #pragma unroll
          for(nvfuser_index_t i55 = 0; i55 < 4; ++i55) {
            ...
            i59 = i9 * i55;
            if ((b58 && (i16 < (-i59)))) {
              loadLocalToGlobal<__half, /*vec_size=*/8, /*is_volatile=*/false>( &T5[(i57 + i59)], &T9[(i56 + (8 * i55))]);
            }
          }
        }
      }
  ...
  for(nvfuser_index_t i60 = 0; i60 < 4; ++i60) {
    if ((Hopper::electSync(4294967295U) && b13)) {
      mbarrier::inval(toSmem((&T12[(i60 + 4LL)])));
    }
  }
  #pragma unroll
  for(nvfuser_index_t i61 = 0; i61 < 4; ++i61) {
    if ((Hopper::electSync(4294967295U) && b13)) {
      mbarrier::inval(toSmem((&T12[i61])));
    }
  }
}
}
...
// MAX_STAGES=4 \
// USE_TMA=1 \
// NVFUSER_PROF=print NVFUSER_DUMP=fusion_ir_math,scheduler_params,launch_param,cuda_to_file,ptxas_verbose,occupancy,segmenter_logging,ptx \
//   /opt/pytorch/nvfuser/build/test_nvfuser  --gtest_filter=${VALUE} 2>&1 |tee 1.log
