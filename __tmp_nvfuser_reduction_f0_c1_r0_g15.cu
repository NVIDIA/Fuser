...
__global__ void nvfuser_reduction_f0_c1_r0_g15(Tensor<float, 3, 3> T163, Tensor<float, 3, 3> T156, Tensor<float, 3, 3> T155, Tensor<float, 3, 3> T117, Tensor<float, 3, 3> T165, Tensor<float, 3, 3> T206, Tensor<float, 3, 3> T212) {
  alignas(16) extern __shared__ char array[];
  void* shared_mem = array;
  nvfuser_index_t i0;
  i0 = ceilDiv(12288, ((nvfuser_index_t)blockDim.x));
  nvfuser_index_t i1;
  i1 = ((nvfuser_index_t)threadIdx.x) + (12288 * ((nvfuser_index_t)blockIdx.x));
  bool b2;
  b2 = ((nvfuser_index_t)threadIdx.x) == 0;
  Array<float, 1, 1> T282;
  T282[0] = 0.000000000e+00f;
  #pragma unroll 1
  for(nvfuser_index_t i3 = 0; i3 < i0; ++i3) {
    nvfuser_index_t i4;
    i4 = ((nvfuser_index_t)blockDim.x) * i3;
    nvfuser_index_t i5;
    i5 = i1 + i4;
    if (((((nvfuser_index_t)threadIdx.x) + i4) < 12288)) {
      Array<float, 1, 1> T157;
      T157[0] = 0;
      T157[0]
        = T156[i5]
        * T155[i5];
      Array<float, 1, 1> T158;
      T158[0]
         = T157[0];
      T282[0]
        = T282[0]
  ...
  Array<float, 1, 1> T159;
  T159[0] = 0.000000000e+00f;
  warp::warpReduceTIDX<false, true>(T159[0], T282[0], [](float &a, float b) { a = a + b; }, static_cast<float*>(shared_mem), true, static_cast<float>(0.000000000e+00f), DefaultBlockDim());
  Array<float, 1, 1> T160;
  T160[0]
     = T159[0];
  if (b2) {
    T165[((nvfuser_index_t)blockIdx.x)]
      = (float) -5.00000000000000000e-01
      * T160[0];
  }
  Array<float, 1, 1> T283;
  T283[0] = 0.000000000e+00f;
  #pragma unroll 1
  for(nvfuser_index_t i6 = 0; i6 < i0; ++i6) {
    nvfuser_index_t i7;
    i7 = ((nvfuser_index_t)blockDim.x) * i6;
    nvfuser_index_t i8;
    i8 = i1 + i7;
    if (((((nvfuser_index_t)threadIdx.x) + i7) < 12288)) {
      Array<float, 1, 1> T166;
      T166[0] = 0;
      T166[0]
        = T163[((nvfuser_index_t)blockIdx.x)]
        * T155[i8];
      Array<float, 1, 1> T168;
      T168[0]
         = -T166[0];
      Array<float, 1, 1> T170;
      T170[0]
         = T168[0];
      T283[0]
        = T283[0]
  ...
  Array<float, 1, 1> T171;
  T171[0] = 0.000000000e+00f;
  warp::warpReduceTIDX<false, true>(T171[0], T283[0], [](float &a, float b) { a = a + b; }, static_cast<float*>(shared_mem), true, static_cast<float>(0.000000000e+00f), DefaultBlockDim());
  Array<float, 1, 1> T174;
  T174[0]
     = T171[0];
  Array<float, 1, 1> T199;
  T199[0]
     = T174[0];
  if (b2) {
    T206[((nvfuser_index_t)blockIdx.x)]
