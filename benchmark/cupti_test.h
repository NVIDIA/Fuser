...
inline void createCounterDataImage(
    int numRanges,
    std::vector<uint8_t>& counterDataImagePrefix,
    std::vector<uint8_t>& counterDataScratchBuffer,
    std::vector<uint8_t>& counterDataImage) {
...
inline void stopProfiling(ProfilingData_t* pProfilingData) {
  ...
  WriteBinaryFile(
      pProfilingData->CounterDataFileName.c_str(),
      pProfilingData->counterDataImage);
  WriteBinaryFile(
      pProfilingData->CounterDataSBFileName.c_str(),
      pProfilingData->counterDataScratchBuffer);
}

inline void callbackHandler(
    void* userdata,
...
inline ProfilingData_t* initCupti(int argc, char** argv) {
  ...
  CUPTI_API_CALL(cuptiEnableCallback(
      1,
      subscriber,
...
inline void finishCupti(ProfilingData_t* profilingData) {
  if (profilingData->bProfiling) {
    ...
    /* Evaluation of metrics collected in counterDataImage, this can also be
     * done offline*/
    NV::Metric::Eval::PrintMetricValues(
        profilingData->chipName,
